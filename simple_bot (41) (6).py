...
def generate_pdf_results(quiz_id, title=None):
    ...
    # First try the comprehensive PDF generator that matches the original format
    try:
        from comprehensive_pdf import generate_comprehensive_pdf
        logger.info("Using comprehensive PDF generator that matches original format")
        ...
        result_file = generate_comprehensive_pdf(quiz_id, title, leaderboard, penalty_value)
        if result_file and os.path.exists(result_file):
            logger.info(f"Successfully generated comprehensive PDF: {result_file}")
            return result_file
    except Exception as e:
        logger.warning(f"Comprehensive PDF generation failed: {e}, trying fallback approaches")
    
    # If comprehensive PDF fails, try our guaranteed simple solution
    try:
        from final_solution import create_guaranteed_pdf
        logger.info("Using guaranteed simple PDF solution")
        result_file = create_guaranteed_pdf(quiz_id, title)
        if result_file and os.path.exists(result_file):
            logger.info(f"Successfully generated simple PDF: {result_file}")
            return result_file
    except Exception as e:
        logger.warning(f"Simple PDF solution failed: {e}, falling back to built-in approach")

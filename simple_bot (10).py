...
async def receive_txt_file(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    file_path = os.path.join(TEMP_DIR, f"{file.file_id}_{file.file_name}")
...
async def set_custom_id_txt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Set custom ID for the imported questions from text file"""
    custom_id = update.message.text.strip()
    ...
    logger.info(f"Custom ID stored in context: {custom_id}")
    ...
    # Process directly after receiving custom ID
    try:
        # Process file directly instead of transitioning to next conversation state
        await process_txt_file(update, context)
        return ConversationHandler.END
    except Exception as e:
        logger.error(f"Error processing file after custom ID: {e}")
        await update.message.reply_text(
...
def main() -> None:
    ...
    # Set up Text Import conversation states
    TXT_UPLOAD, TXT_CUSTOM_ID, TXT_PROCESSING = range(200, 203)
    ...
    txtimport_handler = ConversationHandler(
        ...
        states={
            ...
            TXT_PROCESSING: [
                # This state is called automatically after the custom ID is set
                CommandHandler("cancel", txtimport_cancel),
            ],
        },

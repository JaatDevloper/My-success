    ...
    def create_leaderboard_table(self, leaderboard):
        ...
        for entry in leaderboard:
            ...
            try:
                ...
                # Clean name by removing HTML-like tags that might appear in some names
                cleaned_name = raw_name.replace('<', '').replace('>', '').replace('/', '')
                
                # Try to preserve as much of the name as possible
                display_name = cleaned_name
                
                # If name still has problematic characters, create a more readable version
                if any(ord(c) > 127 for c in cleaned_name):
                    # Use first part of name or username if available
                    ...
                    # Add user_id to make it unique if available
                    user_id = entry.get('user_id')
                    if user_id:
                        display_name += f" #{str(user_id)[-4:]}"
            except Exception as e:
                # Fallback to a safe name
                display_name = f"User {entry.get('rank', '')}"
...
def generate_pdf_results(quiz_id, title=None):
    ...
    try:
        ...
        try:
            # Use 'utf8' encoding option to handle non-Latin characters
            pdf.output(filename, 'F')
            logger.info("PDF output completed")
        except Exception as e:
            logger.error(f"Error in pdf.output: {e}")
            
            # Try with encoding fallback by replacing problematic characters
            try:
                # Try to create a simpler PDF with actual content but avoiding encoding issues
                logger.info("Trying to create PDF with character replacement and actual quiz data...")
                ...
                # Safely add leaderboard entries
                rank = 1
                if leaderboard and isinstance(leaderboard, list):
                    for entry in leaderboard:
                        try:
                            ...
                            # Use a separate variable for display in the PDF to handle encoding
                            display_name = name

...
def ensure_pdf_directory():
    ...
    logger.info(f"Setting PDF directory to current directory: {PDF_RESULTS_DIR}")
    
    # Make sure we have write access
    try:
        # Test write permissions with a small test file
        test_file = os.path.join(PDF_RESULTS_DIR, f"test_write_{datetime.datetime.now().strftime('%H%M%S')}.txt")
        with open(test_file, 'w') as f:
            f.write("Test write access")
        # If we get here, the directory is writable
        os.remove(test_file)
        logger.info(f"Current directory is writable for PDFs")
        return True
    except Exception as e:
        logger.error(f"Error testing write access: {e}")
        # Just return True anyway, we'll handle any write errors during PDF generation
        return True
...
def get_quiz_penalty(quiz_id):
    ...
    try:
        ...
        # Make sure penalties is a dictionary
        if not isinstance(penalties, dict):
            logger.error(f"penalties is not a dictionary, it's {type(penalties)}: {penalties}")
            return DEFAULT_PENALTY
            
        # Convert quiz_id to string for dictionary lookup
        quiz_id_str = str(quiz_id)
        penalty_value = penalties.get(quiz_id_str, DEFAULT_PENALTY)
        
        # Try to convert to float (if it's a string)
        if isinstance(penalty_value, str):
            try:
                return float(penalty_value)
            except (ValueError, TypeError):
                logger.error(f"Could not convert penalty value '{penalty_value}' to float")
                return DEFAULT_PENALTY
                
        return penalty_value
    except Exception as e:
        logger.error(f"Error in get_quiz_penalty: {e}")
        return DEFAULT_PENALTY
...
def get_quiz_leaderboard(quiz_id):
    ...
    try:
        ...
        # Check if quiz_results is a dictionary (expected) or something else
        if not isinstance(quiz_results, dict):
            logger.error(f"quiz_results is not a dictionary, it's {type(quiz_results)}: {quiz_results}")
            # Handle the case where quiz_results is a string or other non-dict type
        ...
        sorted_participants = sorted(
            participants, 
            key=lambda x: float(x.get("adjusted_score", 0)) if isinstance(x, dict) else 0, 
            reverse=True
        )
        ...
        for participant in sorted_participants:
            # Make sure participant is a dictionary
            if not isinstance(participant, dict):
                logger.warning(f"Skipping non-dictionary participant: {participant}")
                continue
        ...
        logger.error(f"Error in get_quiz_leaderboard: {e}")
        # Return an empty list as fallback to avoid errors
        return []
...
class InsaneResultPDF(FPDF):
    ...
    def __init__(self, quiz_id, title=None):
        # Initialize with valid FPDF parameters
        super().__init__()  # Use default parameters which are valid
        ...
        logger.info(f"Created InsaneResultPDF for quiz {quiz_id} with title: {self.title}")
        
        # Add alias for nb_pages for proper page numbering
        self.alias_nb_pages()
    ...
    def header(self):
        try:
            ...
            self.rect(0, 22, 210, 3, style='F')
            
            # Add quiz icon/symbol
            self.set_xy(15, 6)
            ...
            self.set_text_color(*self.text_light)
            self.cell(110, 10, self.title, 0, 0, 'L')
            
            # Add date and quiz ID in right corner with better formatting
            self.set_xy(150, 4) 
            self.set_font('Arial', 'I', 8)
            self.set_text_color(*self.text_light)
            self.cell(45, 6, f'Quiz ID: {self.quiz_id}', 0, 2, 'R')
            self.cell(45, 6, f'Generated: {datetime.datetime.now().strftime("%Y-%m-%d %H:%M")}', 0, 0, 'R')
            
            # Add a subtle decorative element below the header
            self.set_y(30)
            for i in range(15, 195, 5):
                if i % 2 == 0:
                    self.set_draw_color(*self.brand_primary)
                else:
                    self.set_draw_color(*self.brand_secondary)
                self.set_line_width(0.2)
                self.line(i, 30, i+3, 30)
            
            # Reset to original position plus offset for content
    ...
    def footer(self):
        try:
            # Create a more elegant footer with design elements
            
            # First add a subtle dot pattern before the footer
            self.set_y(-25)
            self.set_draw_color(*self.brand_secondary)
            self.set_line_width(0.2)
            
            # Draw dotted line with alternating colors
            for x in range(15, 195, 3):
                if x % 2 == 0:
                    self.set_draw_color(*self.brand_primary)
                else:
                    self.set_draw_color(*self.brand_secondary)
                self.line(x, self.get_y(), x+1, self.get_y())
            
            # Add a subtle gradient-like footer background
            self.set_fill_color(*self.brand_primary)
            self.rect(0, -15, 210, 15, style='F')
            
            # Add thin accent line at top of footer
            self.set_fill_color(*self.brand_secondary)
            self.rect(0, -15, 210, 2, style='F')
            
            # Add QR code icon suggestion on the left (symbolic)
            self.set_y(-12)
            self.set_x(15)
            self.set_font('Arial', 'B', 8)
            self.set_text_color(*self.text_light)
            self.cell(10, 5, "🔍", 0, 0, 'C')
            
            # Add professional branding with better typography
            self.set_x(25)
            self.set_font('Arial', 'B', 8)
            self.set_text_color(*self.text_light)
            self.cell(100, 5, f'PREMIUM QUIZ BOT © {datetime.datetime.now().year}', 0, 0, 'L')
            
            # Add page numbering with elegant styling
            self.set_x(125)
            self.set_font('Arial', 'I', 8)
            self.set_text_color(*self.text_light)
            current_page = self.page_no()
            total_pages = "{nb}" # This will be replaced by total pages
            self.cell(60, 5, f'Page {current_page}/{self.alias_nb_pages()}', 0, 0, 'R')
        except Exception as e:
    ...
    def add_watermark(self):
        ...
        try:
            ...
            # Set very light version of brand color for the main watermark
            r, g, b = self.brand_primary
            light_color = (min(r+200, 255), min(g+200, 255), min(b+200, 255))
            
            # Create large PREMIUM watermark
            self.set_font('Arial', 'B', 80)
            self.set_text_color(*light_color)
            
            # Position the watermark diagonally across the page
            # No rotation - just place it directly for better compatibility
            self.set_xy(35, 100)
            self.cell(140, 40, "PREMIUM", 0, 0, 'C')
            
            # Add a small "VERIFIED RESULTS" banner in the top right
            r2, g2, b2 = self.brand_secondary
            light_secondary = (min(r2+180, 255), min(g2+180, 255), min(b2+180, 255))
            
            self.set_xy(140, 40)
            self.set_font('Arial', 'B', 10)
            self.set_text_color(*light_secondary)
            self.cell(50, 10, "VERIFIED RESULTS", 0, 0, 'R')
            
            # Add subtle quiz icons as part of the watermark design
            symbols = ["✓", "✗", "?", "!"]
            self.set_font('Arial', '', 12)
            
            # Scatter symbols across the page with very light coloring
            for i in range(15):
                # Calculate random-like but deterministic positions
                sym_x = ((i * 37) % 170) + 20
                sym_y = ((i * 17) % 240) + 30
                ...
                self.set_xy(sym_x, sym_y)
                self.cell(10, 10, symbol, 0, 0, 'C')
    ...
    def create_leaderboard_table(self, leaderboard):
        """Create a beautiful leaderboard table showing all participants with their scores and answers"""
        self.add_watermark()
        
        # Debug info
        logger.info(f"Creating leaderboard table with {len(leaderboard) if leaderboard else 0} entries")
        
        # Add section title with branded styling
        self.ln(5)
        self.set_font('Arial', 'B', 16)
        self.set_text_color(*self.brand_primary)
        self.cell(0, 10, "COMPLETE LEADERBOARD", 0, 1, 'L')
        
        # Add subtitle explaining the section
        self.set_font('Arial', 'I', 9)
        self.set_text_color(*self.text_dark)
        self.cell(0, 5, "Showing all participants with their scores, right/wrong answers, and penalties", 0, 1, 'L')
        ...
        # Column widths - make participant column wider
        col_widths = [15, 65, 20, 20, 20, 20, 20]
        header_texts = ["Rank", "Participant", "Score", "Right", "Wrong", "Skip", "Penalty"]
        ...
        if not leaderboard or len(leaderboard) == 0:
            ...
            self.set_x(15)
            self.cell(sum(col_widths), 10, "No participant data available", 1, 1, 'C', True)
            return
            
        # Add rank to each entry if not already present
        ranked_leaderboard = []
        for i, entry in enumerate(sorted(leaderboard, key=lambda x: float(x.get("adjusted_score", 0)), reverse=True)):
            entry_copy = entry.copy()
            entry_copy["rank"] = entry.get("rank", i+1)
            ranked_leaderboard.append(entry_copy)
            
        # Table rows with improved styling
        alternate_color = False
        highlighted_top = False  # To highlight top performers
        
        for i, entry in enumerate(ranked_leaderboard):
            # Special highlight for top 3 performers
            if i < 3 and not highlighted_top:
                if i == 0:  # Gold (1st)
                    self.set_fill_color(255, 223, 0, 0.2)  # Gold with transparency
                    medal = "🥇 "
                elif i == 1:  # Silver (2nd)
                    self.set_fill_color(192, 192, 192, 0.2)  # Silver with transparency
                    medal = "🥈 "
                elif i == 2:  # Bronze (3rd)
                    self.set_fill_color(205, 127, 50, 0.2)  # Bronze with transparency
                    medal = "🥉 "
            else:
                # Regular alternating colors for other participants
                if alternate_color:
            ...
            # Add medal icon to top performers
            if medal:
                display_name = medal + display_name
            
            # Set special text color based on performance metrics
            ...
            # Row content with special formatting
            self.set_x(15)
            
            # Rank with standard formatting
            self.set_text_color(*self.text_dark)
            self.set_font('Arial', 'B', 10)
            self.cell(col_widths[0], 10, str(rank), 1, 0, 'C', True)
            
            # Name with possible highlighting for top performers
            if i < 3:
                self.set_font('Arial', 'B', 10)
            else:
                self.set_font('Arial', '', 10)
            self.cell(col_widths[1], 10, display_name[:30], 1, 0, 'L', True)
            
            # Score with custom color based on value
            if score > 80:
                self.set_text_color(0, 128, 0)  # Green for high scores
            elif score < 40:
                self.set_text_color(192, 0, 0)  # Red for low scores
            else:
                self.set_text_color(*self.text_dark)  # Default for middle scores
            self.set_font('Arial', 'B', 10)
            self.cell(col_widths[2], 10, str(score), 1, 0, 'C', True)
            
            # Right answers - always green
            self.set_text_color(0, 128, 0)  # Green
            self.set_font('Arial', '', 10)
            self.cell(col_widths[3], 10, str(correct), 1, 0, 'C', True)
            
            # Wrong answers - always red
            self.set_text_color(192, 0, 0)  # Red
            self.cell(col_widths[4], 10, str(wrong), 1, 0, 'C', True)
            
            # Skipped - blue
            self.set_text_color(0, 0, 192)  # Blue
            self.cell(col_widths[5], 10, str(skipped), 1, 0, 'C', True)
            
            # Penalty - orange
            self.set_text_color(*self.brand_secondary)  # Orange
            self.cell(col_widths[6], 10, str(penalty), 1, 0, 'C', True)
    ...
    def add_quiz_statistics(self, leaderboard, penalty_value):
        ...
        if not leaderboard or len(leaderboard) == 0:
            logger.info("No leaderboard data available for quiz statistics")
            self.set_font('Arial', 'I', 10)
            self.set_text_color(*self.text_dark)
            self.cell(0, 10, "No quiz data available", 0, 1, 'L')
            return
    ...
    def add_topper_comparison(self, leaderboard):
        """Add enhanced top performers comparison with detailed analytics"""
        if not leaderboard or len(leaderboard) < 1:
            logger.info("No leaderboard data available for topper comparison")
            # Add message in the chart area
            self.ln(15)
            self.set_font('Arial', 'B', 16)
            self.set_text_color(*self.brand_primary)
            self.cell(0, 10, "TOPPER COMPARISONS", 0, 1, 'L')
            
            # Add subtitle
            self.set_font('Arial', 'I', 9)
            self.set_text_color(*self.text_dark)
            self.cell(0, 5, "Analysis of top performers and their comparison with other participants", 0, 1, 'L')
            ...
            self.set_font('Arial', 'I', 10)
            self.set_text_color(*self.text_dark)
            self.cell(0, 10, "No participant data available for comparison", 0, 1, 'L')
            return
        ...
        try:
            ...
            self.set_font('Arial', '', 10)
            self.set_text_color(*self.text_dark)
            self.multi_cell(0, 7, "The following analysis compares top performers with the average participant, highlighting performance metrics across different categories.", 0, 'L')
            self.ln(5)
            ...
            # Create champions podium styling
            self.set_fill_color(245, 245, 245)  # Background color
            self.rect(15, self.get_y(), 180, 15, style='F')
            
            # Add champions header
            self.set_xy(15, self.get_y())
            self.set_font('Arial', 'B', 12)
            self.set_text_color(*self.brand_primary)
            self.cell(180, 15, "🏆 QUIZ CHAMPIONS 🏆", 0, 1, 'C')
            self.ln(5)
            ...
            col_widths = [10, 50, 25, 25, 25, 25, 20]
            header_texts = ["Rank", "Participant", "Score", "Correct", "Wrong", "Skipped", "Penalty"]
            ...
            # Draw rows for top performers with medal styling
            medal_colors = [
                (255, 215, 0),  # Gold
                (192, 192, 192), # Silver
                (205, 127, 50)   # Bronze
            ...
            for i, performer in enumerate(top_performers):
                # Use medal-specific background color with transparency
                r, g, b = medal_colors[i] if i < 3 else (245, 245, 245)
                self.set_fill_color(r, g, b, 0.3)  # Light version of medal color
                
                # Format the name with better styling
                try:
                ...
                # Print row with medal designation
                self.set_x(15)
                
                # Rank column with medal styling
                self.set_text_color(*self.text_dark)
                self.set_font('Arial', 'B', 10)
                self.cell(col_widths[0], 10, str(i+1), 1, 0, 'C', True)
                
                # Name column with medal designation
                self.set_font('Arial', 'B', 10)
                self.cell(col_widths[1], 10, f"{medal_names[i] if i < 3 else ''}: {display_name}", 1, 0, 'L', True)
                
                # Add metrics with color-coded text and performance indicators
                metrics_data = [
                ...
                # Compare with average and add visual indicators
                avg_metrics = [avg_score, avg_correct, avg_wrong, avg_skipped, avg_penalty]
                
                for j, value in enumerate(metrics_data):
                    # Determine comparison symbol (better/worse than average)
                    if j in [0, 1]:  # Score, Correct - higher is better
                        if value > avg_metrics[j] * 1.2:
                            indicator = " ↑↑"  # Much better
                        elif value > avg_metrics[j]:
                            indicator = " ↑"   # Better
                        else:
                            indicator = ""     # Average or worse
                    elif j in [2, 3, 4]:  # Wrong, Skipped, Penalty - lower is better
                        if value < avg_metrics[j] * 0.8:
                            indicator = " ↑↑"  # Much better (lower)
                        elif value < avg_metrics[j]:
                            indicator = " ↑"   # Better (lower)
                        else:
                            indicator = ""     # Average or worse
                    else:
                        indicator = ""
                    
                    # Use bold for values that are significantly better than average
                    if indicator == " ↑↑":
                        self.set_font('Arial', 'B', 10)
                    else:
                        self.set_font('Arial', '', 10)
                    ...
                    self.cell(col_widths[j+2], 10, f"{value}{indicator}", 1, 0, 'C', True)
            ...
            self.cell(col_widths[0], 10, "", 1, 0, 'C', True)
            self.cell(col_widths[1], 10, "AVERAGE (All Participants)", 1, 0, 'L', True)
            ...
            for j, value in enumerate(avg_metrics):
                self.set_text_color(*metric_colors[j])
                self.set_font('Arial', 'B', 10)
                self.cell(col_widths[j+2], 10, str(value), 1, 0, 'C', True)
            ...
            self.set_font('Arial', 'B', 14)
            self.set_text_color(*self.brand_primary)
            self.cell(0, 10, "Performance Gap Analysis", 0, 1, 'L')
            
            # Add visualization explanation
            self.set_font('Arial', '', 9)
            self.set_text_color(*self.text_dark)
            self.multi_cell(0, 5, "This visualization shows how top performers compare to the average participant in different performance metrics.", 0, 'L')
            self.ln(5)
            
            # Create comparison bars visualization
            bar_metrics = ["Score", "Correct Answers", "Wrong Answers", "Penalty Impact"]
            bar_values = [
                [top_performers[0].get("adjusted_score", 0), avg_score],
                [top_performers[0].get("correct_answers", 0), avg_correct],
                [top_performers[0].get("wrong_answers", 0), avg_wrong],
                [top_performers[0].get("penalty", 0), avg_penalty]
            ]
            ...
            for i, metric in enumerate(bar_metrics):
                ...
                self.set_font('Arial', 'B', 8)
                self.set_text_color(*self.text_dark)
                self.cell(label_width, bar_height, metric, 0, 0, 'L')
                
                # Calculate max value for this metric to scale the bars
                max_value = max(bar_values[i][0], bar_values[i][1], 1)
                
                # Draw topper's bar
                topper_width = (bar_values[i][0] / max_value) * max_bar_width
                self.set_fill_color(*bar_colors[i])
                self.rect(15 + label_width, y_pos, topper_width, bar_height, style='F')
                
                # Add topper value
                self.set_xy(15 + label_width + topper_width + 2, y_pos + 2)
                self.set_font('Arial', 'B', 8)
                self.set_text_color(*bar_colors[i])
                self.cell(20, bar_height, str(bar_values[i][0]), 0, 0, 'L')
                
                # Move to draw average bar below
                y_pos += bar_height + 2
                ...
                self.cell(label_width, bar_height, "Avg:", 0, 0, 'L')
                
                # Draw average bar
                avg_width = (bar_values[i][1] / max_value) * max_bar_width
                self.set_fill_color(*bar_colors[i], 0.4)  # More transparent
                self.rect(15 + label_width, y_pos, avg_width, bar_height, style='F')
                
                # Add average value
                self.set_xy(15 + label_width + avg_width + 2, y_pos + 2)
                self.set_font('Arial', '', 8)
                self.set_text_color(100, 100, 100)
                self.cell(20, bar_height, str(round(bar_values[i][1], 1)), 0, 0, 'L')
                
                # Add gap measurement
                gap = bar_values[i][0] - bar_values[i][1]
                gap_text = f"Gap: {round(gap, 1)}" if i != 2 else f"Gap: {round(-gap, 1)}"
                gap_color = self.brand_accent if (i != 2 and gap > 0) or (i == 2 and gap < 0) else (200, 50, 50)
                
                self.set_xy(15 + label_width + max(topper_width, avg_width) + 25, y_pos - bar_height + 2)
                self.set_font('Arial', 'B', 8)
                self.set_text_color(*gap_color)
                self.cell(40, bar_height, gap_text, 0, 0, 'L')
                
                # Move to next metric
                self.ln(bar_height * 2 + 5)
            ...
            # Calculate and add insights with improved formatting
            insights = []
            ...
            if top_performers:
                ...
                avg_skipped_comparison = "fewer" if top_skipped < avg_skipped else "more"
                skip_diff = abs(top_skipped - avg_skipped)
                insights.append(f"• Top performers skipped {round(skip_diff, 1)} {avg_skipped_comparison} questions than the average")
            
            # Insight 5: Penalty impact
            if top_performers:
                top_penalty = sum(p.get("penalty", 0) for p in top_performers) / len(top_performers)
                penalty_diff_pct = ((top_penalty / max(1, avg_penalty)) - 1) * 100
                penalty_comparison = "lower" if penalty_diff_pct < 0 else "higher"
                insights.append(f"• Negative marking impact on top performers: {abs(round(penalty_diff_pct))}% {penalty_comparison} than average")
            
            # Insight 6: Performance consistency
            if len(top_performers) >= 2:
                top1_score = top_performers[0].get("adjusted_score", 0)
                top2_score = top_performers[1].get("adjusted_score", 0)
                gap = top1_score - top2_score
                gap_pct = (gap / max(1, top2_score)) * 100
                insights.append(f"• Gap between 1st and 2nd place: {round(gap, 1)} points ({round(gap_pct)}%)")
            
            # Print the insights with styled bullet points
            ...
            self.set_font('Arial', '', 10)
            self.set_text_color(*self.text_dark)
            self.cell(0, 10, "Top performers analysis (simplified due to error):", 0, 1, 'L')
            ...
            if leaderboard:
                ...
                for i, performer in enumerate(top_performers):
                    ...
                    correct = performer.get("correct_answers", 0)
                    wrong = performer.get("wrong_answers", 0)
                    
                    medal = medal_icons[i] if i < 3 else ""
                    
                    self.set_font('Arial', 'B', 10)
                    self.cell(0, 7, f"{medal} {name}: {score} points", 0, 1, 'L')
                    
                    self.set_font('Arial', '', 9)
                    self.cell(0, 5, f"   Correct: {correct}, Wrong: {wrong}", 0, 1, 'L')
    
    def add_detailed_analytics(self, leaderboard):
        """Add enhanced detailed quiz performance analytics"""
        # Check if we have leaderboard data
        if not leaderboard or len(leaderboard) == 0:
            logger.info("No leaderboard data available for detailed analytics")
            # Add message in the chart area
            self.ln(15)
            self.set_font('Arial', 'B', 16)
            self.set_text_color(*self.brand_primary)
            self.cell(0, 10, "DETAILED QUIZ PERFORMANCE ANALYTICS", 0, 1, 'L')
            
            # Add subtitle
            self.set_font('Arial', 'I', 9)
            self.set_text_color(*self.text_dark)
            self.cell(0, 5, "Comprehensive analysis of quiz performance metrics and patterns", 0, 1, 'L')
            ...
            self.set_font('Arial', 'I', 10)
            self.set_text_color(*self.text_dark)
            self.cell(0, 10, "No detailed analytics available - add participants to generate insights", 0, 1, 'L')
            return
        ...
        try:
            ...
            self.set_font('Arial', '', 10)
            self.set_text_color(*self.text_dark)
            self.multi_cell(0, 7, "This section provides a detailed breakdown of quiz performance metrics including score distribution, answer patterns, and key insights derived from participant data.", 0, 'L')
            self.ln(5)
            ...
            if scores:
                ...
                penalties = [p.get("penalty", 0) for p in leaderboard]
                ...
                total_questions_attempted = avg_correct + avg_wrong + avg_skipped
                correct_percentage = avg_correct / total_questions_attempted * 100 if total_questions_attempted > 0 else 0
                wrong_percentage = avg_wrong / total_questions_attempted * 100 if total_questions_attempted > 0 else 0
                skipped_percentage = avg_skipped / total_questions_attempted * 100 if total_questions_attempted > 0 else 0
                
                # Create a visual analytics grid with KPIs
                # First row - Score Analytics with improved design
                self.set_font('Arial', 'B', 14)
                self.set_text_color(*self.brand_primary)
                self.cell(0, 10, "Score Distribution Analysis", 0, 1, 'L')
                ...
                # Create a row of KPI boxes with enhanced design
                box_width = 42
                box_height = 30
                margin = 4
                
                # Score Analytics metrics with improved styling
                metrics = [
                    {"label": "HIGHEST SCORE", "value": f"{max_score}", "color": self.brand_accent, "icon": "🔝"},
                    {"label": "AVERAGE SCORE", "value": f"{avg_score:.1f}", "color": self.brand_secondary, "icon": "📊"},
                    {"label": "MEDIAN SCORE", "value": f"{median_score}", "color": (100, 100, 150), "icon": "📏"},
                    {"label": "LOWEST SCORE", "value": f"{min_score}", "color": (200, 50, 50), "icon": "📉"}
                ]
                
                # Draw the first row of metrics with enhanced design
                ...
                for i, metric in enumerate(metrics):
                    ...
                    # Draw box with gradient-like effect (darker at top)
                    r, g, b = metric["color"]
                    ...
                    # Darker top bar (gradient effect)
                    self.set_fill_color(*darker)
                    self.rect(x, self.get_y(), box_width, 6, style='F')
                    
                    # Add icon and label
                    self.set_xy(x + 2, self.get_y() + 8)
                    self.set_font('Arial', 'B', 9)
                    self.set_text_color(*self.text_light)
                    self.cell(box_width - 4, 6, f"{metric['icon']} {metric['label']}", 0, 2, 'L')
                    
                    # Add value with larger, bold font
                    self.set_xy(x + 2, self.get_y() + 2)
                    self.set_font('Arial', 'B', 16)
                    self.cell(box_width - 4, 12, metric["value"], 0, 0, 'C')
                ...
                self.set_font('Arial', 'B', 14)
                self.set_text_color(*self.brand_primary)
                self.cell(0, 10, "Answer Pattern Analysis", 0, 1, 'L')
                ...
                # Draw performance distribution as a horizontal stacked bar with enhanced design
                bar_width = 170
                bar_height = 25
                self.set_y(self.get_y() + 5)
                
                # Add title for the bar
                self.set_font('Arial', 'B', 12)
                self.set_text_color(*self.text_dark)
                self.cell(0, 10, "Average Response Distribution", 0, 1, 'L')
                ...
                # Draw the segments of the stacked bar with improved styling
                start_x = 15
                
                # Add border around the entire bar
                self.set_draw_color(80, 80, 80)
                self.set_line_width(0.5)
                self.rect(start_x, self.get_y(), bar_width, bar_height, 'D')
                
                # Correct answers segment (green) with pattern
                self.set_fill_color(*self.brand_accent)
                ...
                # Add percentage labels to segments with improved styling
                # Correct
                self.set_xy(start_x + (correct_width / 2) - 10, self.get_y() + 9)
                self.set_font('Arial', 'B', 10)
                self.set_text_color(*self.text_light)
                ...
                # Add legend below the bar with improved styling
                ...
                # Draw legend items with better spacing and icons
                legend_width = 15
                legend_height = 8
                legend_spacing = 60
                ...
                for i, item in enumerate(legend_items):
                    ...
                    self.set_draw_color(80, 80, 80)
                    self.rect(x, legend_y, legend_width, legend_height, style='D')
                    
                    # Add label with better font
                    self.set_xy(x + legend_width + 3, legend_y + 1)
                    self.set_font('Arial', 'B', 9)
                    self.set_text_color(*self.text_dark)
                    self.cell(50, 6, item["label"], 0, 0, 'L')
                ...
                self.set_font('Arial', 'B', 14)
                self.set_text_color(*self.brand_primary)
                self.cell(0, 10, "Negative Marking Impact", 0, 1, 'L')
                ...
                # Calculate negative marking impact metrics
                total_wrong = sum(wrong_answers)
                total_penalty = sum(penalties)
                avg_penalty_per_wrong = total_penalty / max(1, total_wrong) if total_wrong > 0 else 0
                
                # Calculate participant penalty statistics
                max_penalty = max(penalties) if penalties else 0
                min_penalty = min(penalties) if penalties else 0
                
                # Negative marking impact metrics
                penalty_metrics = [
                    {"label": "AVG PENALTY/PARTICIPANT", "value": f"{avg_penalty:.1f}", "color": (200, 100, 50)},
                    {"label": "MAX PENALTY IMPACT", "value": f"{max_penalty}", "color": (220, 50, 50)},
                    {"label": "TOTAL PENALTIES", "value": f"{total_penalty:.1f}", "color": (180, 80, 30)},
                    {"label": "AVG PENALTY PER WRONG", "value": f"{avg_penalty_per_wrong:.2f}", "color": (150, 70, 20)}
                ]
                
                # Draw penalty impact metrics
                self.set_y(self.get_y() + 5)
                
                # Create a 2x2 grid for penalty metrics
                box_width = 85
                box_height = 25
                margin = 5
                grid_start_x = 15
                ...
                # First row
                for i in range(2):
                    metric = penalty_metrics[i]
                    x = grid_start_x + (i * (box_width + margin))
                    self.set_fill_color(*metric["color"])
                    self.rect(x, self.get_y(), box_width, box_height, style='F')
                    
                    # Add label
                    self.set_xy(x + 4, self.get_y() + 5)
                    self.set_font('Arial', 'B', 9)
                    self.set_text_color(*self.text_light)
                    self.cell(box_width - 8, 6, metric["label"], 0, 2, 'L')
                    
                    # Add value
                    self.set_font('Arial', 'B', 14)
                    self.cell(box_width - 8, 10, metric["value"], 0, 0, 'L')
                
                # Second row
                self.set_y(self.get_y() + box_height + 5)
                for i in range(2):
                    metric = penalty_metrics[i + 2]
                    x = grid_start_x + (i * (box_width + margin))
                    self.set_fill_color(*metric["color"])
                    self.rect(x, self.get_y(), box_width, box_height, style='F')
                ...
                self.set_font('Arial', 'B', 14)
                self.set_text_color(*self.brand_primary)
                self.cell(0, 10, "Participant Performance Distribution", 0, 1, 'L')
                ...
                for score in scores:
                    if score < 0:
                        ...
                        score_ranges["Poor (0-19)"] += 1
                    elif score < 40:
                        score_ranges["Below Average (20-39)"] += 1
                    elif score < 60:
                ...
                # Draw distribution chart with enhanced styling
                bar_height = 15
                max_bar_width = 150
                label_width = 50
                
                # Find the maximum count for scaling
                max_count = max(score_ranges.values()) if score_ranges else 1
                
                # Draw title for chart
                self.set_font('Arial', 'B', 12)
                self.set_text_color(*self.text_dark)
                self.cell(0, 10, "Score Range Distribution", 0, 1, 'L')
                
                # Add explanation
                self.set_font('Arial', 'I', 8)
                self.set_text_color(100, 100, 100)
                self.cell(0, 5, "Number of participants in each score range", 0, 1, 'L')
                self.ln(5)
                
                # Define colors for each score range (gradient from red to green)
                range_colors = [
                    (50, 150, 80),    # Excellent - green
                    (100, 180, 70),   # Good - light green
                    (200, 180, 50),   # Average - yellow
                    (230, 120, 50),   # Below Average - orange
                    (220, 70, 50),    # Poor - red
                    (180, 50, 50)     # Negative - dark red
                ]
                ...
                for i, (range_name, count) in enumerate(score_ranges.items()):
                    ...
                    # Draw range label with percentage
                    self.set_xy(15, y_pos + 4)
                    self.set_font('Arial', 'B', 9)
                    self.set_text_color(*self.text_dark)
                    percentage = (count / total_participants) * 100 if total_participants > 0 else 0
                    self.cell(label_width, bar_height - 5, range_name, 0, 0, 'L')
                    
                    # Calculate and draw bar
                    bar_width = (count / max_count) * max_bar_width if max_count > 0 else 0
                    self.set_fill_color(*range_colors[i])
                    self.rect(15 + label_width, y_pos, bar_width, bar_height, style='F')
                    
                    # Add count and percentage at the end of the bar
                    self.set_xy(15 + label_width + bar_width + 5, y_pos + 4)
                    self.set_font('Arial', 'B', 10)
                    self.set_text_color(*range_colors[i])
                    self.cell(40, bar_height - 5, f"{count} ({percentage:.1f}%)", 0, 0, 'L')
                ...
                # Quiz Insights with enhanced styling
                self.set_font('Arial', 'B', 14)
                self.set_text_color(*self.brand_primary)
                self.cell(0, 10, "Key Quiz Insights", 0, 1, 'L')
                ...
                # Create insights with enhanced styling
                insights = []
                ...
                # Insight 4: Most common score range
                most_common_range = max(score_ranges.items(), key=lambda x: x[1])[0] if score_ranges else "N/A"
                insights.append(f"📈 Most common score range: {most_common_range}")
                
                # Insight 5: Negative marking impact percentage
                if avg_score > 0 and avg_penalty > 0:
                    penalty_impact_pct = (avg_penalty / (avg_score + avg_penalty)) * 100
                    insights.append(f"📉 Negative marking reduced scores by average of {penalty_impact_pct:.1f}%")
                
                # Insight 6: Skipping behavior
                max_skipped_value = max(skipped) if skipped else 0
                insights.append(f"⏩ Maximum questions skipped by a participant: {max_skipped_value}")
                
                # Print the insights with enhanced styling
                for insight in insights:
                    self.set_font('Arial', '', 11)
                    self.set_text_color(*self.text_dark)
                    self.multi_cell(0, 8, insight, 0, 'L')
                ...
                self.set_font('Arial', 'I', 10)
                self.set_text_color(100, 100, 100)
                self.multi_cell(0, 7, "This analysis provides a comprehensive overview of quiz performance patterns, helping identify strengths, weaknesses, and opportunities for improvement in future quizzes.", 0, 'L')
                
            self.ln(5)
                
        except Exception as e:
            logger.error(f"Error in detailed analytics: {e}")
            # Fallback to simple analytics with improved reliability
            self.ln(10)
            self.set_font('Arial', 'B', 14)
            self.set_text_color(*self.brand_primary)
            ...
            self.set_font('Arial', 'I', 10)
            self.set_text_color(*self.text_dark)
            self.cell(0, 7, "Simplified view due to rendering error", 0, 1, 'L')
            
            # Basic stats that are less likely to fail
            self.set_font('Arial', '', 10)
            if leaderboard:
                try:
                    # Count participants
                    self.cell(0, 7, f"Total Participants: {len(leaderboard)}", 0, 1, 'L')
                    
                    # Calculate basic score stats with error handling
                    scores = [p.get("adjusted_score", 0) for p in leaderboard]
                    if scores:
                        self.cell(0, 7, f"Highest Score: {max(scores)}", 0, 1, 'L')
                        avg_score = sum(scores)/len(scores)
                        self.cell(0, 7, f"Average Score: {avg_score:.1f}", 0, 1, 'L')
                        self.cell(0, 7, f"Lowest Score: {min(scores)}", 0, 1, 'L')
                    
                    # Negative marking info if available
                    penalties = [p.get("penalty", 0) for p in leaderboard]
                    if penalties:
                        avg_penalty = sum(penalties)/len(penalties)
                        self.cell(0, 7, f"Average Penalty: {avg_penalty:.1f}", 0, 1, 'L')
                    
                except Exception as inner_e:
                    # Ultra-simple fallback
                    logger.error(f"Error in simple analytics fallback: {inner_e}")
                    self.cell(0, 7, "Error calculating statistics", 0, 1, 'L')
    
    def add_score_distribution(self, leaderboard):
        """Add score distribution graph with visual bar chart"""
        if not leaderboard or len(leaderboard) == 0:
            logger.info("No leaderboard data available for score distribution")
            # Add message in the chart area
            self.ln(15)
            self.set_font('Arial', 'B', 14)
            self.set_text_color(*self.brand_primary)
            ...
            self.set_font('Arial', 'I', 10)
            self.set_text_color(*self.text_dark)
            self.cell(0, 10, "No score data available", 0, 1, 'L')
            return
...
def generate_pdf_results(quiz_id, title=None):
    ...
    try:    
        ...
        # Check if the leaderboard data is empty and log detailed info
        if not leaderboard or len(leaderboard) == 0:
            logger.error(f"Leaderboard data is empty for quiz {quiz_id}")
            # Try to get quiz results directly as a fallback
            results = get_quiz_results(quiz_id)
            
            # Verify results is a dictionary
            if not isinstance(results, dict):
                logger.error(f"Quiz results is not a dictionary, it's {type(results)}: {results}")
                # Create an empty leaderboard as fallback to avoid errors
                leaderboard = []
            # Verify participants is a list
            elif not isinstance(results.get("participants", []), list):
                logger.error(f"Participants is not a list, it's {type(results.get('participants'))}")
                # Create an empty leaderboard as fallback to avoid errors
                leaderboard = []
            # Process participants from results
            elif results and isinstance(results.get("participants", []), list) and len(results.get("participants", [])) > 0:
                logger.info(f"Found {len(results.get('participants', []))} results for quiz {quiz_id}, converting to leaderboard format")
                # Convert quiz results to leaderboard format
                leaderboard = []
                rank = 1
                for result in results.get("participants", []):
                    ...
                    rank += 1
                logger.info(f"Created leaderboard with {len(leaderboard)} entries from quiz results")
        else:
            logger.info(f"Found leaderboard data with {len(leaderboard)} entries for quiz {quiz_id}")
    except Exception as e:
    ...
    try:
        ...
        # alias_nb_pages() is already called in the constructor
        pdf.add_page()
        ...
        # Use a more reliable approach for saving PDFs
        pdf_saved = False
        logger.info("Starting PDF saving process...")
        
        try:
            # Strategy 1: Standard output with better error handling
            logger.info("Attempting PDF output with standard method...")
            # First check if the directory is writable by writing a test file
            test_file = os.path.join(PDF_RESULTS_DIR, "pdf_test.txt")
            with open(test_file, 'w') as f:
                f.write("PDF directory is writable")
            os.remove(test_file)
            
            # Now try to output the PDF
            pdf.output(filename)
            ...
            # Strategy 2: Try binary mode with clearer file handling
            try:
                logger.info("Trying binary output method...")
                # Generate content to memory first
                pdf_content = pdf.output(dest='S')
                
                # If we got here, the PDF generation worked, now try to save it
                logger.info(f"PDF generated in memory successfully, size: {len(pdf_content)} bytes")
                
                # Write the file with ASCII encoding to avoid Unicode issues
                with open(filename, 'wb') as f:
                    # Use ASCII encoding with replacement of non-ASCII characters
                    # This ensures no Unicode issues even with emojis or special chars
                    safe_content = ''.join(c if ord(c) < 128 else '_' for c in pdf_content)
                    f.write(safe_content.encode('ascii', errors='replace'))
                logger.info("PDF output completed successfully with safe ASCII encoding")
                pdf_saved = True
            except Exception as e2:
                logger.error(f"Error in binary PDF output: {e2}")
                
                # Strategy 3: Try a different filename and location
                try:
                    # Try with a simpler filename in the current directory
                    simple_name = f"quiz_{quiz_id}_results.pdf"
                    logger.info(f"Trying with simpler filename: {simple_name}")
                    
                    # Use memory output with ASCII encoding
                    pdf_content = pdf.output(dest='S')
                    logger.info(f"Generated PDF content in memory, size: {len(pdf_content)}")
                    
                    # Use ASCII encoding to avoid any Unicode issues
                    safe_content = ''.join(c if ord(c) < 128 else '_' for c in pdf_content)
                    ...
                    # If successful, use this filename
                    filename = simple_name
                    logger.info(f"PDF saved successfully with simpler filename: {filename}")
                    pdf_saved = True
                except Exception as e3:
                    logger.error(f"Error with simpler filename: {e3}")
                    
                    # Final strategy: Create a basic PDF as fallback
                    try:
                        logger.info("Creating fallback basic PDF...")
                        # Use a clean, simple PDF with minimal content
                        simple_pdf = FPDF()
                        simple_pdf.add_page()
                        ...
                        simple_pdf.cell(0, 10, f"Generated: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}", 0, 1, 'C')
                        
                        # Save with a different name in current directory
                        fallback_filename = f"quiz_{quiz_id}_basic.pdf"
                        
                        # Try to save the basic PDF
                        logger.info(f"Trying to save basic PDF: {fallback_filename}")
                        
                        # Use memory output with ASCII encoding for the basic PDF too
                        pdf_content = simple_pdf.output(dest='S')
                        logger.info(f"Generated basic PDF content in memory: {len(pdf_content)} bytes")
                        
                        # Make sure content is ASCII-only
                        safe_content = ''.join(c if ord(c) < 128 else '_' for c in pdf_content)
                        ...
                        logger.info(f"Fallback basic PDF saved successfully: {filename}")
                        pdf_saved = True
                    except Exception as e4:
                        logger.error(f"All PDF saving methods failed: {e4}")
        
        # Return the filename if any method succeeded, None otherwise
        if pdf_saved:
            logger.info(f"PDF file saved successfully: {filename}")
            return filename
        else:
            logger.error("All PDF saving methods failed")
            return None
    except Exception as e:
...
async def handle_quiz_end_with_pdf(update, context, quiz_id, user_id, user_name, 
    ...
    try:
        ...
        # Simplified file verification - less strict to avoid false failures
        file_valid = False
        if pdf_file:
            try:
                ...
                # Simple verification: just check if file exists and has some content
                if os.path.exists(pdf_file):
                    file_size = os.path.getsize(pdf_file)
                    logger.info(f"Found PDF file: {pdf_file} with size {file_size} bytes")
                    
                    # Accept any file with some content (even very small)
                    if file_size > 0:
                        file_valid = True
                        logger.info(f"PDF file considered valid based on existence and non-zero size")
                    else:
                        logger.error(f"PDF file exists but is empty (0 bytes): {pdf_file}")
                else:
                    logger.error(f"PDF file does not exist: {pdf_file}")
            except Exception as e:
                logger.error(f"Error verifying PDF file: {e}")
                # FAILSAFE: Try a very basic check if the more detailed one fails
                try:
                    import os
        ...
        if file_valid:
            ...
            # Try once more with a direct attempt to save PDF in the current directory
            try:
                logger.info("Making one final attempt to generate PDF in current directory...")
                # Create a professional, branded PDF with better styling
                final_pdf = FPDF()
                final_pdf.add_page()
                
                # Define brand colors
                brand_primary = (25, 52, 152)     # Deep blue
                brand_secondary = (242, 100, 25)  # Vibrant orange
                brand_accent = (50, 168, 82)      # Green
                text_dark = (45, 45, 45)          # Almost black
                text_light = (250, 250, 250)      # Almost white
                bg_light = (245, 245, 245)        # Light gray
                
                # Add a colored header
                final_pdf.set_fill_color(*brand_primary)
                final_pdf.rect(0, 0, 210, 25, style='F')
                
                # Add title in the header
                final_pdf.set_font('Arial', 'B', 18)
                final_pdf.set_text_color(*text_light)
                final_pdf.set_y(10)
                final_pdf.cell(0, 10, f'Quiz {quiz_id} Results', 0, 1, 'C')
                
                # Add decorative line
                final_pdf.set_y(30)
                final_pdf.set_draw_color(*brand_secondary)
                final_pdf.set_line_width(0.5)
                final_pdf.line(40, 30, 170, 30)
                
                # Add participant info with better styling
                final_pdf.set_y(40)
                final_pdf.set_text_color(*text_dark)
                final_pdf.set_font('Arial', 'B', 14)
                final_pdf.cell(0, 10, f'Results for: {user_name}', 0, 1, 'C')
                
                # Add a results summary box
                final_pdf.set_y(60)
                final_pdf.set_fill_color(240, 240, 250)
                final_pdf.rect(30, 60, 150, 80, style='F')
                
                # Add score in a highlighted box
                final_pdf.set_y(65)
                final_pdf.set_x(35)
                final_pdf.set_fill_color(*brand_primary)
                final_pdf.set_text_color(*text_light)
                final_pdf.set_font('Arial', 'B', 16)
                final_pdf.cell(140, 15, f"Score: {adjusted_score}", 0, 1, 'C', True)
                
                # Add stats with better styling
                final_pdf.set_y(85)
                final_pdf.set_text_color(*text_dark)
                final_pdf.set_font('Arial', 'B', 12)
                
                # Create two columns for stats
                final_pdf.set_x(40)
                final_pdf.cell(60, 10, "Results:", 0, 0, 'L')
                final_pdf.set_x(100)
                final_pdf.cell(60, 10, "Values:", 0, 1, 'L')
                
                # Draw a thin line under the headers
                final_pdf.set_x(40)
                final_pdf.set_draw_color(*brand_secondary)
                final_pdf.line(40, 95, 160, 95)
                
                # Add stats in two columns with alternating background
                y_pos = 100
                stats = [
                    ["Correct Answers:", f"{correct_answers}"],
                    ["Wrong Answers:", f"{wrong_answers}"],
                    ["Skipped Questions:", f"{skipped}"],
                    ["Penalty:", f"{penalty}"]
                ]
                ...
                for i, stat in enumerate(stats):
                    ...
                    if i % 2 == 0:
                        final_pdf.set_fill_color(235, 235, 235)
                    else:
                        final_pdf.set_fill_color(245, 245, 245)
                        
                    final_pdf.set_y(y_pos)
                    final_pdf.set_x(40)
                    final_pdf.cell(60, 8, stat[0], 0, 0, 'L', True)
                    final_pdf.set_x(100)
                    final_pdf.cell(60, 8, stat[1], 0, 1, 'L', True)
                    y_pos += 8
                
                # Add a performance visualization if there are any wrong answers
                if correct_answers > 0 or wrong_answers > 0:
                    final_pdf.set_y(140)
                    final_pdf.set_x(30)
                    final_pdf.set_font('Arial', 'B', 14)
                    final_pdf.set_text_color(*brand_primary)
                    final_pdf.cell(150, 10, "Performance Summary", 0, 1, 'C')
                    
                    # Add a simple bar chart for correct vs wrong
                    total = correct_answers + wrong_answers + skipped
                    if total > 0:
                        correct_width = (correct_answers / total) * 140
                        wrong_width = (wrong_answers / total) * 140
                        skip_width = (skipped / total) * 140
                        
                        # Draw the bars
                        y_bar = 155
                        final_pdf.set_y(y_bar)
                        final_pdf.set_x(35)
                        
                        # Correct answers bar
                        if correct_answers > 0:
                            final_pdf.set_fill_color(*brand_accent)
                            final_pdf.rect(35, y_bar, correct_width, 10, style='F')
                        
                        # Wrong answers bar
                        if wrong_answers > 0:
                            final_pdf.set_fill_color(*brand_secondary)
                            final_pdf.rect(35 + correct_width, y_bar, wrong_width, 10, style='F')
                        
                        # Skipped questions bar
                        if skipped > 0:
                            final_pdf.set_fill_color(150, 150, 150)
                            final_pdf.rect(35 + correct_width + wrong_width, y_bar, skip_width, 10, style='F')
                        
                        # Add legend
                        final_pdf.set_y(170)
                        final_pdf.set_text_color(*text_dark)
                        final_pdf.set_font('Arial', '', 10)
                        
                        # Correct legend
                        final_pdf.set_x(35)
                        final_pdf.set_fill_color(*brand_accent)
                        final_pdf.rect(35, 170, 5, 5, style='F')
                        final_pdf.set_x(45)
                        final_pdf.cell(35, 5, f"Correct: {correct_answers}", 0, 0, 'L')
                        
                        # Wrong legend
                        final_pdf.set_x(85)
                        final_pdf.set_fill_color(*brand_secondary)
                        final_pdf.rect(85, 170, 5, 5, style='F')
                        final_pdf.set_x(95)
                        final_pdf.cell(35, 5, f"Wrong: {wrong_answers}", 0, 0, 'L')
                        
                        # Skipped legend
                        final_pdf.set_x(135)
                        final_pdf.set_fill_color(150, 150, 150)
                        final_pdf.rect(135, 170, 5, 5, style='F')
                        final_pdf.set_x(145)
                        final_pdf.cell(35, 5, f"Skipped: {skipped}", 0, 0, 'L')
                
                # Add branded footer
                final_pdf.set_y(240)
                final_pdf.set_fill_color(*brand_primary)
                final_pdf.rect(0, 240, 210, 20, style='F')
                
                # Add timestamp in footer
                final_pdf.set_y(245)
                final_pdf.set_font('Arial', 'I', 10)
                final_pdf.set_text_color(*text_light)
                final_pdf.cell(0, 8, f"Generated: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}", 0, 1, 'C')
                
                # Add branding in footer
                final_pdf.set_y(250)
                final_pdf.set_font('Arial', 'B', 8)
                final_pdf.cell(0, 8, "PREMIUM TELEGRAM QUIZ BOT", 0, 1, 'C')
                
                # Save PDF with a very simple name in the current directory
                emergency_filename = f"quiz_{quiz_id}_results_emergency.pdf"
                final_pdf.output(emergency_filename)
                
                # If we get here, the PDF was successfully created
                logger.info(f"Emergency PDF created successfully: {emergency_filename}")
                
                # Try to send this PDF
                with open(emergency_filename, 'rb') as emergency_file:
                    await context.bot.send_document(
                        chat_id=update.effective_chat.id,
                        document=emergency_file,
                        filename=f"Quiz_{quiz_id}_Results.pdf",
                        caption=f"Quiz {quiz_id} Results"
                    )
                return True
            except Exception as e:
                # Now we've really tried everything, notify the user
                logger.error(f"All PDF generation attempts failed: {e}")
                await update.message.reply_text("❌ Sorry, PDF generation failed after multiple attempts. Please try again or contact support.")
                return False
    except Exception as e:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Standard welcome message if no quiz ID was provided
    welcome_text = (
...
async def end_quiz(context, chat_id):
    ...
    if final_scores:
        ...
        for i, data in enumerate(final_scores):  # Show all participants
            ...
            # Use text alternatives instead of emojis to prevent encoding issues
            medal_text = ["GOLD", "SILVER", "BRONZE"][i] if i < 3 else f"{i+1}."
            
            # Format the line with text alternatives instead of emoji icons
            results_message += (
                f"{medal_text} {name} | CORRECT: {correct} | WRONG: {wrong} | SCORE: {adjusted:.2f} |\n"
                f"TIME: {data.get('participation', 0)}s | PERF: {percentage:.2f}% | ACC: {accuracy_percentage:.2f}%\n"
            )

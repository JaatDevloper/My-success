...
def ensure_pdf_directory():
    ...
    potential_dirs = [
        os.path.join(os.getcwd(), "pdf_results"),  # Standard location
        os.path.join(os.getcwd(), "tmp", "pdf_results"),  # Render-friendly nested tmp folder
        os.path.join("/tmp", "pdf_results"),  # System tmp folder (good for Render)
...
def get_quiz_pdf_path(quiz_id, quiz_title):
    ...
    try:
        ...
        try:
            ...
            # If not writable, try /tmp directory (usually works on Render)
            logger.warning(f"Path not writable: {path}, error: {write_error}")
            if os.path.exists("/tmp") and os.access("/tmp", os.W_OK):
                path = os.path.join("/tmp", filename)
                logger.info(f"Using /tmp directory instead: {path}")
            else:
        ...
        # Try to use /tmp directory for Render compatibility
        if os.path.exists("/tmp") and os.access("/tmp", os.W_OK):
            fallback_path = os.path.join("/tmp", f"quiz_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S%f')}.pdf")
        else:
            fallback_path = os.path.join(os.getcwd(), f"quiz_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S%f')}.pdf")
        logger.warning(f"Using absolute fallback PDF path: {fallback_path}")

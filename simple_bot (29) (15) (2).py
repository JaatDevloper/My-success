...
async def handle_quiz_end_with_pdf(update, context, quiz_id, user_id, user_name, 
    ...
    """Handle quiz end with PDF generation only (HTML reports available via /htmlreport command)"""
    try:
        ...
        if file_valid:
            try:
                ...
                # Inform user about manual HTML report option
                await update.message.reply_text(
                    "ℹ️ *Quiz completed successfully!*\n\n"
                    "PDF results have been generated. If you would like an interactive HTML report,\n"
                    f"use the command: `/htmlreport {quiz_id}`",
                    parse_mode="Markdown"
                )
            except Exception as e:
                logger.error(f"Error sending PDF document: {e}")
                await update.message.reply_text(
                    "❌ Error sending PDF results. Please try again or contact the administrator.",
                    parse_mode="Markdown"
                )
        else:
            # If PDF failed, send appropriate error message
            await update.message.reply_text(
                "❌ Could not generate PDF results. Please try again or contact the administrator.",
                parse_mode="Markdown"
            )
    except Exception as e:
        logger.error(f"Error in quiz end handler: {e}")
        import traceback
        ...
        try:
            await update.message.reply_text(
                "❌ An error occurred while generating quiz results. Please try again later.",
                parse_mode="Markdown"
            )
        except Exception:
            # If even the error message fails, just log it and continue
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await start(update, context)
                                        ...
                                        <div class="stat-box">
                                            ...
                                            <div class="stat-value">{len(sanitized_leaderboard)}</div>
                                        </div>
                                        <div class="stat-box">
                    ...
                    # Use the direct generator function
                    html_generator = type('HtmlGenerator', (), {})()
                    html_generator.generate_html_report = generate_html_report_direct
...
async def html_report_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        quiz_results = get_quiz_results(quiz_id)
        if not quiz_results or not isinstance(quiz_results, dict) or "participants" not in quiz_results:
            await update.message.reply_text(
        ...
        logger.info(f"Found {len(quiz_results.get('participants', []))} participants for quiz ID {quiz_id}")
        ...
        class HtmlGenerator:
            def generate_html_report(self, quiz_id, title=None, questions_data=None, leaderboard=None, quiz_metadata=None):
                return generate_html_report_direct(quiz_id, title, questions_data, leaderboard, quiz_metadata)
        ...
        # Ensure we have a dictionary with 'participants' key
        if not isinstance(quiz_results, dict) or "participants" not in quiz_results:
            quiz_results = {"participants": []}
        
        # Print some diagnostics
        logger.info(f"Found {len(quiz_results.get('participants', []))} participants for quiz {quiz_id}")
        ...
        if total_questions == 0 and quiz_results:
            ...
            # Make sure we're iterating over the participants list, not the dictionary itself
            participants = quiz_results.get('participants', [])
        ...
        # Process participant data for time displays and ensure required fields
        # Make sure we're working with the participants list from the dictionary
        participants_list = quiz_results.get("participants", [])
        ...
        sanitized_results = []
        logger.info(f"Pre-cleaning quiz results, participants count: {len(participants_list)}")
        for participant in participants_list:
            if isinstance(participant, dict):
                ...
                if file_size > 100:  # Basic validation
                    ...
                    with open(html_file, 'rb') as file:
                        await context.bot.send_document(
                            ...
                            document=file,
                            filename=f"Quiz_{quiz_id}_Interactive_Results.html",
                            caption=f"📈 *Interactive Quiz {quiz_id} Analysis*\n\nOpen this HTML file in any web browser for a detailed interactive dashboard with charts and statistics.\n\nTotal Participants: {len(sanitized_results)}\nNegative Marking: {get_quiz_penalty(quiz_id)} points/wrong",
                            parse_mode="MARKDOWN"
                        )

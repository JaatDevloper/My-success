...
class InsaneResultPDF(FPDF):
    ...
    def add_questions_with_answers(self, questions_data):
        """Add questions with correct answers marked with checkmarks
        
        Args:
            questions_data (dict): Quiz data containing questions and their correct answers
        """
        if not questions_data or 'questions' not in questions_data:
            logger.warning("No questions data provided for PDF")
            return
            
        try:
            # Add a page break before questions section
            self.add_page()
            
            # Add section title
            self.set_font('Arial', 'B', 16)
            self.set_text_color(0, 51, 102)  # Dark blue
            self.cell(0, 15, "Quiz Questions with Correct Answers", 0, 1, 'C')
            self.ln(5)
            ...
            for i, question in enumerate(questions, 1):
                ...
                # Multi-cell for question text to handle long questions
                self.multi_cell(0, 8, f"Q{i}. {q_text}")
                self.ln(3)
                ...
                for j, option in enumerate(options):
                    ...
                    self.set_x(15)  # Indent options
                    
                    if j == correct_option:
                        # Mark correct answer with checkmark and green color
                        self.set_text_color(0, 128, 0)  # Green
                        self.multi_cell(0, 6, f"✓ {option}")
                    else:
                ...
                # Add a line separator between questions
                self.set_draw_color(200, 200, 200)  # Light gray
                self.line(10, self.get_y(), 200, self.get_y())
                self.ln(8)
        except Exception as e:
            logger.error(f"Error adding questions to PDF: {e}")

def generate_pdf_results(quiz_id, title=None, include_questions=False):
    """Generate PDF results for a quiz
    
    Args:
        quiz_id: The ID of the quiz
        title: Optional title for the PDF
        include_questions: Whether to include questions with correct answers (disabled by default)
    ...
    logger.info(f"Starting PDF generation for quiz ID: {quiz_id} with questions: {include_questions}")
    ...
    try:    
        ...
        if include_questions:
            ...
            if str(quiz_id) in questions:
                questions_data = questions[str(quiz_id)]
                logger.info(f"Loaded {len(questions_data.get('questions', []))} questions for quiz {quiz_id}")
            else:
                logger.warning(f"No questions found for quiz {quiz_id}")
    except Exception as e:
        logger.error(f"Error getting quiz data: {e}")
        return None
    ...
    try:
        ...
        # Add questions with correct answers if requested
        if include_questions and questions_data:
            try:
                logger.info("Adding questions with correct answers...")
                pdf.add_questions_with_answers(questions_data)
            except Exception as e:
                logger.error(f"Error adding questions to PDF: {e}")
                # Continue anyway
        ...
        try:
            ...
            try:
                ...
                # Add questions with correct answers if available (for the simplified PDF version too)
                if include_questions and questions_data and 'questions' in questions_data:
                    try:
                        ...
                        # Add section title
                        simple_pdf.set_font('Arial', 'B', 14)
                        simple_pdf.cell(0, 10, "Quiz Questions with Correct Answers", 0, 1, 'C')
                        simple_pdf.ln(5)
                        ...
                        for i, question in enumerate(questions, 1):
                            # Question text
                            simple_pdf.set_font('Arial', 'B', 12)
                            simple_pdf.set_text_color(0, 0, 0)  # Black
                            q_text = question.get('question', f"Question {i}")
                            simple_pdf.multi_cell(0, 8, f"Q{i}. {q_text}")
                            simple_pdf.ln(2)
                            ...
                            for j, option in enumerate(options):
                                # Set position for option text
                                simple_pdf.set_x(15)  # Indent options
                                
                                if j == correct_option:
                                    # Mark correct answer with checkmark and green color
                                    simple_pdf.set_text_color(0, 128, 0)  # Green
                                    simple_pdf.multi_cell(0, 6, f"✓ {option}")
                                else:
                                    # Other options in black
                                    simple_pdf.set_text_color(0, 0, 0)  # Black
                                    simple_pdf.multi_cell(0, 6, f"   {option}")
                            ...
                            simple_pdf.set_draw_color(200, 200, 200)  # Light gray
                            simple_pdf.line(10, simple_pdf.get_y(), 200, simple_pdf.get_y())
                            simple_pdf.ln(4)
                    except Exception as e:
                        logger.error(f"Error adding questions to simplified PDF: {e}")
...
def generate_questions_pdf(quiz_id, title=None):
    """
    Generate a PDF containing only quiz questions with correct answers marked
    ...
    logger.info(f"Starting question PDF generation for quiz ID: {quiz_id}")
    ...
    if not FPDF_AVAILABLE:
        logger.warning("FPDF library not available, cannot generate PDF")
        return None
    ...
    # Create PDF specifically for questions
    try:
        # Create the FPDF object
        pdf_title = title if title else f"Quiz {quiz_id} Questions with Answers"
        pdf = FPDF()
        pdf.alias_nb_pages()
        pdf.add_page()
        ...
        pdf.cell(0, 15, pdf_title, 0, 1, 'C')
        pdf.ln(5)
        
        # Add subtitle
        pdf.set_font('Arial', 'I', 12)
        pdf.set_text_color(0, 0, 0)  # Black
        pdf.cell(0, 10, f"Quiz ID: {quiz_id}", 0, 1, 'C')
        ...
        if 'questions' in questions_data:
            ...
            pdf.set_text_color(0, 51, 102)  # Dark blue
            pdf.cell(0, 10, "Questions with Correct Answers", 0, 1, 'L')
            pdf.ln(5)
            ...
            for i, question in enumerate(questions, 1):
                ...
                # Add a line separator between questions except after the last one
                if i < len(questions):
        ...
        # Save the PDF
        timestamp = datetime.datetime.now().strftime('%Y%m%d%H%M%S')
        filename = os.path.join(PDF_RESULTS_DIR, f"quiz_{quiz_id}_questions_{timestamp}.pdf")
        logger.info(f"Saving questions PDF to: {filename}")
        
        pdf.output(filename, 'F')
        logger.info("Questions PDF created successfully")
        
        return filename
        
    except Exception as e:
        logger.error(f"Error generating questions PDF: {e}")
        return None
...
def process_quiz_end(quiz_id, user_id, user_name, total_questions, correct_answers, 
    ...
    # Generate regular PDF results (without questions)
    pdf_file = generate_pdf_results(quiz_id, include_questions=False)
...
async def end_quiz(context, chat_id):
    ...
    # Generate and send a separate PDF with questions and answers
    try:
        await context.bot.send_message(
            chat_id=chat_id,
            text="📋 Generating PDF with all quiz questions and correct answers marked..."
        )
        
        # Generate the questions PDF
        questions_pdf = generate_questions_pdf(quiz_id, f"Quiz {quiz_id} Questions with Answers")
        
        if questions_pdf and os.path.exists(questions_pdf):
            # Send the questions PDF
            with open(questions_pdf, 'rb') as file:
                await context.bot.send_document(
                    chat_id=chat_id,
                    document=file,
                    filename=f"Quiz_{quiz_id}_Questions_with_Answers.pdf",
                    caption=f"📝 Questions from Quiz {quiz_id} with Correct Answers Marked ✅"
                )
            logger.info(f"Questions PDF sent successfully: {questions_pdf}")
        else:
            logger.error(f"Failed to generate questions PDF for quiz {quiz_id}")
            await context.bot.send_message(
                chat_id=chat_id,
                text="❌ Sorry, couldn't generate the questions PDF."
            )
    except Exception as e:
        logger.error(f"Error generating/sending questions PDF: {e}")
        await context.bot.send_message(
            chat_id=chat_id,
            text=f"❌ Error generating questions PDF: {str(e)}"
        )
    ...
    if quiz_id and FPDF_AVAILABLE and final_scores:
        try:
            ...
            # Use the enhanced PDF generation function with questions
            # Pass quiz_data to include questions in the PDF
            quiz_data = load_questions().get(str(quiz_id), {})
            if quiz_data:
                logger.info(f"Found questions data for quiz ID {quiz_id}, including in PDF")
                
                # Use the new function that generates PDF with questions and correct answers
                pdf_file = generate_pdf_results(
                    quiz_id, 
                    title=f"Quiz {quiz_id} Results", 
                    include_questions=True
                )
                
                # If PDF was generated successfully, send it directly
                    ...
                    try:
                        with open(pdf_file, 'rb') as file:
                            await context.bot.send_document(
                                chat_id=chat_id,
                                document=file,
                                filename=f"Quiz_{quiz_id}_Results_with_Answers.pdf",
                                caption=f"📝 Quiz {quiz_id} Results with Correct Answers Marked ✅"
                            )
                        logger.info(f"PDF with questions sent successfully")
                    except Exception as e:
                        logger.error(f"Error sending PDF with questions: {e}")
                else:
                    # Fallback to original method if PDF generation failed
                ...
                # Fallback to original method if no questions data found
                logger.warning(f"No questions data found for quiz ID {quiz_id}")
                await handle_quiz_end_with_pdf(
                    fake_update, context, quiz_id, user_id, user_name,
                    total_questions, correct_answers, wrong_answers,
                    skipped, penalty, score, adjusted_score

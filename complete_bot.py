...
def extract_text_from_pdf(pdf_file):
    ...
    try:
        ...
        # If text is empty and OCR is available, try OCR
        if not text.strip() and OCR_AVAILABLE:
            ...
            for i, image in enumerate(images):
                page_text = pytesseract.image_to_string(image, lang='eng+hin')  # Support English and Hindi
                text += page_text + "\n\n"
...
def parse_pdf_questions(pdf_text):
    ...
    if language == "hi":
        ...
        answer_pattern = r'(?:उत्तर|उत्तर:|उत्तर\s*:|समाधान|समाधान:|समाधान\s*:)\s*([अआइईउऊएओ])'
    else:
        # English patterns
        question_pattern = r'(?:\d+[\.\)]\s*)([^\n]+)'
        option_patterns = [
        ...
        answer_pattern = r'(?:answer|answer:|ans|ans:|correct answer|correct answer:)\s*([a-dA-D])'
    
    # Split text into chunks (each chunk possibly containing a question and its options)
    chunks = re.split(r'\d+[\.\)]\s*', pdf_text)
    
    current_question = None
    options = []
    
    for i, chunk in enumerate(chunks):
        if i == 0:  # Skip the first chunk (usually empty or header)
            continue
        ...
        if question_match:
            ...
            for pattern in option_patterns:
                option_match = re.search(pattern, chunk)
                if option_match:
                    options.append(option_match.group(1).strip())
            
            # Extract answer
            answer = None
            answer_match = re.search(answer_pattern, chunk, re.IGNORECASE)
            if answer_match:
                answer_text = answer_match.group(1).upper()
                # Convert to index (0-based)
                if language == "hi":
                    answer_map = {"अ": 0, "आ": 1, "इ": 2, "ई": 3}
                    answer = answer_map.get(answer_text, 0)
                else:
                    answer_map = {"A": 0, "B": 1, "C": 2, "D": 3}
                    answer = answer_map.get(answer_text, 0)
            
            # Only add if we have a question, at least 2 options, and an answer
            if current_question and len(options) >= 2 and answer is not None:
                questions.append({
...
def extract_and_save_pdf_questions(pdf_data):
    ...
    try:
        ...
        return {
            ...
            "message": f"Successfully imported {questions_added} questions from the PDF."
        }
...
async def import_pdf_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        ...
        "I'll extract the questions and add them to the quiz database.\n\n"
...
async def poll_answer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if user_id not in context.chat_data.get("participants", {}):
    ...
    try:
        ...
        if not participants:
            await context.bot.send_message(
                chat_id,
                "📊 Quiz ended! No one participated in this quiz."
            )
            return
...
def main() -> None:
    ...
    pdf_import_handler = ConversationHandler(
        ...
        states={
            WAITING_FOR_PDF: [
                MessageHandler(filters.Document.PHOTO, pdf_received),
    ...
    application.add_handler(MessageHandler(
        filters.FORWARDED & filters.StatusUpdate.FORWARD_DATE & filters.Poll.ALL, 

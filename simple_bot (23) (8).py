...
async def quizpdf_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Try text file approach (most reliable)
        try:
            # Create text file with quiz content (handles all character types)
            ...
            # Send the text file first (always works)
            ...
            # Now try to generate a PDF as well
            try:
                # Create simple PDF using FPDF with ASCII-only content
                from fpdf import FPDF
                ...
                # Add questions (ASCII only mode for compatibility)
                for i, question_data in enumerate(quiz_questions, 1):
                    # Get question (safe ASCII-only version)
                    question_text = f"Question {i}"
                    try:
                        original = str(question_data.get('question', f'Question {i}'))
                        # Test if can be encoded as latin-1
                        original.encode('latin-1', 'strict')
                        question_text = original
                    except UnicodeEncodeError:
                        # Keep the fallback if not latin-1 compatible
                        pass
                    ...
                    pdf.set_font("Arial", "", 11)
                    for j, option in enumerate(options):
                        # Get option text (safe ASCII-only version)
                        ...
                        try:
                            ...
                            # Keep fallback if not latin-1 compatible
                            pass
                ...
                # Send the PDF file too if it was created successfully
                ...
                await context.bot.edit_message_text(
                    chat_id=chat_id,
                    message_id=processing_message.message_id,
                    text=f"✅ Quiz #{quiz_id} files have been generated and sent!"
                )
            except Exception as pdf_err:
                # If PDF fails, at least we sent the text file
                await context.bot.edit_message_text(
                    chat_id=chat_id,
                    message_id=processing_message.message_id,
                    text=f"✅ Text file sent! PDF generation failed: {str(pdf_err)[:50]}..."
                )
        except Exception as text_err:
            # If even text file fails, show error
            await context.bot.edit_message_text(
                chat_id=chat_id,
                message_id=processing_message.message_id,

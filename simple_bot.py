...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "7630314344:AAEoA-w7mdylmgErRcRR4-OC1yBkR1f81X0")
...
class InsaneResultPDF(FPDF):
    ...
    def add_watermark(self):
        ...
        self.rotate(45)  # Rotate text
        
        # Calculate position for the watermark to be centered
        text_width = self.get_string_width("INSANE")
        page_width = self.w
        page_height = self.h
        
        # Position the watermark in the center of the page
        self.set_xy(40, 50)
        self.cell(text_width, 30, "INSANE", 0, 0, 'C')
    ...
    def create_leaderboard_table(self, leaderboard):
        ...
        for entry in leaderboard:
            ...
            self.cell(col_widths[1], 10, entry.get("user_name", "")[:25], 1, 0, 'L', True)
...
def generate_pdf_results(quiz_id, title=None):
    ...
    try:
        ...
        filename = f"{PDF_RESULTS_DIR}/quiz_{quiz_id}_results_{timestamp}.pdf"
        pdf.output(filename)
        
        # Verify the PDF was created successfully
        if os.path.exists(filename) and os.path.getsize(filename) > 0:
            logger.info(f"Successfully generated PDF: {filename} (Size: {os.path.getsize(filename)} bytes)")
            return filename
        else:
...
async def handle_quiz_end_with_pdf(update, context, quiz_id, user_id, user_name, 
    ...
    # The actual PDF generation happens directly in the end_quiz function
    # to avoid the ExtBot.send_message error with fake objects
    
    try:
        # Get the chat ID from the update object safely
        chat_id = None
        if hasattr(update, 'effective_chat') and update.effective_chat:
            chat_id = update.effective_chat.id
        
        # If we have a valid chat ID, send a message
        if chat_id:
            try:
                await context.bot.send_message(
                    chat_id=chat_id,
                    text="📊 *Generating Quiz Results PDF...*",
                    parse_mode="Markdown"
                )
            except Exception as msg_error:
                logger.error(f"Error sending PDF generation message: {msg_error}")
        
        # We don't need to do anything else here as the actual PDF
        # generation happens in the end_quiz function directly
        return True
    except Exception as e:
        logger.error(f"Unexpected error in PDF handler wrapper: {e}")
        return False
...
async def end_quiz(context, chat_id):
    ...
    for user_id, user_data in participants.items():
        ...
        adjusted_score = float(max(0, correct_count - penalty_points))
    ...
    if quiz_id and FPDF_AVAILABLE and final_scores:
        try:
            ...
            # Generate PDF with simplified, robust approach
            logger.info(f"Starting simplified PDF generation for quiz ID: {quiz_id}")
            ...
            try:
                # Create Simple PDF - no rotation or complex watermark
                from fpdf import FPDF
                ...
                class SimplePDF(FPDF):
                    def __init__(self, quiz_id):
                        super().__init__()
                        self.quiz_id = quiz_id
                        self.title = f"Quiz {quiz_id} Results"
                        self.set_author("QuizBot")
                        self.set_title(self.title)
                ...
                # Generate PDF
                pdf = SimplePDF(quiz_id)
                pdf.add_page()
                
                # Add Basic Watermark
                pdf.set_font('Arial', 'B', 50)
                pdf.set_text_color(220, 220, 220)  # Light gray
                pdf.text(55, 100, "INSANE LEARNING")
                pdf.set_text_color(0, 0, 0)  # Reset to black
                
                # Add Table Header
                pdf.set_font('Arial', 'B', 12)
                pdf.cell(15, 10, "Rank", 1, 0, 'C')
                pdf.cell(60, 10, "Participant", 1, 0, 'C')
                pdf.cell(25, 10, "Score", 1, 0, 'C')
                pdf.cell(25, 10, "Correct", 1, 0, 'C')
                pdf.cell(25, 10, "Wrong", 1, 0, 'C')
                pdf.cell(25, 10, "Skipped", 1, 0, 'C')
                pdf.ln()
                ...
                for i, entry in enumerate(leaderboard):
                    ...
                    # Row content
                    pdf.cell(15, 10, str(entry.get("rank", "")), 1, 0, 'C', True)
                    pdf.cell(60, 10, entry.get("user_name", "")[:25], 1, 0, 'L', True)
                    pdf.cell(25, 10, str(entry.get("adjusted_score", 0)), 1, 0, 'C', True)
                    pdf.cell(25, 10, str(entry.get("correct_answers", 0)), 1, 0, 'C', True)
                    pdf.cell(25, 10, str(entry.get("wrong_answers", 0)), 1, 0, 'C', True)
                    pdf.cell(25, 10, str(entry.get("skipped", 0)), 1, 0, 'C', True)
                    pdf.ln()
                
                # Add Statistics
                pdf.ln(10)
                pdf.set_font('Arial', 'B', 14)
                pdf.cell(0, 10, "Quiz Statistics", 0, 1, 'L')
                
                total_participants = len(leaderboard)
                
                # Calculate average score if there are participants
                if total_participants > 0:
                    avg_score = sum(float(p.get("adjusted_score", 0)) for p in leaderboard) / total_participants
                else:
                    avg_score = 0.0
                
                pdf.set_font('Arial', '', 12)
                pdf.cell(0, 8, f"Total Participants: {total_participants}", 0, 1, 'L')
                pdf.cell(0, 8, f"Average Score: {avg_score:.2f}", 0, 1, 'L')
                pdf.cell(0, 8, f"Negative Marking: {penalty_value} points per wrong answer", 0, 1, 'L')
                pdf.cell(0, 8, f"Generated: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}", 0, 1, 'L')
                ...
                if os.path.exists(filename) and os.path.getsize(filename) > 100:
                    ...
                    await context.bot.send_message(
                        chat_id=chat_id,
                        text="✅ PDF Results generated successfully!"
                    )
                else:
                    await context.bot.send_message(
                        chat_id=chat_id,
                        text="❌ PDF could not be generated (file verification failed)"
                    )
            except Exception as e:

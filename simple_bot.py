...
# Ensure Tesseract can find the language data
import os
...
def extract_text_from_pdf_with_ocr_fallback(pdf_file_path):
    ...
    try:
        ...
        for page in reader.pages:
            ...
            if text:
                ...
                if len(text) < 50 or not any('\u0900' <= ch <= '\u097F' for ch in text):
                    raise ValueError("Likely unreadable Hindi, triggering OCR")
                text_list.append(text)
            else:
                raise ValueError("Empty or bad text")
        return text_list
        ...
        for page in doc:
            ...
            text_list.append(clean_hindi_text(text))
        return text_list
# OCR & Hindi Fix End
...
async def process_pdf_file(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        extracted_text_list = extract_text_from_pdf_with_ocr_fallback(pdf_file_path)

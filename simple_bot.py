...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "7631768276:AAF9XQfEfm9X1FT1X_GGVjZXdeiQe1qbwoo")
...
def extract_questions_from_txt(lines):
    ...
    # Only match patterns that have question marks in the text
    question_pattern1 = re.compile(r'^(?:Q\.?|Question)\s?(\d+)[\.:\)]?\s+(.*\?.*)', re.IGNORECASE)
    question_pattern2 = re.compile(r'^(\d+)[\.:\)]\s+(.*\?.*)', re.IGNORECASE)
    # No more backup patterns without question marks - we want to be strict about this
    
    # Pattern for options - match A), A., (A), etc.
    ...
    hindi_option_pattern = re.compile(r'^\s*\(?([१२३४]|[क-ञ])[\.:\)]?\s*(.*)', re.UNICODE)
    ...
    for line in lines:
        ...
        if question_match1 or question_match2 or hindi_match:
        ...
        if option_match or hindi_option_match:
            if processing_options:  # Only process options if we're expecting them after a question
                if option_match:
                    ...
                    hindi_letter = hindi_option_match.group(1)
                    option_letter = hindi_to_english.get(hindi_letter, 'A')
                    option_text = hindi_option_match.group(2).strip()
                ...
                if option_text.endswith('*') or option_text.endswith('✓') or option_text.endswith('✔'):

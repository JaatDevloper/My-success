...
# --- Conversation states for paste import ---
PASTE_CUSTOM_ID, PASTE_TEXT = range(103, 105)

# --- Paste Questions Conversation ---
async def paste_questions_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    await update.message.reply_text("Paste mode activated.\n\nPlease enter a custom ID to save your questions under:")
    return PASTE_CUSTOM_ID

async def paste_custom_id_received(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    context.user_data['paste_custom_id'] = update.message.text.strip()
    await update.message.reply_text(
        "Now paste your questions in the following format:\n\n"
        "1. Question text...\n"
        "A) Option 1\nB) Option 2\nC) Option 3\nD) Option 4\nउत्तर: B\n\n"
        "You can paste multiple questions at once."
    )
    return PASTE_TEXT

async def paste_text_received(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    import re
    raw_text = update.message.text
    custom_id = context.user_data.get('paste_custom_id', 'manual_paste')
    ...
    for block in question_blocks:
        ...
        for line in lines:
            line = line.strip()
            if re.match(r'^\d+\.|^Q\.', line, re.IGNORECASE):
                question = re.sub(r'^\d+\.|^Q\.', '', line).strip()
            elif re.match(r'^[A-D][).]', line):
                options.append(re.sub(r'^[A-D][).]\s*', '', line).strip())
            elif re.match(r'^(उत्तर|Answer)[:\-\s]+[A-D]', line, re.IGNORECASE):
                match = re.search(r'[A-D]', line.upper())
                if match:
                    answer_index = {'A': 0, 'B': 1, 'C': 2, 'D': 3}.get(match.group(), 0)
    ...
    if not parsed_questions:
        await update.message.reply_text("❌ No valid questions found. Please check your format.")
    ...
    all_questions[custom_id].extend(parsed_questions)
    save_questions(all_questions)

    await update.message.reply_text(f"✅ Successfully added {len(parsed_questions)} questions under ID: {custom_id}")
    return ConversationHandler.END
...
def main() -> None:
    ...
    # Paste questions import
    paste_conv_handler = ConversationHandler(
        entry_points=[CommandHandler("pastequestions", paste_questions_start)],
        states={
            PASTE_CUSTOM_ID: [MessageHandler(filters.TEXT & ~filters.COMMAND, paste_custom_id_received)],
            PASTE_TEXT: [MessageHandler(filters.TEXT & ~filters.COMMAND, paste_text_received)],
        },
        fallbacks=[CommandHandler("cancel", cancel)],
    )
    application.add_handler(paste_conv_handler)

...
async def generate_questions_pdf(quiz_id, questions_list):
    ...
    try:
        # Forcing ReportLab to be available since we installed it
        REPORTLAB_AVAILABLE = True
        
        # Import ReportLab for better Unicode support
        from reportlab.pdfgen import canvas
        ...
        if REPORTLAB_AVAILABLE:
            # Use ReportLab for Unicode support
            logger.info("Using ReportLab for PDF generation with Unicode support")
            
            # Register Hindi fonts using Noto Sans Devanagari
            hindi_font_path = os.path.join(os.getcwd(), "fonts", "NotoSansDevanagari-Regular.ttf")
            hindi_bold_font_path = os.path.join(os.getcwd(), "fonts", "NotoSansDevanagari-Bold.ttf")
            
            # Check if font files exist
            if os.path.exists(hindi_font_path) and os.path.exists(hindi_bold_font_path):
                try:
                    # Register Hindi fonts - we already imported the necessary modules
                    pdfmetrics.registerFont(TTFont('NotoSansDevanagari', hindi_font_path))
                    pdfmetrics.registerFont(TTFont('NotoSansDevanagari-Bold', hindi_bold_font_path))
                    logger.info("Successfully registered Hindi fonts")
                    
                    # Use custom Hindi font
                    hindi_font_registered = True
                except Exception as e:
                    logger.error(f"Error registering Hindi fonts: {e}")
                    hindi_font_registered = False
            else:
                logger.warning(f"Hindi font files not found at {hindi_font_path}")
                hindi_font_registered = False
            ...
            doc = SimpleDocTemplate(
                pdf_path,
                pagesize=A4,
                title=f"Quiz Questions - {quiz_id}",
                author="QuizBot"
            )
            ...
            # Add custom style for Hindi text - use Devanagari font if available
            if hindi_font_registered:
                font_name = 'NotoSansDevanagari'
            else:
                font_name = 'Helvetica'
                logger.warning("Using Helvetica instead of Hindi font - Unicode characters may not display correctly")
            ...
            # Title - Using HindiBold for better Hindi support in title
            styles.add(ParagraphStyle(
                name='Title_Hindi',
                fontName=bold_font_name,  # Use Hindi bold font if available
                fontSize=18,
                leading=22,
            ...
            elements.append(Paragraph(f"Quiz Questions - {quiz_id}", styles["Title_Hindi"]))
            
            # Date - Using Hindi font for consistent appearance
            date_text = f"Generated on: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}"
            elements.append(Paragraph(date_text, styles["Hindi"]))
            ...
            for i, question_data in enumerate(questions_list, 1):
                ...
                # Question number - use HindiBold for consistent font
                elements.append(Paragraph(f"<b>Question {i}:</b>", styles["HindiBold"]))
                
                # Question text
                elements.append(Paragraph(question_text, styles["Hindi"]))
                ...
                for j, option in enumerate(options):
                    ...
                    if j == correct_answer:
                        # Bold for correct answer - use HindiBold for Hindi text
                        option_text = f"<b>{option_letter}. {option} (Correct)</b>"
                        elements.append(Paragraph(option_text, styles["HindiBold"]))
                    else:
                        option_text = f"{option_letter}. {option}"
                        elements.append(Paragraph(option_text, styles["Hindi"]))
                
                # Category if available
                if category:
                    elements.append(Spacer(1, 0.1*inch))
                    elements.append(Paragraph(f"<i>Category: {category}</i>", styles["Hindi"]))
        ...
        # Fallback to FPDF if ReportLab not available
            ...
            logger.warning("Using FPDF - non-Latin characters may not display correctly")
            
            # Define safe text function for FPDF
            def safe_text(text):
            ...
            # Process all questions and options
            sanitized_questions = []
            ...
            # Save the PDF
            pdf.output(pdf_path)
            logger.info(f"PDF generated successfully with FPDF: {pdf_path}")
            return pdf_path
        
        else:
            logger.error("No PDF library available (neither ReportLab nor FPDF)")
            return None

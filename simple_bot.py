        ...
        if file_valid:
            try:
                ...
                try:
                    ...
                    # Direct import for simplicity and reliability
                    sys.path.insert(0, os.path.abspath('.'))
                    try:
                        import html_results_generator
                        logger.info("Successfully imported HTML results generator directly")
                    except ImportError as e:
                        logger.error(f"Direct import failed: {e}")
                        # Notify user about the HTML report generation failure
                        await update.message.reply_text("❌ HTML report generation failed: Module not found. PDF results are still available.")
                    ...
                    try:
                        html_file = html_results_generator.generate_html_report(
                            quiz_id=quiz_id,
...
async def html_report_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Try to import the HTML generator directly
        try:
            import html_results_generator
            logger.info("Successfully imported HTML results generator module")
        except ImportError as e:
            logger.error(f"Failed to import HTML generator: {e}")
            await update.message.reply_text(

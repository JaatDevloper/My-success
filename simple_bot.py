...
async def generate_questions_pdf(quiz_id, questions_list):
    """Generate a PDF document containing all questions for a quiz ID."""
    ...
    try:
        ...
        pdf.cell(0, 10, f"Quiz Questions - {quiz_id}", 0, 1, "C")
        ...
        for i, question_data in enumerate(questions_list, 1):
            ...
            for j, option in enumerate(options):
                if j == correct_answer:
                    pdf.set_font("Helvetica", "B", 10)  # Bold for correct answer
                    pdf.cell(0, 7, f"{chr(65+j)}. {option} (Correct)", 0, 1)
                else:
                    pdf.set_font("Helvetica", "", 10)
                    pdf.cell(0, 7, f"{chr(65+j)}. {option}", 0, 1)
            
            # Category if available
            if "category" in question_data and question_data["category"]:
                pdf.ln(2)
                pdf.set_font("Helvetica", "I", 10)
                pdf.cell(0, 7, f"Category: {question_data['category']}", 0, 1)

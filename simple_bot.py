...
async def generate_user_profile_pdf(user_id, user_name, user_profile=None):
    ...
    try:
        ...
        # Create PDF object
        pdf = UserProfilePDF(user_id, user_name)
        ...
        # Save PDF to file and also to memory for sending
        pdf.output(file_path)
        
        # Return the file path and a reopened file object for sending
        file_obj = open(file_path, 'rb')
        return file_path, file_obj
    
    except Exception as e:
        logger.error(f"Error generating user profile PDF: {e}")
        return None, None
...
async def user_profile_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Show user profile with comprehensive statistics"""
    try:
        user = update.effective_user
        if not user:
            await update.message.reply_text("❌ Could not identify user!")
        ...
        # Get recent activity (last 7 quizzes)
        quizzes_taken = user_profile.get("quizzes_taken", [])
        recent_quizzes = sorted(quizzes_taken, key=lambda x: x.get("timestamp", ""), reverse=True)[:7]
        ...
        # Format time period statistics
        time_period_stats = (
            f"<b>📅 Time Period Statistics:</b>\n"
            f"• <b>Today:</b> {daily_count} quizzes" + (f" (Avg: {daily_avg:.1f}%)" if daily_count > 0 else "") + "\n"
            f"• <b>Last 7 days:</b> {weekly_count} quizzes" + (f" (Avg: {weekly_avg:.1f}%)" if weekly_count > 0 else "") + "\n"
            f"• <b>Last 30 days:</b> {monthly_count} quizzes" + (f" (Avg: {monthly_avg:.1f}%)" if monthly_count > 0 else "") + "\n"
            f"• <b>Last 365 days:</b> {yearly_count} quizzes" + (f" (Avg: {yearly_avg:.1f}%)" if yearly_count > 0 else "")
        )
        
        # Format achievements
        achievements = user_profile.get("achievements", [])
        achievement_text = "<b>🏆 Achievements:</b>\n"
        
        if achievements:
            for achievement in achievements:
                emoji = get_achievement_emoji(achievement)
                description = get_achievement_description(achievement)
                achievement_text += f"• {emoji} <b>{description}</b>\n"
        else:
            achievement_text += "• No achievements yet. Keep taking quizzes!\n"
        
        # Format category statistics
        category_text = "<b>📚 Top Categories:</b>\n"
        if top_categories:
            for i, cat in enumerate(top_categories):
                category_text += f"• <b>{cat['name']}:</b> {cat['quizzes']} quizzes (Avg: {cat['avg_score']:.1f}%)\n"
        else:
            category_text += "• No category data available yet.\n"
        
        # Format recent activity
        recent_activity = "<b>🔄 Recent Activity:</b>\n"
        if recent_quizzes:
            for i, quiz in enumerate(recent_quizzes):
                ...
                recent_activity += f"• <b>{title}</b>: {score:.1f}% on {date}\n"
        else:
            recent_activity += "• No recent activity.\n"
        
        # Format streak information
        streak_text = (
            f"<b>🔥 Streak:</b> {current_streak} day" + ("s" if current_streak != 1 else "") + "\n"
            f"<b>🏆 Best Streak:</b> {best_streak} day" + ("s" if best_streak != 1 else "") + "\n"
            f"<b>📆 Last Quiz:</b> {last_quiz_date}"
        )
        
        # Build premium section
        premium_text = ""
        if is_premium:
            premium_text = f"<b>💎 PREMIUM STATUS:</b> Active\n"
        else:
            premium_text = (
                f"<b>💎 Premium Features:</b>\n"
                f"• Bypass force subscription requirements\n"
                f"• Access to exclusive quizzes\n"
                f"• Ad-free experience\n"
                f"Contact @JaatSupreme to upgrade!"
            )
        
        # Build complete profile message with styling
        profile_message = (
            f"<b>📊 QUIZ PROFILE: {user.first_name}</b>\n\n"
        ...
        # Create inline keyboard with download PDF button
        keyboard = [
            [InlineKeyboardButton("📥 Download PDF Report", callback_data="download_profile_pdf")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        # Send profile message with the download button
        await update.message.reply_html(profile_message, reply_markup=reply_markup)
        
    except Exception as e:
        logger.error(f"Error displaying user profile: {e}")
        await update.message.reply_text("❌ An error occurred while retrieving your profile. Please try again later.")
    # Update user profile
    user_profile.update({
        "quizzes_taken": quizzes_taken,
        "total_quizzes": total_quizzes,
...
async def handle_refresh_profile_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle refresh button on user profile"""
    query = update.callback_query
    await query.answer("Refreshing your profile...")
    
    # Get or create user profile
    user = update.effective_user
    detailed_profile = get_detailed_user_profile(user.id)
    
    # Rest of the handler continues below
...
async def handle_download_profile_pdf_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not detailed_profile:
        ...
        # Update the last_refreshed field
        detailed_profile["last_refreshed"] = datetime.datetime.now().isoformat()
        # Also check and update premium status
        detailed_profile["premium_status"] = is_premium_user(user.id)
        # Save the updated profile
        save_detailed_user_profile(detailed_profile)
    
    # Create a custom update object to simulate a message
    class CustomUpdate:
    ...
    class CustomMessage:
        ...
        async def reply_html(self, text, reply_markup=None):
            # Replace the current message instead of replying
            await query.edit_message_text(
                text=text,
                parse_mode='HTML',
                reply_markup=reply_markup
            )
            # Return a dummy message that supports delete()
            return type('obj', (object,), {'delete': lambda: None})
    ...
    # Create the custom message and update
    custom_message = CustomMessage(query.message.chat_id)
    custom_update = CustomUpdate(update.effective_user, custom_message)
    
    # Forward to the user profile command with our custom update

...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "7631768276:AAHrOtFVMwsi32sP4KgvkV4c-CaPQL6Feyw")
...
async def add_question_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    print(f"Received question text: {update.message.text}")
    logger.info(f"Received question text: {update.message.text}")
    
    # Store the question text in user_data with a simpler structure
    context.user_data["question_text"] = update.message.text
    
    # Also store in the expected "new_question" dictionary structure
    # This helps ensure backward compatibility
    context.user_data["new_question"] = {
        "question": update.message.text
    }
    
    # Explicitly clear any previous data
    ...
    print("Moving to OPTIONS state")
    logger.info("Moving to OPTIONS state")
    return OPTIONS
...
async def add_question_options(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    print(f"Received options text: {update.message.text}")
    
    # Parse options from message text
    ...
    # Create a formatted text for displaying options
    ...
    print(f"Moving to ANSWER state with {len(options)} options")
    return ANSWER
...
async def add_question_answer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        # Log received answer
        print(f"Received answer: {update.message.text}")
        ...
        if 0 <= answer < len(options):
            ...
            # Reconstruct the question data structure
            context.user_data["new_question"] = {
            ...
            print("Moving to CUSTOM_ID state")
            return CUSTOM_ID
        else:
            await update.message.reply_text(
...
async def custom_id_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Log the callback
    print(f"Received custom_id_callback: {query.data}")
    logger.info(f"Received custom_id_callback: {query.data}")
    
    if query.data == "auto_id":
        # Auto-generate ID and continue to category
        context.user_data["auto_id_selected"] = True
        # Remove any custom ID flags if present
        ...
        print("Moving to CATEGORY state with auto ID")
        logger.info("Moving to CATEGORY state with auto ID")
        return CATEGORY
    else:  # custom_id selected
        # Mark that we're waiting for a custom ID input
        context.user_data["awaiting_custom_id"] = True
        # Remove auto ID flag if present
        if "auto_id_selected" in context.user_data:
            del context.user_data["auto_id_selected"]
        
        # Ask user to input a custom ID
        await query.edit_message_text(
            "Please enter a custom ID for this question (can be text or numbers).\n\n"
            "If the ID already exists, your question will be added to that ID without overwriting existing questions."
        )
        print("Staying in CUSTOM_ID state, waiting for user input")
        logger.info("Staying in CUSTOM_ID state, waiting for user input")
        return CUSTOM_ID

async def custom_id_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Handle custom ID input."""
    # Log the incoming message
    print(f"Received custom ID input: {update.message.text}")
    
    # Check if we're actually waiting for an ID
    if not context.user_data.get("awaiting_custom_id", False):
        print("Warning: received custom ID but not waiting for one")
        await update.message.reply_text(
            "I wasn't expecting an ID at this time. Please start over with /add"
        )
        return ConversationHandler.END
    
    # Accept any string as the ID
    custom_id = update.message.text.strip()
    context.user_data["custom_id"] = custom_id
    del context.user_data["awaiting_custom_id"]
    
    print(f"Accepted custom ID: {custom_id}")
    ...
    print("Moving to CATEGORY state")
    return CATEGORY
...
async def poll_to_question(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    print("Starting poll2q conversation")
...
async def handle_forwarded_poll(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if message.forward_from_chat and hasattr(message, 'poll') and message.poll:
        ...
        return ANSWER  # Return state for ConversationHandler
    else:
        await update.message.reply_text(
            "That doesn't seem to be a poll message. Please forward a message containing a poll."
        )
        return ConversationHandler.END  # End conversation if not a valid poll
...
async def handle_poll_answer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Log the selection
    print(f"Poll answer selected: {query.data}")
    ...
    print(f"Poll data updated: {poll_data}")
    ...
    print("Moving to CUSTOM_ID state for poll")
    return CUSTOM_ID
...
async def handle_poll_id_selection(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    print(f"Received poll ID selection: {query.data}")
    ...
    if query.data == "pollid_auto":
        ...
        print("Moving to CATEGORY state with auto poll ID")
        ...
        print("Staying in CUSTOM_ID state, waiting for poll ID input")
...
async def handle_poll_custom_id(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    print(f"Handling poll custom ID: {update.message.text}")
    print(f"Context data: {context.user_data}")
    ...
    print(f"Received custom ID for poll: {custom_id}")
    ...
    print(f"Moving to CATEGORY state for poll with custom ID: {custom_id}")
...
async def handle_custom_negative_marking(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        logger.info(f"Custom negative marking: Retrieved quiz_id={quiz_id}")
        
        if not quiz_id:
            logger.error("Custom negative marking: No quiz ID found in context")
...
async def quiz_with_id_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    quiz_id = " ".join(context.args).strip()
    logger.info(f"Starting quiz with ID: {quiz_id}")
    
    # Load all questions
    all_questions = load_questions()
    
    # Store the questions in user_data temporarily - this fixes the issue with questions not being available
    # in the callback handler for negative marking
    ...
    # Store questions in context for use in the callback
    # This is critical for ensuring the questions are available when the negative marking callback runs
    context.user_data[f"temp_quiz_{quiz_id}_questions"] = questions.copy()
...
async def receive_txt_file(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        try:
            ...
            # Ask user if they want to use auto ID or custom ID
            keyboard = [
                [InlineKeyboardButton("Use Auto ID", callback_data=f"txtid_auto")],
                [InlineKeyboardButton("Set Custom ID", callback_data=f"txtid_custom")],
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            
            await update.message.reply_text(
                f"I've prepared your file for processing.\n\n"
                f"Auto-generated ID: <b>{auto_id}</b>\n\n"
                f"Would you like to use this ID or set a custom one?",
                reply_markup=reply_markup,
                parse_mode='HTML'
            )
...
async def process_txt_file(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    print(f"Processing text file with context: {context.user_data}")
    logger.info(f"Processing text file with context keys: {list(context.user_data.keys())}")
    ...
    print(f"Processing txt file. Path: {file_path}, ID: {custom_id}")
    logger.info(f"Processing txt file. Path: {file_path}, ID: {custom_id}")
    
    # Early validation
    if not file_path:
        print("No file path found in context")
    ...
    # Determine the appropriate way to send messages based on the update type
    # If the update comes from a callback query (button press), use the callback query
    # If it comes from a message, use the message object
    ...
    if not questions:
        ...
        try:
            ...
            print(f"Error sending no questions found message: {e}")
    ...
    try:
        ...
        try:
            ...
            print(f"Final attempt to send message failed: {e2}")
...
async def handle_txtid_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    print(f"Received txtid callback: {callback_data}")
    ...
    if callback_data == "txtid_auto":
        ...
        print(f"Processing text import with auto ID: {auto_id}")
...
def extract_questions_from_txt(lines):
    ...
    Specially optimized for Hindi/Rajasthani quiz formats with lettered options (A/B/C/D) and checkmarks
    """
    questions = []
    ...
    # Add special handling for Hindi/Rajasthani format with checkmarks
    # Pattern to detect lines with questions and options in Hindi format
    hindi_format_pattern = re.compile(r'(.+?)\s*\(([A-D])\)\s*([^\(]+?)(\s*\(([A-D])\).*|$)', re.UNICODE)
    ...
    # First, try to process each line as a complete question with options (Hindi/Rajasthani format)
    # This format: "बप्पा रावल कौन था? (A) राजा ✅ (B) मजदूर (C) कारीगर (D) उद्यमी"
    ...
    for line in lines:
        ...
        # Check if this line contains both a question and options with checkmarks
        if "?" in line and "(" in line and ")" in line and ("✅" in line or "✓" in line or "✔" in line):
            # This is likely a complete question line with options and a marked answer
            
            # First, split by question mark to get the question part
            question_parts = line.split("?", 1)
            if len(question_parts) < 2:
                continue  # No valid question mark found
                
            question_text = question_parts[0].strip() + "?"
            options_text = question_parts[1].strip()
            
            # Extract options using regex pattern for (A), (B), etc.
            # Enhanced pattern to better handle Hindi/Rajasthani format
            option_matches = re.findall(r'\(([A-D])\)\s*([^\(]+?)(?=\s*\([A-D]\)|$)', options_text, re.UNICODE)
            logger.info(f"Found {len(option_matches)} options in single-line format: {option_matches}")
            
            if len(option_matches) < 2:  # Need at least 2 options
                continue
                
            # Process the options
            options = ["" for _ in range(4)]  # Initialize with empty strings for A,B,C,D
            correct_answer = 0
            
            for match in option_matches:
                letter, text = match
                option_index = ord(letter.upper()) - ord('A')  # Convert A->0, B->1, etc.
                
                # Check if this option has a checkmark
                has_checkmark = any(mark in text for mark in ["✅", "✓", "✔", "*"])
                
                # Clean the option text
                clean_text = re.sub(r'[✓✔✅\*]', '', text).strip()
                
                # If the option is too long, truncate it
                if len(clean_text) > MAX_OPTION_LENGTH:
                    clean_text = clean_text[:MAX_OPTION_LENGTH-3] + "..."
                
                # Add the option
                if 0 <= option_index < len(options):
                    options[option_index] = clean_text
                    
                    # If this option has a checkmark, mark it as correct
                    if has_checkmark:
                        correct_answer = option_index
            
            # Remove any trailing empty options
            ...
            if question_text and len(options) >= 2:
                ...
                logger.info(f"Extracted Hindi format question: {question_text[:30]}..., options: {len(options)}")
    
    # If hindi format processing worked, return the questions
    if questions:
        logger.info(f"Successfully extracted {len(questions)} questions using Hindi format parser")
        return questions
    
    # If we reach here, try the block-based approach
    logger.info("Hindi format parser found no questions, trying block-based approach")
...
def main() -> None:
    ...
    pdf_import_conv_handler = ConversationHandler(
        ...
        per_message=False,  # Added to fix custom ID handling
        name="pdf_import_handler"  # Adding name for better tracking in logs
    )
    ...
    poll2q_conv_handler = ConversationHandler(
        ...
        fallbacks=[CommandHandler("cancel", cancel)],
        per_message=False,  # Critical parameter for fixing the conversation flow
    ...
    # We no longer need this additional handler since we incorporated it into the ConversationHandler
    # Remove it to prevent handler conflicts
    # application.add_handler(MessageHandler(
    ...
    add_question_handler = ConversationHandler(
        ...
        fallbacks=[CommandHandler("cancel", cancel)],
        per_message=False,  # Critical parameter for fixing custom ID handling
        allow_reentry=True,  # Allow re-entering the conversation
    ...
    # Text Import conversation handler with custom ID support

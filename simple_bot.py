...
class InsaneResultPDF(FPDF):
    ...
    def __init__(self, quiz_id, title=None):
        ...
        self.set_margin(10)  # Set all margins to 10mm
...
def generate_pdf_results(quiz_id, title=None):
    ...
    try:
        ...
        try:
            ...
            simple_filename = os.path.join(PDF_RESULTS_DIR, f"quiz_{quiz_id}.pdf")
            try:
                pdf.output(simple_filename)
                filename = simple_filename
                logger.info(f"PDF output succeeded with simplified filename: {filename}")
            except Exception as e2:

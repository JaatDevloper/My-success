...
class InsaneResultPDF(FPDF):
    ...
    def add_quiz_statistics(self, leaderboard, penalty_value):
        ...
        self.cell(0, 7, f"Report generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", 0, 1, 'L')
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                simple_pdf.cell(0, 10, f"Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", 0, 1, 'R')
                ...
                try:
                    ...
                    try:
                        ...
                        minimal_pdf.ln(10)
                        minimal_pdf.cell(0, 10, f'Generated on: {datetime.now().strftime("%Y-%m-%d %H:%M")}', 0, 1, 'C')
...
class QuizQuestionsPDF:
    ...
    def __init__(self):
        ...
        ensure_pdf_directory()
        logger.info("QuizQuestionsPDF initialized")
    
    def generate_quiz_pdf(self, quiz_data, quiz_id):
        """Generate a PDF with quiz questions, options, and correct answers"""
        logger.info(f"Generating quiz PDF for quiz ID: {quiz_id}")
        
        # Validate quiz data
        if not quiz_data or not isinstance(quiz_data, list):
            logger.error(f"Invalid quiz data: {type(quiz_data)}")
            return None
        
        # Try ReportLab first if available
        if REPORTLAB_AVAILABLE:
            logger.info("Attempting to generate PDF with ReportLab")
            pdf_path = self.generate_quiz_pdf_reportlab(quiz_data, quiz_id)
            if pdf_path and os.path.exists(pdf_path):
                logger.info(f"Successfully generated PDF with ReportLab: {pdf_path}")
                return pdf_path
        
        # Fall back to FPDF if ReportLab fails or isn't available
        if FPDF_AVAILABLE:
            logger.info("Attempting to generate PDF with FPDF")
            pdf_path = self.generate_quiz_pdf_fpdf(quiz_data, quiz_id)
            if pdf_path and os.path.exists(pdf_path):
                logger.info(f"Successfully generated PDF with FPDF: {pdf_path}")
                return pdf_path
        
        # If all methods fail, return None
        logger.error("Failed to generate quiz PDF - no PDF libraries available or generation failed")
        return None
        ...
        try:
            ...
            for i, question_data in enumerate(quiz_data, 1):
                ...
                for j, option in enumerate(options):
                    # Mark correct answer with (CORRECT) to avoid encoding issues
                    if j == correct_answer_index:
                        option_text = f"{chr(65+j)}. {option} (CORRECT)"
                    else:
    ...
    try:
        ...
        if context.args and len(context.args) > 0:
            ...
            if recent_quizzes:
                ...
                # If no recent quizzes, use '123' as default quiz ID
                quiz_id = '123'
                logger.info(f"No recent quizzes found, using default quiz ID: {quiz_id}")
        
        # Get quiz questions
        questions = load_questions()
        quiz_questions = []
        
        # Convert quiz_id to string to ensure dictionary key match
        str_quiz_id = str(quiz_id)
        quiz_questions = questions.get(str_quiz_id, [])
        
        # Log information
        logger.info(f"Quiz ID: {quiz_id}, String Quiz ID: {str_quiz_id}")
        logger.info(f"Available quiz IDs: {list(questions.keys())}")
        logger.info(f"Found {len(quiz_questions)} questions for quiz ID {str_quiz_id}")
        
        if not quiz_questions:
            # If no questions found for the ID, try '123' as fallback
            fallback_id = '123'
            quiz_questions = questions.get(fallback_id, [])
            logger.info(f"Using fallback ID {fallback_id}, found {len(quiz_questions)} questions")
            ...
            if not quiz_questions:
                ...
                # Use fallback ID instead
                quiz_id = fallback_id
                logger.info(f"Using fallback quiz ID: {quiz_id}")
        ...
        if not ensure_pdf_directory():
            await context.bot.edit_message_text(
                chat_id=chat_id,
                message_id=processing_message.message_id,
                text="❌ Failed to create PDF directory. Please try again later."
            )
            return
        
        # Define the file path directly for more control
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        pdf_filename = f"quiz_{quiz_id}_{timestamp}.pdf"
        pdf_path = os.path.join(PDF_RESULTS_DIR, pdf_filename)
        
        try:
            # Initialize PDF generator with more detailed logging
            logger.info("Creating PDF generator instance...")
            ...
            if quiz_questions:
                logger.info(f"First question sample: {quiz_questions[0]}")
            
            # Generate PDF with more detailed error handling
            logger.info("Calling generate_quiz_pdf method...")
            pdf_path = pdf_generator.generate_quiz_pdf(quiz_questions, quiz_id)
            logger.info(f"PDF generation completed. Path: {pdf_path}")
            
            # Check if PDF was created
            if not pdf_path or not os.path.exists(pdf_path):
                logger.error(f"PDF generation failed. Path: {pdf_path}, Exists: {os.path.exists(pdf_path) if pdf_path else False}")
                
                # Try direct generation with FPDF as last resort
                try:
                    logger.info("Attempting direct FPDF generation as fallback...")
                    if FPDF_AVAILABLE:
                        pdf = FPDF()
                        pdf.add_page()
                        pdf.set_font("Arial", "B", 16)
                        pdf.cell(0, 10, f"Quiz #{quiz_id} - Questions", 0, 1, "C")
                        pdf.set_font("Arial", "", 12)
                        
                        for i, q in enumerate(quiz_questions, 1):
                            pdf.cell(0, 10, f"Question {i}: {q.get('question', 'No text')}", 0, 1)
                        ...
                        pdf.output(pdf_path)
                        logger.info(f"Direct FPDF generation successful: {pdf_path}")
                    else:
                        logger.error("FPDF not available for fallback generation")
                except Exception as fallback_err:
                    logger.error(f"Fallback PDF generation failed: {fallback_err}")
                    await context.bot.edit_message_text(
                        chat_id=chat_id,
                        message_id=processing_message.message_id,
            ...
            # Double check the file exists before trying to send
            if not os.path.exists(pdf_path):
                logger.error(f"PDF file does not exist at path: {pdf_path}")
                await context.bot.edit_message_text(
                    chat_id=chat_id,
                    message_id=processing_message.message_id,
                    text="❌ Failed to create PDF file. Please try again."
                )
                return
                
            logger.info(f"PDF file created successfully at: {pdf_path}")
        except Exception as e:
            ...
            await context.bot.edit_message_text(
                chat_id=chat_id,
                message_id=processing_message.message_id,
                text=f"❌ Error generating PDF: {str(e)[:50]}..."
            )
            return
        
        # Send the PDF file - with explicit file opening
        try:
            ...
            logger.error(f"Error sending PDF file: {send_err}")
            import traceback
            logger.error(traceback.format_exc())
            await context.bot.edit_message_text(
                chat_id=chat_id,
                message_id=processing_message.message_id,
                text=f"❌ PDF was generated but failed to send: {str(send_err)[:50]}..."
            )
...
def cleanup_old_pdfs():
    """Clean up old PDF files to avoid disk space issues"""
    from datetime import timedelta
    
    # Only keep PDFs from the last hour
    ...
    try:
        for filename in os.listdir(PDF_RESULTS_DIR):
            if filename.endswith('.pdf'):
                filepath = os.path.join(PDF_RESULTS_DIR, filename)
                file_modified = datetime.fromtimestamp(os.path.getmtime(filepath))
                
                if file_modified < one_hour_ago:
                    try:
                        os.remove(filepath)
                        logger.info(f"Removed old PDF file: {filename}")
                    except Exception as e:
                        logger.warning(f"Could not remove old PDF file {filename}: {e}")
    except Exception as e:
        logger.warning(f"Error during PDF cleanup: {e}")

# Enhanced ensure_pdf_directory function - call this before every PDF generation
# to make sure old PDFs are cleaned up
old_ensure_pdf_directory = ensure_pdf_directory

def enhanced_ensure_pdf_directory():
    """Enhanced version of ensure_pdf_directory that also cleans up old files"""
    result = old_ensure_pdf_directory()
    if result:
        try:
            cleanup_old_pdfs()
        except Exception as e:
...
ensure_pdf_directory = enhanced_ensure_pdf_directory

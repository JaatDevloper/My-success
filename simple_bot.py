...
async def show_negative_marking_options(update: Update, context: ContextTypes.DEFAULT_TYPE, quiz_id, questions=None):
    ...
    # Create inline keyboard with advanced negative marking options
    keyboard = []
    
    # Add buttons for each negative marking option
    ...
    await update.message.reply_text(
        f"📊 Select negative marking value for this quiz:\n\n"
        f"This determines how many points are deducted for wrong answers.\n"
        f"Choose 'Custom' for a specific value not listed here.",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
    
    # Store questions in context for later use after user selects negative marking
    if questions:
        context.user_data[f"temp_quiz_{quiz_id}_questions"] = questions

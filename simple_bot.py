...
async def end_quiz(context, chat_id):
    ...
    # Generate and send PDF with all questions and answers
    try:
        ...
        if REPORTLAB_AVAILABLE:
            ...
            if quiz_id and quiz_id in load_questions():
                first_question = load_questions()[quiz_id][0] if load_questions()[quiz_id] else {}
                category = first_question.get("category", "")
                if category:
                    quiz_title = f"{category} Quiz #{quiz_id}"
            
            # Send the PDF with questions and answers
            if questions:
                await send_quiz_pdf(context, chat_id, quiz_id, quiz_title, questions)
    except Exception as e:

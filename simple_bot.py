...
async def receive_txt_file(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    file_path = os.path.join(TEMP_DIR, f"{file.file_name}")
...
async def process_txt_file(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not file_path or not os.path.exists(file_path):
        await update.message.reply_text("❌ File not found or download failed.")
        return ConversationHandler.END
    ...
    add_questions_with_id(custom_id, questions)
...
def main() -> None:
    ...
    txtimport_handler = ConversationHandler(
        ...
        states={
            ...
            TXT_PROCESSING: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, process_txt_file),
                CommandHandler("cancel", txtimport_cancel),
            ],
        },

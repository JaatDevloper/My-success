...
def extract_questions_from_text(text):
    ...
    for line in lines:
        ...
        # Check for question pattern (Q1, 1., Question 1, etc.)
        q_match = re.search(r'(?:^|\n)((?:Q|Question)?\s*\d+\.?\s*)(.*?)(?:\?|$)', line, re.IGNORECASE)
        if q_match:
            # Save previous question if exists
            if current_question:
                questions.append({
        ...
        # Check for options (A) Option, B. Option, etc.)
        opt_match = re.search(r'^([A-D])[.)]?\s+(.*)', line, re.IGNORECASE)
...
def scrape_quiz_content(url):
    ...
    if not questions:
        ...
        try:
            ...
            question_elements = soup.select('.question, .quiz-question, .mcq, .question-text')
...
def format_questions_for_bot(questions, category="Web Scraped"):
    ...
    for qa_pair in questions:
        ...
        if qa_pair['options']:
            ...
            for i, (opt_letter, opt_text) in enumerate(qa_pair['options']):
                options.append(opt_text)
                
                # Check if this is the correct answer
                if qa_pair['answer'] and opt_letter == qa_pair['answer']:
            ...
            # Generate fake options if none are available
...
async def web_url_received(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not is_valid_url(url):
        await update.message.reply_text(
            "⚠️ Invalid URL format. Please enter a valid URL starting with http:// or https://\n"
    ...
    if error:
        await update.message.reply_text(
            f"❌ Error: {error}\n\n"
            "Please try a different URL or send /cancel to abort."
        )
        return WEB_URL

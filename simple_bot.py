...
# --- Updated /result Command (Live User Score Based) ---
async def result(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_data:
        await update.message.reply_text("❌ You haven't completed any quiz yet.")
        return

    html_path = generate_watermarked_html(
        username=username,
        total=user_data.get("total", 0),
        correct=user_data.get("correct", 0),
        incorrect=user_data.get("incorrect", 0),
        rank=user_data.get("rank", None)
    )
    await context.bot.send_document(chat_id=update.effective_chat.id, document=open(html_path, "rb"), filename=html_path.name)



# ✅ Debug-Ready /result Command
async def result(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        user = update.effective_user
        user_id = user.id
        ...
        print(f"[DEBUG] /result command triggered by user: {user_id}")
        await update.message.reply_text("⏳ Generating your result...")

        stats = get_extended_user_stats(user_id)

        from html_result import generate_watermarked_html  # adjust if inside same file
        ...
        await context.bot.send_document(
            ...
            caption=f"📊 Result for @{username}"
        )

    except Exception as e:
        print(f"[ERROR] /result command failed: {e}")
        await update.message.reply_text("❌ An error occurred while generating your result.")

...
def main() -> None:
    ...
    # Quiz ID command and callback
    application.add_handler(CommandHandler("quizid", quizid_command))
    application.add_handler(CallbackQueryHandler(
        lambda update, context: start_quiz_with_id(update, context, update.callback_query.data.replace("quiz_id_", "")),
        pattern=r"^quiz_id_"
    ))
    
    # TXT import conversation handler
    txt_import_handler = ConversationHandler(
        entry_points=[CommandHandler("txtimport", import_txt_start)],
        states={
            TXT_UPLOAD: [
                MessageHandler(filters.Document.ALL & ~filters.COMMAND, import_txt_upload),
            ],
            TXT_CUSTOM_ID: [
                CallbackQueryHandler(import_txt_custom_id, pattern=r"^(auto_id|custom_id)$"),
                MessageHandler(
                    filters.TEXT & ~filters.COMMAND, 
                    lambda update, context: process_txt_file(update, context) if context.user_data.get('awaiting_custom_id') else None,
                ),
            ],
        },

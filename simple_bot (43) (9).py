...
FORCE_SUB_CHANNEL_URL = "https://t.me/NegativeMarkingTestbot"
FORCE_SUB_CHANNEL_USERNAME = "NegativeMarkingTestbot"
FORCE_SUB_BYPASS_COMMANDS = ["/start", "/help", "/about", "/stats", "/faqs", "/join"]
...
async def mongodb_debug_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Try to get database and collection
        try:
            db = mongodb_client[MONGO_DB_NAME]
            collection = db[MONGO_QUIZ_COLLECTION]
            await update.message.reply_text(f"✅ Access to collection {MONGO_QUIZ_COLLECTION} successful")
        except Exception as e:
...
# ---------- FORCE SUBSCRIPTION IMPLEMENTATION ----------
# All force subscription functionality is implemented directly in this file
...
async def join_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Command to join the force subscription channel"""
    keyboard = [
        [InlineKeyboardButton("Join Our Channel", url=FORCE_SUB_CHANNEL_URL)]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    await update.message.reply_text(
        "📢 <b>Join Our Official Channel</b>\n\n"
        "Thank you for using our quiz bot! Please join our official channel to receive "
        "updates about new features, quiz competitions, and more.",
        reply_markup=reply_markup,
        parse_mode=ParseMode.HTML
    )
    
    # If user already subscribed, let them know
    user_id = update.message.from_user.id
    if FORCE_SUB_ENABLED and await check_user_subscription(context, user_id):
        await update.message.reply_text(
            "✅ You are already subscribed to our channel. Thank you for your support!",
            parse_mode=ParseMode.HTML
        )
...
async def about_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    about_text = (
        "📝 <b>INSANE Quiz Bot</b> 📝\n\n"
        "A powerful Telegram bot for creating and sharing interactive quizzes.\n\n"
        "<b>Version:</b> 2.0\n"
        "<b>Created by:</b> @NegativeMarkingTestbot\n\n"
        "<b>Features:</b>\n"
        ...
        "• Quiz sharing\n"
        "• MongoDB integration\n\n"
        "Join our channel for updates and new features!"
    )
    
    # Add keyboard with join button
    keyboard = [
        [InlineKeyboardButton("Join Our Channel", url="https://t.me/NegativeMarkingTestbot")],
        [InlineKeyboardButton("Create a Quiz", callback_data="dummy_create_quiz")]
    ]
...
async def handle_database_channel_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    is_in_database_channel = False
    
    # Check username
    if hasattr(message.chat, 'username') and message.chat.username == DATABASE_CHANNEL_USERNAME:
        is_in_database_channel = True
        
    # Check title as fallback
    if hasattr(message.chat, 'title') and "QuizbotDatabase" in message.chat.title:
        is_in_database_channel = True
        
    # Also check chat id if available
    if hasattr(message.chat, 'id') and str(message.chat.id) == context.bot_data.get('database_channel_id'):
        is_in_database_channel = True
        
    if not is_in_database_channel:
    ...
    logger.info(f"Database channel message text: {message_text[:100]}...")
    
    # More comprehensive pattern matching for quiz ID
    quiz_id_match = None
    for pattern in [
        r"Quiz ID:\s*([A-Za-z0-9]+)",
        r"quiz_id[:=]\s*([A-Za-z0-9]+)",
        r"ID[:=]\s*([A-Za-z0-9]+)",
        r"Quiz:\s*([A-Za-z0-9]+)",
        r"^([A-Za-z0-9]{6,})$"  # Fallback for just ID
    ]:
    ...
    # If we still don't have a match, check via inline keyboard buttons
    if not quiz_id_match and message.reply_markup:
        for row in message.reply_markup.inline_keyboard:
            for button in row:
                if 'callback_data' in button and button.callback_data:
                    # Look for patterns like quiz_12345 or start_quiz_12345
                    cb_match = re.search(r'(?:quiz_|start_quiz_)([A-Za-z0-9]+)', button.callback_data)
                    if cb_match:
                        quiz_id_match = cb_match
                        break
    
    # Process if we found a quiz ID
    if quiz_id_match:
        ...
        # Extract quiz metadata - more comprehensive patterns
        quiz_name = None
        for pattern in [
            r"Quiz Name:\s*(.+?)(?:\n|$)",
            r"Title:\s*(.+?)(?:\n|$)",
        ...
        quiz_data = {
            ...
            **creator_info  # Add creator info if available
        }
        ...
        if save_quiz_to_mongodb(quiz_data):
            ...
            try:
                count = quiz_collection.count_documents({})
                logger.info(f"Total quizzes in MongoDB after save: {count}")
            ...
            try:
                await message.reply_text(
                    ...
                    f"Total quizzes in database: {count if 'count' in locals() else 'unknown'}"
                )
            except Exception as e:
...
async def mongodb_status_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Check MongoDB connection and list saved quizzes."""
    try:
        ...
        # Get 5 recent quizzes to display
        recent_quizzes = list(quiz_collection.find().sort("created_at", -1).limit(5))
        
        message = f"✅ MongoDB connection is working\n\n"
        message += f"📊 Total quizzes stored: {count}\n\n"
        
        if len(recent_quizzes) > 0:
            message += "📝 Most recent quizzes:\n"
            for quiz in recent_quizzes:
                ...
                if isinstance(questions, list):
                    ...
                    questions_count = len(questions.keys())
                else:
                    questions_count = 0
                    
                creator_name = quiz.get("creator_name", "Unknown")
                message += f"- {quiz_title} (ID: {quiz_id}, Questions: {questions_count}, Creator: {creator_name})\n"
        else:
            message += "No quizzes found in the database yet."
...
async def check_user_subscription(context: ContextTypes.DEFAULT_TYPE, user_id: int) -> bool:
    ...
    try:
        # Get chat member status from channel
        chat_member = await context.bot.get_chat_member(f"@{FORCE_SUB_CHANNEL_USERNAME}", user_id)
        
        # Check if user is a member of the channel
        is_member = chat_member.status in ["member", "administrator", "creator"]
        logger.info(f"User {user_id} subscription check: {chat_member.status}")
        return is_member
    except Exception as e:
        logger.error(f"Error checking user subscription: {e}")
        # In case of error, don't block user access
        return True

async def send_force_subscription_message(update: Update) -> None:
    """Send force subscription message with join button"""
    image_path = None
    
    # Try to send image from file (to match the screenshot)
    try:
        image_dir = "bot_images"
        if os.path.exists(image_dir):
            image_files = [f for f in os.listdir(image_dir) if f.lower().endswith(('.png', '.jpg', '.jpeg'))]
            if image_files:
                image_path = os.path.join(image_dir, image_files[0])
    except Exception as e:
        logger.error(f"Error finding subscription image: {e}")
    
    # Create keyboard with join button
    keyboard = [
        [InlineKeyboardButton("Join Now...", url=FORCE_SUB_CHANNEL_URL)]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    # Use an image in the message if available
    try:
    ...
    return await update.message.reply_text(
        ...
        "To access all the amazing features of this bot, you need to join our official channel first.",
        reply_markup=reply_markup,
        parse_mode=ParseMode.HTML
    )

async def check_subscription_filter(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    """
    Check if user is subscribed to the required channel
    Returns True if user is subscribed or force subscription is disabled
    Returns False if user needs to subscribe
    """
    # Skip check if force subscription is disabled
    if not FORCE_SUB_ENABLED:
        return True
        
    # Don't check subscription for channel messages and callback queries
    if not update.message or not update.message.from_user:
        return True
        
    # Don't check for bypass commands
    command = update.message.text
    if command and any(command.startswith(cmd) for cmd in FORCE_SUB_BYPASS_COMMANDS):
        return True
    
    # Check user's subscription status
    user_id = update.message.from_user.id
    is_subscribed = await check_user_subscription(context, user_id)
    
    # If not subscribed, send force subscription message
    if not is_subscribed:
        await send_force_subscription_message(update)
        return False
        
    return True

# Force subscription wrapper for command handlers
def create_force_sub_handler(command_handler):
    """
    Create a wrapper to check force subscription before executing a command
    Returns a handler function that checks subscription status before executing the command
    """
    async def wrapped_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        # Skip check for certain bypass commands
        ...
        if not FORCE_SUB_ENABLED or command_name in FORCE_SUB_BYPASS_COMMANDS:
            # No need to check subscription, proceed with command
        ...
        if is_subscribed:
            ...
            logger.info(f"User {user_id} is subscribed, executing command")
            return await command_handler(update, context)
        else:
            # User not subscribed, show force subscription message
...
def main() -> None:
    ...
    # Add join channel command (always accessible)
    application.add_handler(CommandHandler("join", join_command))
    
    # Basic command handlers without force subscription
    application.add_handler(CommandHandler("start", start))
    application.add_handler(CommandHandler("help", help_command))
    application.add_handler(CommandHandler("features", features_command))
    application.add_handler(CommandHandler("about", about_command))
    
    # Add MongoDB debug commands (always accessible)
    application.add_handler(CommandHandler("mongodbstatus", mongodb_status_command))
    application.add_handler(CommandHandler("mongodebug", mongodb_debug_command))
    
    # Commands that require force subscription
    application.add_handler(CommandHandler("quiz", create_force_sub_handler(quiz_command)))
    application.add_handler(CommandHandler("create", create_force_sub_handler(create_command)))
    application.add_handler(CommandHandler("myquizzes", create_force_sub_handler(myquizzes_command)))
    application.add_handler(CommandHandler("export", create_force_sub_handler(export_command)))
    application.add_handler(CommandHandler("import", create_force_sub_handler(import_command)))
    application.add_handler(CommandHandler("stop", create_force_sub_handler(stop_quiz_command)))
    application.add_handler(CommandHandler("stats", create_force_sub_handler(stats_command)))
    application.add_handler(CommandHandler("delete", create_force_sub_handler(delete_command)))
    ...
    create_quiz_conv_handler = ConversationHandler(
        entry_points=[CommandHandler("create", create_force_sub_handler(create_command))],
        states={
    ...
    pdf_import_conv_handler = ConversationHandler(
        entry_points=[CommandHandler("pdfimport", create_force_sub_handler(pdf_import_command))],
        states={
    ...
    add_question_handler = ConversationHandler(
        entry_points=[CommandHandler("add", create_force_sub_handler(add_question_start))],
        states={
    ...
    txtimport_handler = ConversationHandler(
        entry_points=[CommandHandler("txtimport", create_force_sub_handler(txtimport_start))],
        states={
            TXT_UPLOAD: [

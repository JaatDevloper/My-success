...
# Bot token - Using provided token for testing
BOT_TOKEN = "7631768276:AAFWUidQIXRnw-CLxaNAPvc0YGef6u1iZWQ"
    ...
    try:
        ...
        with open(pdf_path, 'rb') as file:
            ...
            for page_num in range(len(reader.pages)):
                ...
                logger.info(f"Direct extraction page {page_num+1}: {len(page_text)} characters")
        
        # Log direct extraction results
        logger.info(f"Direct PDF extraction complete. Total text length: {len(direct_text)}")
        logger.info(f"Direct extraction sample: {direct_text[:300]}...")
        
        # Try OCR (especially for Hindi or scanned documents)
        ...
        for i, img in enumerate(images):
            ...
            if hindi_support:
                ...
                try:
                    # Try with both Hindi and English support
                    page_text = pytesseract.image_to_string(
                        img, 
                        lang='hin+eng',
                        config='--psm 6'  # Assume a single uniform block of text
                    )
                    logger.info(f"OCR with Hindi+English successful for page {i+1}")
                except Exception as ocr_error:
                    logger.error(f"Hindi OCR error: {ocr_error}, falling back to English")
                    try:
                        ...
                        logger.info(f"OCR with English successful for page {i+1}")
                    except Exception as eng_error:
                        logger.error(f"English OCR error: {eng_error}")
                        page_text = ""
            else:
                try:
                    page_text = pytesseract.image_to_string(
                    ...
                    logger.info(f"English-only OCR successful for page {i+1}")
                except Exception as ocr_error:
                    logger.error(f"OCR error: {ocr_error}")
                    page_text = ""
            
            ocr_text += page_text + "\n\n"
            logger.info(f"OCR page {i+1} results: {len(page_text)} characters")
            if page_text:
                logger.info(f"OCR page {i+1} sample: {page_text[:100]}...")
        
        # Log OCR results
        logger.info(f"OCR complete. Total OCR text length: {len(ocr_text)}")
        if ocr_text:
            logger.info(f"OCR text sample: {ocr_text[:300]}...")
        
        # Choose the better result (usually OCR for Hindi, direct extraction for English)
        if len(ocr_text) > len(direct_text) * 0.8 or hindi_support:  # Prefer OCR for Hindi even if slightly shorter
            text = ocr_text
            logger.info("Using OCR text (better for Hindi/scanned content)")
        else:
            text = direct_text
            logger.info("Using direct extraction text (better for native PDF text)")
        
        # Special handling for PDFs with questions - try to identify question structure
        # This helps with PDFs that have questions but in unusual formats
        if "?" in text or re.search(r'\b(what|which|who|when|where|how|why)\b', text, re.IGNORECASE):
            logger.info("Question marks or question words detected in text")
        else:
            logger.info("No obvious question indicators found")
            
        # Try to find common option indicators
        option_indicators = ['a)', 'b)', 'c)', 'd)', 'A)', 'B)', 'C)', 'D)', '1)', '2)', '3)', '4)']
        found_indicators = [ind for ind in option_indicators if ind in text]
        if found_indicators:
            logger.info(f"Found option indicators: {', '.join(found_indicators)}")
        else:
            logger.info("No common option indicators found")
...
async def receive_pdf(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Check for text message - support for direct question input
    if update.message.text and not update.message.text.startswith('/'):
        # User is sending question text directly
        try:
            logger.info("Received direct question text input")
            direct_text = update.message.text
            
            # Create a basic question structure if the text contains a question mark
            if '?' in direct_text:
                question_parts = direct_text.split('?', 1)
                question_text = question_parts[0] + '?'
                
                # Try to extract options from the text after the question mark
                options_text = question_parts[1] if len(question_parts) > 1 else ""
                ...
                # Store for review
                context.user_data["pdf_questions"] = [("manual_1", question)]
                
                # Show preview
                preview_text = "✅ Created a question from your text input:\n\n"
                preview_text += f"Question: {question['question'][:50]}...\n"
                ...
                keyboard = [
                    [InlineKeyboardButton("✅ Import Question", callback_data="pdf_import_all")],
                ...
                await update.message.reply_text(
                    "Your text doesn't contain a question mark (?). Please either:\n"
                    "1. Send a PDF file with questions, or\n"
                    "2. Send a text message with a question mark, like 'What is the capital of France?'"
                )
                return PDF_RECEIVING
                
        except Exception as e:
            logger.error(f"Error processing direct text input: {e}")
            await update.message.reply_text(
                "Sorry, I couldn't process your text input. Please try sending a properly formatted PDF file instead."
            )
            return PDF_RECEIVING
    ...
    if not update.message.document or not update.message.document.file_name.lower().endswith('.pdf'):
        await update.message.reply_text(
            ...
            "Or you can send your questions directly as text in this format:\n"
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            if not extracted_text or len(extracted_text) < 50:
                # If extraction failed, offer to create a question manually
                await update.message.reply_text(
                    "❌ Could not extract sufficient text from the PDF.\n\n"
                    "As an alternative, you can send your questions directly as text in this format:\n"
                    "'What is the capital of France? Paris, London, Berlin, Rome'"
                )
                return PDF_RECEIVING
            ...
            if not parsed_questions:
                # If no questions parsed, create a "raw text" question as fallback
                raw_question = {
                    "question": "Question extracted from PDF",
                    "options": [
                        "Option 1", 
                        "Option 2", 
                        "Option 3", 
                    ...
                    "pdf_imported": True,
                    "needs_review": True,
                    "raw_text": extracted_text[:1000]  # Save first 1000 chars of raw text
                }
                
                context.user_data["pdf_questions"] = [("raw_1", raw_question)]
                
                await progress_message.edit_text(
                    "⚠️ No structured questions could be detected in the PDF.\n\n"
                    "I've extracted the raw text that you can review manually. You'll need to "
                    "format this into proper questions later.\n\n"
                    "Would you like to import this raw text for later editing?"
                )
                
                # Create import confirmation keyboard
                keyboard = [
                    [InlineKeyboardButton("✅ Import Raw Text", callback_data="pdf_import_all")],
                    [InlineKeyboardButton("❌ Cancel Import", callback_data="pdf_cancel")]
                ]
                reply_markup = InlineKeyboardMarkup(keyboard)
                
                await progress_message.edit_reply_markup(reply_markup=reply_markup)
...
def main() -> None:
    """Start the bot."""
    try:
        # Create the Application with latest python-telegram-bot syntax
        ...
        pdf_import_handler = ConversationHandler(
            ...
            states={
                PDF_RECEIVING: [
                    ...
                    MessageHandler(filters.TEXT & ~filters.COMMAND, receive_pdf),  # Allow text input
        ...
        # Custom ID message handler for poll - using newer version syntax
        application.add_handler(MessageHandler(
        ...
        logger.info("Bot handlers set up successfully, starting polling...")
        
        # Start the Bot with proper error handling for newer versions

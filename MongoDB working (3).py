...
CHANNEL_USERNAME = "NegativeMarkingTestbot"
CHANNEL_URL = f"https://t.me/{CHANNEL_USERNAME}"

# Owner ID to bypass force subscription
OWNER_ID = 7656415064

# Robot image URL (instead of base64 encoding)
ROBOT_IMAGE_URL = "https://i.ibb.co/HPFckph/robot-image.png"
...
async def check_subscription(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    try:
        # Use getChatMember API to check subscription status
        chat_member = await context.bot.get_chat_member(f"@{CHANNEL_USERNAME}", user_id)
        
        # Valid member statuses
        if chat_member.status in ['member', 'creator', 'administrator']:
            logger.info(f"User {user_id} is subscribed to channel with status: {chat_member.status}")
            return True
        else:
            logger.info(f"User {user_id} is NOT subscribed to channel (status: {chat_member.status})")
            return False
            
    except Exception as e:
        logger.error(f"Error checking subscription: {e}")
        # Must return False to enforce subscription check
        return False
...
async def force_subscription_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # NOTE: Initial message MUST match screenshot exactly - one button only
    # The "Check Again" button will be added only after the user clicks "check_subscription"
    
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    try:
        # Use external image URL instead of base64 (more reliable)
        # Use exact caption from screenshot: "Join our channel to use the bot"
        await update.effective_message.reply_photo(
            photo=ROBOT_IMAGE_URL,
            caption="Join our channel to use the bot",
            reply_markup=reply_markup
        )
    except Exception as e:
        logger.error(f"Error sending subscription message: {e}")
        # Fallback method if image sending fails
        await update.effective_message.reply_text(
            "Join our channel to use the bot",
            reply_markup=reply_markup
        )

# Wrapper function to check subscription before executing commands
...
async def handle_subscription_check_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Explicitly get fresh chat member data to check subscription
    user_id = update.effective_user.id
    
    # Check if user is owner (bypass)
    if user_id == OWNER_ID:
        await query.edit_message_text(
            "✅ Owner access granted! You can use all bot features.",
            parse_mode=ParseMode.HTML
        )
        return
    
    try:
        # Force fresh API call to get current status
        chat_member = await context.bot.get_chat_member(f"@{CHANNEL_USERNAME}", user_id)
        
        # If user has subscribed
        if chat_member.status in ['member', 'creator', 'administrator']:
            logger.info(f"User {user_id} verified as channel subscriber with status: {chat_member.status}")
            
            # Show success message
            await query.edit_message_text(
                "✅ Thank you for subscribing to our channel! You can now use the bot.",
                parse_mode=ParseMode.HTML
            )
            return
            
    except Exception as e:
        logger.error(f"Error rechecking subscription: {e}")
    
    # If we get here, user is not subscribed or error occurred
    # Show the original subscription message again, but now with both buttons
    try:
        # Create buttons - Join Now first, then Check Again
        keyboard = [
            [InlineKeyboardButton("Join Now...", url=CHANNEL_URL)],
        ...
        if query.message.photo:
            # If it's a photo message, we need to keep the photo and just update caption
            await query.edit_message_caption(
                caption="Join our channel to use the bot",
                reply_markup=reply_markup
            )
        else:
            # If it's a text message, we need to send a new message with the photo
            try:
                ...
                logger.error(f"Error sending photo message: {photo_error}")
                # Fall back to just updating the text
                await query.edit_message_text(
                    "Join our channel to use the bot",
                    reply_markup=reply_markup
                )
    except Exception as e:
        logger.error(f"Error updating subscription check message: {e}")

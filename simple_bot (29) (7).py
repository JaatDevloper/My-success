        ...
        if file_valid:
            try:
                ...
                try:
                    ...
                    # Try to import the HTML generator module
                    try:
                        spec = importlib.util.spec_from_file_location("html_results_generator", "html_results_generator.py")
                        if spec is not None and spec.loader is not None:
                            html_generator = importlib.util.module_from_spec(spec)
                            spec.loader.exec_module(html_generator)
                            logger.info("Successfully imported HTML results generator")
                        else:
                            logger.warning("Failed to find HTML results generator module")
                            return True  # PDF was still sent successfully
                    except ImportError as e:
                        logger.warning(f"HTML generator module not available: {e}")
                        return True  # PDF was still sent successfully
                    
                    # Get quiz questions
                    quiz_questions = [q for q in load_questions() if str(q.get("id", "")).startswith(str(quiz_id))]
                    ...
                    # Process participant data for time displays
                    ...
                    if html_file and os.path.exists(html_file):
                        ...
                        logger.error("HTML file generation failed or file not found")
                except Exception as html_error:
                    logger.error(f"Error generating HTML report: {html_error}")
                    # Don't notify user about HTML error since PDF was sent successfully

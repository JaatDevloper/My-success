...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "7631768276:AAEHxHv8uyqs5r7qqlFUkTVEJnHOH89DyfI")
...
def extract_questions_from_txt(lines):
    ...
    Improved to detect proper questions with question marks or specific formats
    """
    questions = []
    ...
    # Improved patterns to identify real questions
    # Match patterns like "Q1.", "Question 1:", "1.", "1)", etc. followed by actual question text
    # Added Hindi question patterns (प्रश्न) and improved Unicode support
    question_pattern1 = re.compile(r'^(?:Q\.?|Question|प्रश्न)\s?(\d+)[\.:\)]?\s+(.*\?.*)', re.IGNORECASE | re.UNICODE)
    question_pattern2 = re.compile(r'^(\d+)[\.:\)]\s+(.*\?.*)', re.IGNORECASE | re.UNICODE)
    
    # Special pattern for emoji-prefixed questions like in the sample file (✳️, 👉, etc.)
    question_pattern_emoji = re.compile(r'^[✳️👉✨🔮♦️🔸🔹🔺☑️◉🔱✴️](.*?\?)', re.IGNORECASE | re.UNICODE)
    
    # More flexible pattern for lines with question marks - will match ANY line with a question mark
    # This ensures we catch ALL possible questions, even without prefixes or special formatting
    question_pattern3 = re.compile(r'^(.*?\?)', re.IGNORECASE | re.UNICODE)
    # No more backup patterns without question marks - we want to be strict about this
    
    # Pattern for options - match A), A., (A), etc. and also fancy Unicode formats like 【𝗔】
    # Extended to match more formats including square brackets, various decorative styles
    option_pattern = re.compile(r'^\s*(?:\(?([A-D])[\.:\)\-]?\s*|\[([A-D])\]\s*|【([𝗔𝗕𝗖𝗗])】\s*|\[([𝗔𝗕𝗖𝗗])\]\s*)(.*)', re.IGNORECASE | re.UNICODE)
    
    # Special pattern for options with checkmark at the end
    # Extended to match all the same formats as the regular option pattern
    option_with_checkmark = re.compile(r'^.*?(?:\(?([A-D])[\.:\)\-]?\s*|\[([A-D])\]\s*|【([𝗔𝗕𝗖𝗗])】\s*|\[([𝗔𝗕𝗖𝗗])\]\s*).*[✅✓✔].*$', re.IGNORECASE | re.UNICODE)
    
    # Enhanced patterns for answers in English and Hindi
    answer_pattern1 = re.compile(r'^\s*(?:Ans(?:wer)?|उत्तर|सही उत्तर|जवाब)[\.:]?\s*([A-D])', re.IGNORECASE)
    answer_pattern2 = re.compile(r'^\s*\(?([A-D])\)?\s*(.*?)\s*[\*✓✔✅]')
    answer_pattern3 = re.compile(r'^\s*(?:Correct Answer|सही उत्तर|उत्तर:)[\.:]?\s*([A-D])', re.IGNORECASE)
    answer_pattern4 = re.compile(r'^\s*(?:उत्तर|सही उत्तर)\s*\(?([A-D])\)?.*?', re.IGNORECASE)
    answer_pattern5 = re.compile(r'^\s*उत्तर\s+([A-D])\s+', re.IGNORECASE)
    ...
    # Additional Hindi patterns for identifying questions
    hindi_question_pattern = re.compile(r'.*\?.*', re.UNICODE)  # Simple check for question mark
    # Enhanced Hindi option pattern with more formats (similar to English option pattern)
    hindi_option_pattern = re.compile(r'^\s*(?:\(?([१२३४]|[क-ञ])[\.:\)\-]?\s*|\[([१२३४]|[क-ञ])\]\s*)(.*)', re.UNICODE)
    
    # Hindi to English option letter mapping
    hindi_to_english = {
        'क': 'A', 'ख': 'B', 'ग': 'C', 'घ': 'D',
        '१': 'A', '२': 'B', '३': 'C', '४': 'D'
    }

...
def get_quiz_pdf_path(quiz_id, quiz_title):
    ...
    try:
        ...
        fallback_path = os.path.join(os.getcwd(), f"quiz_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf")
        logger.warning(f"Using fallback PDF path: {fallback_path}")

...
async def create_type_received(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🔗 Share Quiz", switch_inline_query=f"share_{safe_quiz_id}")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def inline_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Check if it's a quiz share request
    if query.startswith("quiz_") or query.startswith("share_"):
        ...
        logger.info(f"Inline query for {action} with quiz ID: {quiz_id}")
        ...
        if action == "quiz" or action == "share":
            ...
            keyboard = [
                [InlineKeyboardButton("Start Quiz", callback_data=f"start_quiz_{quiz_id}")]
            ]
...
def main() -> None:
    ...
    # Add inline query handler for quiz sharing

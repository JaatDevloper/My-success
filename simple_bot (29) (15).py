        ...
        if file_valid:
            try:
                ...
                try:
                    ...
                    # Define a direct HTML generator function instead of importing the module
                    def generate_html_report_direct(quiz_id, title=None, questions_data=None, leaderboard=None, quiz_metadata=None):
                        """
                        Generate an HTML quiz results report directly within the bot
                            ...
                            if isinstance(questions_data, list):
                                for q in questions_data:
                                    if isinstance(q, dict):
                                        # Create a clean, simple copy with only essential fields
                                        cleaned_q = {
                                            "id": q.get("id", "unknown"),
                                        ...
                                        sanitized_questions.append(cleaned_q)
                                    else:
                                        logger.warning(f"Skipping non-dictionary question: {type(q)}")
                            else:
                                logger.warning(f"questions_data is not a list: {type(questions_data)}")

...
async def check_subscription(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    try:
        # Make sure channel username has @ prefix for proper API call
        channel = CHANNEL_USERNAME
        if not channel.startswith('@'):
            channel = f"@{channel}"
        
        logger.info(f"Checking if user {user_id} is subscribed to channel {channel}")
        ...
        logger.info(f"User {user_id} has status {chat_member.status} in channel, is_member={is_member}")
        return is_member
...
async def handle_subscription_check_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Make sure the channel username has @ prefix
    channel = CHANNEL_USERNAME
    if not channel.startswith('@'):
        channel = f"@{channel}"
    
    try:
        # Force a fresh API call with explicit parameters to check current status
        logger.info(f"Making explicit API call to check if user {user_id} is subscribed to channel {channel}")
        chat_member = await context.bot.get_chat_member(chat_id=channel, user_id=user_id)
        ...
        if status in ['member', 'creator', 'administrator']:
            logger.info(f"User {user_id} is confirmed as channel subscriber with status: {status}")

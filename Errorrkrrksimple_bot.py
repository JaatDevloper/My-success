...
async def handle_quiz_end_with_pdf(update, context, quiz_id, user_id, user_name, 
    ...
    try:
        ...
        if file_valid:
            ...
            # If PDF generation failed, notify the user with a more helpful message
            logger.error("PDF file validation failed")
            await update.message.reply_text("📋 Creating a simplified text summary instead of PDF...\n\n" + 
                                           f"Quiz: {quiz_id}\n" +
                                           f"Score: {adjusted_score} ({correct_answers} correct, {wrong_answers} wrong)\n" +
                                           f"Penalty: {penalty}\n\n" +
                                           "You can try the /htmlreport command to view detailed results.")
            return False
    except Exception as e:

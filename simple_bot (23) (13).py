        ...
        if REPORTLAB_AVAILABLE:
            ...
            if quiz_questions:
                ...
                if result:
                    ...
                    with open(pdf_path, 'rb') as pdf_file:
                        await context.bot.send_document(
                            chat_id=chat_id,
                            document=pdf_file,
                            filename=f"Quiz_{quiz_id or 'temp'}_{quiz_title}.pdf",
                            caption=f"📝 Questions and answers for Quiz: {quiz_title}"
                        )
                    ...
                    await context.bot.edit_message_text(
                        chat_id=chat_id,
                        message_id=message.message_id,
                        text="✅ Quiz PDF generated and sent successfully!"
                    )
                else:
                    await context.bot.edit_message_text(
                        chat_id=chat_id,
                        message_id=message.message_id,
                        text="❌ Failed to generate quiz PDF."
                    )
            else:
                ...
                await context.bot.edit_message_text(
                    chat_id=chat_id,
                    message_id=message.message_id,
                    text="❓ No valid questions found for PDF generation."
                )
    except Exception as e:

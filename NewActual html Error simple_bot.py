    ...
    for i, cat in enumerate(top_categories):
        ...
        html_parts.append(f"""<div class="category-item">
            ...
            <div class="category-details">
                ...
                <div class="category-stats">{cat["total"]} questions (Avg: {score_formatted}%)</div>
            </div>
        </div>""")
        ...
        html_parts.append(f"""<div class="quiz-item">
            <div class="quiz-info">
                ...
                <div>
                    ...
                    <div style="font-size: 12px; color: #666;">{quiz['date']}</div>
                </div>
            </div>
            <div class="quiz-result {result_class}">{score_formatted}%</div>
        </div>""")
    ...
    if is_premium:
        ...
        <div class="section premium-user-section">
            <div class="premium-badge">
                <span class="premium-star">★</span> PREMIUM USER <span class="premium-star">★</span>
            </div>
            <div class="premium-benefits">
                <p>Thank you for being a premium member! Enjoy all exclusive features.</p>
            </div>
        </div>
        """
    ...
    return """
        <div class="section">
            ...
            <div class="premium-features">
                <h3 class="premium-title">Upgrade to Premium for:</h3>
                ...
                <div class="premium-cta">
                    <a href="https://t.me/JaatSupreme" class="premium-button">Contact @JaatSupreme to Upgrade!</a>
                </div>
            </div>
        </div>
        """
        ...
        <div class="question-item">
            ...
            <div class="answers-container">
                <div class="correct-answer">
                    ...
                    <span class="answer-text">{correct_answer}</span>
                    <span class="check-mark">✓</span>
                </div>
                <div class="user-answer {'' if is_correct else 'incorrect'}">
                    <span class="answer-label">Your answer:</span> 
                    <span class="answer-text">{user_answer}</span>
...
async def handle_download_profile_html_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    keyboard = [
        [InlineKeyboardButton("📋 Show HTML Code", callback_data="html_show_code")],
        [InlineKeyboardButton("📥 Download HTML File", callback_data="html_download_file")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    try:
        ...
        try:
            ...
            performance_html = (
                ...
                f"• <b>Total Quizzes:</b> {total_quizzes}\n"
                f"• <b>Questions Answered:</b> {total_questions}\n"
                f"• <b>Correct Answers:</b> {total_correct} ({accuracy:.1f}%)\n"
                f"• <b>Incorrect Answers:</b> {total_incorrect}\n"
                f"• <b>Average Score:</b> {avg_score:.1f}%\n\n"
            )
            ...
            time_period_html = (
                f"<b>📅 ACTIVITY TRENDS</b>\n"
                f"• <b>Today:</b> {daily_stats.get('quizzes', 0)} quizzes\n"
                f"• <b>This Week:</b> {weekly_stats.get('quizzes', 0)} quizzes\n"
                f"• <b>This Month:</b> {monthly_stats.get('quizzes', 0)} quizzes\n"
                f"• <b>This Year:</b> {yearly_stats.get('quizzes', 0)} quizzes\n\n"
            )
            ...
            if categories:
                ...
                for i, cat in enumerate(top_cats):
                    if i < len(medals):
                        ...
                        category_html += f"{medal} <b>{cat_name}:</b> {cat_total} questions (Avg: {cat_score:.1f}%)\n"
                category_html += "\n"
            else:
                category_html += "No category data available yet.\n\n"
            ...
            if recent_quizzes:
                for quiz in recent_quizzes[:5]:  # Show only the 5 most recent
                    ...
                    recent_activity_html += f"{emoji} <b>Quiz {quiz_id}:</b> {quiz_score:.1f}% on {quiz_date}\n"
            else:
    ...
    <style>
        ...
        * {{
        ...
        body {{
            ...
            background-color: {('#121212' if is_premium else '#f8f9fa')};
            padding: 20px;
            position: relative;
        }}
        ...
        .watermark {{
            ...
            font-size: 16px;
            opacity: 0.7;
            color: {('#ffd700' if is_premium else '#4a6ee0')};
            z-index: 1000;
            text-shadow: 0 0 5px rgba(0,0,0,0.3);
            transform: rotate(-5deg);
            animation: pulse-watermark 3s infinite;
        }}
        
        @keyframes pulse-watermark {{
            0% {{ opacity: 0.5; transform: rotate(-5deg) scale(1); }}
            50% {{ opacity: 0.8; transform: rotate(-5deg) scale(1.1); }}
            100% {{ opacity: 0.5; transform: rotate(-5deg) scale(1); }}
        }}
        
        .profile-container {{
            max-width: 800px;
            margin: 0 auto;
            background-color: {('#1a1a1a' if is_premium else '#fff')};
            border-radius: 12px;
            box-shadow: {('0 5px 20px rgba(255, 215, 0, 0.15)' if is_premium else '0 5px 15px rgba(0, 0, 0, 0.1)')};
            overflow: hidden;
        ...
        .question-item {
            background: {('#262626' if is_premium else '#fff')};
            border-radius: 8px;
            padding: 15px;
            margin-bottom: 15px;
            box-shadow: 0 3px 10px rgba(0, 0, 0, 0.05);
...
<body>
    ...
    <!-- Owner Watermark -->
    <div class="watermark">💔🗿𝘐𝘕𝘚𝘈𝘕𝘌</div>
</body>
</html>
"""

...
def ensure_pdf_directory():
    ...
    # List of potential directories to try, in order of preference for Render
    potential_dirs = [
        # Render-specific paths (highest priority)
        "/tmp/pdf_results",  # Render primary tmp directory
        "/tmp",              # Render fallback directory
        
        # Standard locations (medium priority)
        os.path.join(os.getcwd(), "pdf_results"),  # Standard project location
        os.path.join(os.getcwd(), "tmp", "pdf_results"),  # Nested project tmp
        
        # Last resort options (lowest priority)
        os.path.join(os.path.expanduser("~"), "pdf_results"),  # Home directory
        os.path.join(os.getcwd(), "temp"),  # Alternative temp dir
        os.path.join(os.getcwd()),  # Last resort: current directory
    ]
    
    # Try each potential directory location
    for dir_path in potential_dirs:
        try:
            ...
            PDF_RESULTS_DIR = dir_path
            logger.info(f"PDF directory verified and writable: {PDF_RESULTS_DIR}")
            return True
        except Exception as e:
            logger.warning(f"Cannot use directory {dir_path}: {e}")
    
    # If all attempts fail, set to current directory and return False to indicate issues
    PDF_RESULTS_DIR = "."
    logger.error("All PDF directory attempts failed, using current directory as fallback")
    return False
...
# PDF generation functions for quiz questions
def create_quiz_pdf_styles():
    """Create paragraph styles for the quiz PDF"""
...
def get_quiz_pdf_path(quiz_id, quiz_title):
    """
    Get file path for a quiz PDF with robust error handling
    Always returns a valid string path, never None or boolean
    Enhanced for Render deployment compatibility
    """
    try:
        # Ensure we have string versions of the parameters
        quiz_id_str = str(quiz_id) if quiz_id is not None else "unknown"
        quiz_title_str = str(quiz_title) if quiz_title is not None else "Quiz"
        
        # Get a valid PDF directory - ensure_pdf_directory returns True/False
        # but also sets the global PDF_RESULTS_DIR to a valid path
        ensure_pdf_directory()
        
        # Get our directory from the global variable that was set
        pdf_dir = PDF_RESULTS_DIR
        
        # Extra check to make sure we have a valid string path
        if not isinstance(pdf_dir, str) or pdf_dir is False or pdf_dir is True or pdf_dir is None:
            # Force fallback to a valid directory
            pdf_dir = "/tmp" if os.path.exists("/tmp") and os.access("/tmp", os.W_OK) else os.getcwd()
            logger.warning(f"Invalid PDF directory type: {type(pdf_dir)}, using fallback: {pdf_dir}")
        
        # Verify the directory exists
        if not os.path.exists(pdf_dir):
            try:
                os.makedirs(pdf_dir, exist_ok=True)
                logger.info(f"Created PDF directory that didn't exist: {pdf_dir}")
            except Exception as dir_error:
                # If we can't create the directory, fall back to current directory
                pdf_dir = os.getcwd()
                logger.warning(f"Couldn't create directory, using current dir: {pdf_dir}")
        
        # Create a safe title string
        try:
            safe_title = "".join(c for c in quiz_title_str if c.isalnum() or c in " _-").rstrip()
            # If title is empty after cleaning, use a default
            if not safe_title:
                safe_title = "quiz"
                logger.warning(f"Empty title after cleaning, using default: {safe_title}")
        except Exception as title_error:
            safe_title = "quiz"
            logger.error(f"Error cleaning title: {title_error}, using default: {safe_title}")
            
        # Generate timestamp
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        
        # Create filename
        filename = f"quiz_{quiz_id_str}_{safe_title}_{timestamp}.pdf"
        
        # Join path components and verify
        ...
        try:
            ...
            os.remove(path)
            logger.info(f"Verified path is writable: {path}")
        except Exception as write_error:
            # If not writable, try Render-specific alternatives
            logger.warning(f"Path not writable: {path}, error: {write_error}")
            
            # Try Render-compatible directories
            render_paths = [
                "/tmp",                    # Primary Render temp directory
                "/opt/render/project/tmp",  # Alternative Render path
                tempfile.gettempdir(),     # System default temp directory
                os.path.join(os.path.expanduser("~"), "tmp")  # Home directory tmp
            ]
            
            for render_dir in render_paths:
                if os.path.exists(render_dir) and os.access(render_dir, os.W_OK):
                    try:
                        render_path = os.path.join(render_dir, filename)
                        # Verify this path works by writing a test file
                        with open(render_path, 'w') as test_file:
                            test_file.write("test")
                        os.remove(render_path)
                        path = render_path
                        logger.info(f"Using Render-compatible directory: {render_dir}, path: {path}")
                        break
                    except Exception:
                        # Just continue to the next potential directory
                        continue
            else:
                # If all Render paths fail, use last resort with a unique timestamp
                path = os.path.join(os.getcwd(), f"quiz_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S%f')}.pdf")
                logger.info(f"Using last resort path: {path}")
        
        # Log the final path
        logger.info(f"Generated PDF path: {path} (type: {type(path)})")
        
        # Return the path as a string
        return str(path)
        
    except Exception as e:
        # Generate an absolute fallback path in case of any error
        logger.error(f"Error generating PDF path: {e}")
        
        # Try multiple Render-compatible paths
        render_fallback_paths = [
            "/tmp",
            "/opt/render/project/tmp",
            tempfile.gettempdir(),
            os.getcwd()
        ]
        
        # Find the first writable directory
        for render_dir in render_fallback_paths:
            if os.path.exists(render_dir) and os.access(render_dir, os.W_OK):
                unique_filename = f"quiz_emergency_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S%f')}.pdf"
                fallback_path = os.path.join(render_dir, unique_filename)
                try:
                    # Test if we can write to this path
                    with open(fallback_path, 'w') as test_file:
                        test_file.write("test")
                    os.remove(fallback_path)
                    logger.warning(f"Using Render fallback PDF path: {fallback_path}")
                    return str(fallback_path)
                except Exception:
                    continue
        
        # Ultimate fallback - current directory with timestamp
        fallback_path = os.path.join(os.getcwd(), f"quiz_final_fallback_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S%f')}.pdf")
        logger.warning(f"Using ultimate fallback PDF path: {fallback_path}")
        return str(fallback_path)

def generate_quiz_pdf(quiz_id, quiz_title, questions_data, file_path=None):
    """
    Generate a PDF with all quiz questions and mark correct answers
    
    Args:
        quiz_id (str): ID of the quiz
        quiz_title (str): Title of the quiz
        questions_data (list): List of question dictionaries
        file_path (str, optional): Path to save the PDF. If None, returns BytesIO
    """
    if not REPORTLAB_AVAILABLE:
    ...
    try:
        # Create styles
        styles = create_quiz_pdf_styles()
        if not styles:
            logger.error("Failed to create PDF styles")
            return None
        
        # FIXED: Better handling for different file_path types
        # Ensure file_path is a string or path-like object
        if file_path is None or file_path is False:
            # Use BytesIO for None or False
            file_path = None
            logger.info("Using BytesIO for PDF generation (no file path provided)")
        elif file_path is True or not isinstance(file_path, (str, bytes, os.PathLike)):
            # Generate a default path for True or any non-path object
            file_path = get_quiz_pdf_path(quiz_id, quiz_title)
            logger.warning(f"Invalid file_path type: {type(file_path)}, using default path: {file_path}")
        
        # Create PDF buffer or file
        buffer = BytesIO() if file_path is None else None
        
        # Extra safety check to prevent boolean from reaching SimpleDocTemplate
        if isinstance(file_path, bool):
            logger.error(f"Boolean still in file_path after previous checks: {file_path}")
            # Force a default path
            file_path = get_quiz_pdf_path(str(quiz_id), str(quiz_title))
            logger.warning(f"Emergency fix - using absolute default path: {file_path}")
            
        # One final check to ensure we don't pass a boolean to SimpleDocTemplate
        target = file_path if (file_path is not None and not isinstance(file_path, bool)) else buffer
        
        # Debug output
        logger.info(f"Target type: {type(target)}, Value: {target}")
        
        # Safely create PDF document with verified target
        doc = SimpleDocTemplate(
            target,
            pagesize=A4,
            title=f"Quiz {quiz_id} - {quiz_title}",
            author="Telegram Quiz Bot"
        )
        ...
        title_text = f"Quiz {quiz_id}: {quiz_title} ({current_date})"
        ...
        for i, question in enumerate(questions_data):
            ...
            question_num = i + 1
            question_para = Paragraph(f"Q {question_num}. {q_text}", styles['question'])
            ...
            for j, option_text in enumerate(options):
                ...
                option_letter = option_letters[j] if j < len(option_letters) else str(j+1)
                
                # Choose style based on whether this is the correct answer
                if j == correct_index:
                    # Correct answer - blue with checkmark
                    option_full = f"({option_letter}) {option_text} ✅"
                    option_para = Paragraph(option_full, styles['correct'])
                else:
                    # Regular option - black
                    option_full = f"({option_letter}) {option_text}"
                    option_para = Paragraph(option_full, styles['option'])
        ...
        try:
            doc.build(elements)
            logger.info(f"PDF for quiz {quiz_id} generated successfully")
            
            # If using buffer, get the PDF data
            if buffer:
                buffer.seek(0)
                return buffer
            else:
...
async def send_quiz_pdf(context, chat_id, quiz_id, quiz_title, questions):
    """
    Generate and send a PDF with quiz questions and answers
    
    Args:
        context: Bot context
        chat_id: Chat ID to send PDF to
    ...
    if not REPORTLAB_AVAILABLE:
        await context.bot.send_message(
            chat_id=chat_id,
            text="Sorry, PDF generation is not available."
        )
        return
    
    try:
        # Send "generating PDF" message
        pdf_message = await context.bot.send_message(
            chat_id=chat_id,
            text="📊 Generating quiz PDF with answers, please wait..."
        )
        
        # Generate PDF with safe path handling
        try:
            # Make sure quiz_id and quiz_title are strings
            quiz_id_str = str(quiz_id) if quiz_id is not None else "unknown"
            quiz_title_str = str(quiz_title) if quiz_title is not None else "Quiz"
            
            # Get a safe file path
            pdf_path = get_quiz_pdf_path(quiz_id_str, quiz_title_str)
            logger.info(f"PDF path type: {type(pdf_path)}, Value: {pdf_path}")
            
            # Double check that pdf_path is a valid string
            if not isinstance(pdf_path, (str, bytes, os.PathLike)):
                logger.error(f"Invalid PDF path type: {type(pdf_path)}")
                pdf_path = os.path.join(os.getcwd(), "pdf_results", f"quiz_{quiz_id_str}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf")
                logger.warning(f"Using emergency fallback path: {pdf_path}")
                
            # Generate the PDF with validated path
            result = generate_quiz_pdf(quiz_id_str, quiz_title_str, questions, pdf_path)
        except Exception as path_error:
            logger.error(f"Error preparing PDF path: {path_error}")
            await context.bot.edit_message_text(
                chat_id=chat_id,
                message_id=pdf_message.message_id,
                text=f"❌ Error preparing PDF path: {str(path_error)}"
            )
            return
        ...
        with open(pdf_path, 'rb') as pdf_file:
            await context.bot.send_document(
                chat_id=chat_id,
                document=pdf_file,
                filename=f"Quiz_{quiz_id}_{quiz_title}.pdf",
                caption=f"📝 Questions and answers for Quiz: {quiz_title}"
            )
        ...
        logger.info(f"Quiz PDF for {quiz_id} sent to chat {chat_id}")
        
    except Exception as e:
        logger.error(f"Error sending quiz PDF: {e}")
        await context.bot.send_message(
            chat_id=chat_id,
...
async def end_quiz(context, chat_id):
    ...
    # Generate and send PDF with all questions and answers - directly extract questions data
    try:
        # Only proceed if ReportLab is available
        if REPORTLAB_AVAILABLE:
            message = await context.bot.send_message(
                chat_id=chat_id,
                text="📊 Generating quiz questions PDF with answers, please wait..."
            )
            
            # Create a title for the quiz
            quiz_title = f"Quiz {quiz_id}" if quiz_id else "Recent Quiz"
            
            # Try to get a better title if category is available
            if quiz_id and quiz_id in load_questions():
                first_q = load_questions()[quiz_id][0] if load_questions()[quiz_id] else {}
                if isinstance(first_q, dict) and "category" in first_q and first_q["category"]:
                    quiz_title = f"{first_q['category']} Quiz #{quiz_id}"
            
            # ==================== IMPROVED QUESTION EXTRACTION ====================
            # Get the actual quiz questions from the current context storage
            # This ensures we're using the actual questions from the quiz
            quiz_questions = []
            
            # First, try to get from current quiz context
            if questions:
                for q in questions:
                    # Handle different question formats in polls
                    try:
                        # Handle normal quiz questions
                        if isinstance(q, dict):
                            if "question" in q:
                                ...
                                options_list = q.get("options", [])
                                answer_index = q.get("correct_option", 0)
                            else:
                                # Skip invalid questions
                                continue
                        ...
                        logger.error(f"Error formatting question for PDF: {format_error}")
                        continue
            
            # If no questions found in context, try to load from storage
            if not quiz_questions and quiz_id:
                try:
                    all_questions = load_questions()
                    if quiz_id in all_questions:
                        loaded_questions = all_questions[quiz_id]
                        if isinstance(loaded_questions, list):
                            # Process each question from storage
                    ...
                    logger.error(f"Error loading questions for PDF from storage: {load_error}")
            
            # Generate PDF if we have questions
            if quiz_questions:
                # Log success with question count
                logger.info(f"Found {len(quiz_questions)} questions for PDF generation")
                
                # Create directory and path
                pdf_dir = ensure_pdf_directory()
                safe_title = "".join(c for c in quiz_title if c.isalnum() or c in " _-").rstrip()
                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                pdf_filename = f"quiz_{quiz_id or 'temp'}_{safe_title}_{timestamp}.pdf"
                pdf_path = os.path.join(pdf_dir, pdf_filename)
                
                # Generate the PDF with our robust formatted questions
                logger.info(f"Generating PDF at {pdf_path}")
                result = generate_quiz_pdf(quiz_id or "temp", quiz_title, quiz_questions, pdf_path)
                ...
                if result:
                    ...
                    try:
                        with open(pdf_path, 'rb') as pdf_file:
                            # Send document with improved caption
                            await context.bot.send_document(
                                chat_id=chat_id,
                                document=pdf_file,
                                filename=f"Quiz_{quiz_id or 'auto'}_{safe_title}.pdf",
                                caption=f"📝 Questions and correct answers (marked with ✅) for: {quiz_title}"
                            )
                        ...
                        await context.bot.edit_message_text(
                            chat_id=chat_id,
                            message_id=message.message_id,
                            text="✅ Quiz PDF with questions and answers generated successfully!"
                        )
                    except Exception as send_error:
                        logger.error(f"Error sending PDF: {send_error}")
                        # Try to recover with simplified approach
                        try:
                            await context.bot.send_message(
                                chat_id=chat_id,
                                text=f"❌ Error sending PDF: {str(send_error)}\n\nThe PDF was generated but could not be sent."
                            )
                        except:
                            pass
                else:
                    ...
                    await context.bot.edit_message_text(
                        chat_id=chat_id,
                        message_id=message.message_id,
                        text="❌ Failed to generate quiz PDF. Please try using /quizpdf command."
                    )
            else:
                # No valid questions found - update message instead of deleting
                logger.warning("No valid questions found for PDF generation")
                await context.bot.edit_message_text(
                    chat_id=chat_id,
                    message_id=message.message_id,
                    text="❓ No valid questions found for PDF generation. Try using /quizpdf command with quiz ID."
                )
        else:
            # Log if ReportLab is not available
            logger.warning("ReportLab not available for PDF generation")
    except Exception as e:
        logger.error(f"Error in quiz PDF generation: {e}")
        # Try to send error message
        try:
            await context.bot.send_message(
                chat_id=chat_id,
                text=f"❌ Error generating PDF: {str(e)}\n\nYou can try using the /quizpdf command instead."
            )
        except:
            pass
        # Don't let PDF errors break quiz functionality
...
async def pdf_info_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    info_text = (
        ...
        "📊 <b>PDF Generation</b>\n"
        "Use the /quizpdf command to generate a PDF with questions and answers.\n\n"
        "PDF generation features:\n"
        "1. Automatically generates at the end of each quiz.\n"
        "2. Manually generate PDF with /quizpdf or /quizpdf [QUIZ_ID].\n"
        "3. Shows all questions with options and marks correct answers with ✅.\n"
        "4. Works with quizzes in any language including Hindi.\n\n"
        "PDF Format Tips:\n"
...
# ====== /stop command ======
async def quizpdf_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Generate and send a PDF with quiz questions and answers for a specific quiz ID or recent quiz."""
    chat_id = update.effective_chat.id
    
    # Check if an ID was provided with the command
    quiz_id = None
    if context.args and len(context.args) > 0:
        # Try to get quiz ID from command arguments
        try:
            quiz_id = context.args[0]
            logger.info(f"PDF requested for quiz ID: {quiz_id}")
        except (ValueError, IndexError):
            quiz_id = None
    
    # If no ID provided or invalid ID, try to use most recent quiz data
    questions = []
    quiz_title = "Quiz Questions"
    
    # First try: Check if there's an active quiz in this chat
    if quiz_id is None:
        # Try to get questions from active quiz in this chat
        active_quiz = context.chat_data.get("quiz", {})
        if active_quiz and active_quiz.get("questions"):
            quiz_id = active_quiz.get("quiz_id", "recent")
            questions = active_quiz.get("questions", [])
            quiz_title = f"Active Quiz {quiz_id}" if quiz_id else "Current Active Quiz"
            logger.info(f"Using active quiz with {len(questions)} questions")
    
    # Second try: If no active quiz or quiz_id was provided, try to load from storage
        ...
        try:
            ...
            if quiz_id in all_questions:
                ...
                if isinstance(questions_data, list):
                    ...
                    # Try to get a title from category if available
                    if questions and "category" in questions[0]:
                        category = questions[0].get("category", "")
                        if category:
                            quiz_title = f"{category} Quiz #{quiz_id}"
                        else:
                            quiz_title = f"Quiz #{quiz_id}"
                    else:
                        quiz_title = f"Quiz #{quiz_id}"
                    logger.info(f"Loaded {len(questions)} questions from storage for quiz ID {quiz_id}")
                else:
                    # Single question case
                    questions = [questions_data]
                    quiz_title = f"Quiz #{quiz_id}"
                    logger.info(f"Loaded single question from storage for quiz ID {quiz_id}")
        except Exception as e:
            logger.error(f"Error loading questions for PDF: {e}")
    
    # If we still don't have questions, inform the user
    if not questions:
        await update.message.reply_text(
            "❌ No quiz questions found. Please specify a valid quiz ID (e.g., /quizpdf 123) "
            "or run this command during an active quiz."
        )
        return
    ...
    if formatted_questions:
        try:
            # Send status message
            status_message = await update.message.reply_text(
                "📊 Generating quiz PDF with questions and answers, please wait..."
            )
            
            # Create PDF path with comprehensive error handling
            # Start PDF generation process
            pdf_dir = ensure_pdf_directory()
            logger.info(f"PDF directory: {pdf_dir}")
            
            # Make sure we have a string title, not None or other type
            if not isinstance(quiz_title, str):
                quiz_title = str(quiz_title) if quiz_title is not None else "Quiz"
                logger.warning(f"Converting non-string quiz_title to string: {quiz_title}")
            
            # Make sure quiz_id is a string
            quiz_id_str = str(quiz_id) if quiz_id is not None else "manual"
            
            # Clean title for filename - handle any potential errors
            try:
                safe_title = "".join(c for c in quiz_title if c.isalnum() or c in " _-").rstrip()
                if not safe_title:
                    safe_title = "untitled"
                    logger.warning("Empty title after cleaning, using 'untitled'")
            except Exception as title_error:
                logger.error(f"Error cleaning title: {title_error}")
                safe_title = "error_title"
            
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            
            # Create filename and full path
            pdf_filename = f"quiz_{quiz_id_str}_{safe_title}_{timestamp}.pdf"
            
            # Create a guaranteed valid filesystem path
            try:
                # Primary path choice
                pdf_path = os.path.join(pdf_dir, pdf_filename)
                # Test path validity
                with open(pdf_path, 'w') as test_file:
                    test_file.write("test")
                os.remove(pdf_path)
            except Exception as path_error:
                logger.error(f"Error with primary PDF path: {path_error}")
                # Emergency fallback
                pdf_path = os.path.join(os.getcwd(), f"quiz_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf")
                logger.warning(f"Using emergency fallback path: {pdf_path}")
            
            logger.info(f"Final verified PDF path: {pdf_path} (type: {type(pdf_path)})")
            
            # Generate PDF with properly validated path
            result = generate_quiz_pdf(quiz_id_str, quiz_title, formatted_questions, pdf_path)
            ...
            if result:
                ...
                with open(pdf_path, 'rb') as pdf_file:
                    await context.bot.send_document(
                        chat_id=chat_id,
                        document=pdf_file,
                        filename=f"Quiz_{quiz_id or 'manual'}_{safe_title}.pdf",
                        caption=f"📝 Questions and answers for {quiz_title}"
                    )
                ...
                logger.info(f"PDF generated and sent for quiz ID {quiz_id or 'manual'}")
            else:
                # Handle failure
                await context.bot.edit_message_text(
                    chat_id=chat_id,
                ...
                logger.error(f"Failed to generate PDF for quiz ID {quiz_id or 'manual'}")
                
        except Exception as e:
            logger.error(f"Error in PDF command: {e}")
            await update.message.reply_text(f"❌ Error generating PDF: {str(e)}")
    else:
        await update.message.reply_text("❌ No valid questions found for PDF generation.")

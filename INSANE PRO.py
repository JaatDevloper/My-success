...
async def mongodb_status_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        header += f"📊 <i>Total quizzes stored: {count}</i>\n\n"
        ...
        if len(all_quizzes) > 0:
            ...
            for quiz in all_quizzes:
                ...
                quiz_entry += f"- <font color='#9966CC'>ID</font> ID: {quiz_id}\n"
                quiz_entry += f"- 📄 Type: {quiz_type}\n"
                quiz_entry += f"- 👥 Engagement: {engagement_count}\n"
                quiz_entry += f"- ✏️ Edit: /edit {quiz_id}\n"
                quiz_entry += f"{'_' * 35}\n\n"

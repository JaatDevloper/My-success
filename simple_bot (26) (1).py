...
async def poll_to_question(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.args:
        ...
        try:
            ...
            await update.message.reply_text("Invalid custom ID. It must be a number. Try again with /poll2q [number]")
            return
            
    # No custom ID
    await update.message.reply_text(
        "To convert a Telegram poll to a quiz question, please forward me a poll message."
        "\n\nMake sure it's the poll itself, not just text.\n\nYou can also use /poll2q [number] to specify a custom ID."
    )
...
async def handle_forwarded_poll(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Check if the message has a poll, regardless of how it was sent
...
async def receive_txt_file(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        try:
            ...
            # Ask for a custom numeric ID
            await update.message.reply_text(
                "✅ File received and ready for import!\n\n"
                "📝 Now, please provide a numeric ID for these questions (e.g., 123, 456).\n"
                "This ID will be used to organize and access your questions later."
            )
            
            # Move to the custom ID state
...
async def set_custom_id_txt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Validate that the custom ID is a number
    try:
        # Convert to integer to ensure it's a number
        custom_id = int(custom_id)
        # Store the numeric ID for later use
        context.user_data['txt_custom_id'] = custom_id
        logger.info(f"Valid numeric custom ID: {custom_id}")
    except ValueError:
        # Not a valid number
        await update.message.reply_text(
            "❌ Invalid ID. Please provide a numeric ID (e.g., 123).\n"
            "Try again or /cancel to abort."
        )
        return TXT_CUSTOM_ID
...
def main() -> None:
    ...
    # Handle any message containing a poll, not just forwarded ones
    application.add_handler(MessageHandler(
    ...
    # Text Import conversation handler with custom ID step

...
async def handle_quiz_end_with_pdf(update, context, quiz_id, user_id, user_name, 
    ...
    try:
        ...
        # Force FPDF availability
        global FPDF_AVAILABLE
        try:
            from fpdf import FPDF
            FPDF_AVAILABLE = True
        except ImportError:
            logger.error("FPDF import failed in handler")
            FPDF_AVAILABLE = False
            
        # Create PDF directly here for better control
        if FPDF_AVAILABLE:
            ensure_pdf_directory()
            leaderboard = get_quiz_leaderboard(quiz_id)
            penalty_value = get_quiz_penalty(quiz_id)
            
            pdf = InsaneResultPDF(quiz_id)
            pdf.alias_nb_pages()
            pdf.add_page()
            ...
            # Save the PDF - use current directory for simplicity
            timestamp = datetime.datetime.now().strftime('%Y%m%d%H%M%S')
            filename = f"./quiz_{quiz_id}_results_{timestamp}.pdf"
            
            try:
                pdf.output(filename)
                logger.info(f"PDF generated at: {filename}")
                
                # Basic file check
                file_valid = os.path.exists(filename) and os.path.getsize(filename) > 100
            ...
            file_valid = False
            filename = None
            logger.error("FPDF not available for PDF generation")
        
        # If PDF was generated successfully and verified, send it
        if file_valid and filename:
            try:
                # Send the PDF file
            ...
            await update.message.reply_text("❌ Sorry, couldn't generate PDF results.")
            return False
    except Exception as e:
        logger.error(f"Unexpected error in PDF handler: {e}")
...
async def end_quiz(context, chat_id):
    ...
    if quiz_id and FPDF_AVAILABLE and final_scores:
        try:
            ...
            # Create a fake update object for the enhanced PDF handler
            class FakeMessage:

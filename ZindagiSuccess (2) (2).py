...
def generate_pdf_results(quiz_id, title=None):
    ...
    # Make sure PDF_RESULTS_DIR exists - Fix for PDF generation issue
    try:
        os.makedirs(PDF_RESULTS_DIR, exist_ok=True)
        logger.info(f"PDF results directory created/verified at: {PDF_RESULTS_DIR}")
    except Exception as dir_error:
        logger.error(f"Error creating PDF results directory: {dir_error}")
        # Try an alternative directory as fallback
        PDF_RESULTS_DIR = "."
        logger.info(f"Using fallback directory: {PDF_RESULTS_DIR}")
...
async def handle_quiz_end_with_pdf(update, context, quiz_id, user_id, user_name, 
    ...
    # If PDF was generated successfully, send it
    if pdf_file and os.path.exists(pdf_file):
        ...
        with open(pdf_file, 'rb') as file:
            ...
            await update.message.reply_text("✅ PDF Results generated successfully!")
...
async def end_quiz(context, chat_id):
    ...
    for user_id, user_data in participants.items():
        ...
        # Ensure we're preserving decimal values (using float to avoid any implicit conversion)
    ...
    if quiz_id and FPDF_AVAILABLE and final_scores:
        try:
            # Process the quiz results and generate PDF
            ...
            # Add quiz results to database and generate PDF
            ...
            # Generate the PDF
            pdf_file = generate_pdf_results(quiz_id, f"Quiz {quiz_id} Results")

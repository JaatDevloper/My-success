...
async def handle_quiz_end_with_pdf(update, context, quiz_id, user_id, user_name, 
    ...
    if pdf_file:
        try:
            if os.path.exists(pdf_file) and os.path.getsize(pdf_file) > 100:  # Ensure at least 100 bytes
                ...
                logger.error(f"PDF file too small or doesn't exist: {pdf_file}")
        except Exception as e:
    ...
    if file_valid:
        try:
            ...
            await update.message.reply_text("❌ Error sending PDF results.")
            return False
    else:

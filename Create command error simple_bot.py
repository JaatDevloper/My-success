...
async def create_type_received(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for q in quiz_data["questions"]:
        q["quiz_id"] = quiz_id
        add_question_with_id(quiz_id, q)
    save_questions(all_questions)
...
def main() -> None:
    ...
    create_quiz_conv_handler = ConversationHandler(
        ...
        states={
            ...
            CREATE_QUESTIONS: [
                MessageHandler(filters.Document.PDF | filters.Document.TEXT, create_questions_file_received),
                MessageHandler(filters.TEXT & ~filters.COMMAND, create_questions_file_received),
            ],
            CREATE_SECTIONS: [MessageHandler(filters.TEXT & ~filters.COMMAND, create_sections_received)],
            CREATE_TIMER: [MessageHandler(filters.TEXT & ~filters.COMMAND, create_timer_received)],
            CREATE_NEGATIVE_MARKING: [MessageHandler(filters.TEXT & ~filters.COMMAND, create_negative_marking_received)],
            CREATE_TYPE: [MessageHandler(filters.TEXT & ~filters.COMMAND, create_type_received)],
        },

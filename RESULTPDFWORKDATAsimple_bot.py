...
def generate_pdf_results(quiz_id, title=None):
    ...
    try:
        ...
        try:
            ...
            try:
                # Try to create a simpler PDF without special characters
                logger.info("Trying to create PDF with character replacement...")
                
                # Use a clean, simple PDF with minimal content
                simple_pdf = FPDF()
                simple_pdf.add_page()
                simple_pdf.set_font('Arial', 'B', 16)
                simple_pdf.cell(40, 10, f'Quiz {quiz_id} Results')
                simple_pdf.ln(10)
                simple_pdf.set_font('Arial', '', 12)
                simple_pdf.cell(40, 10, 'Simplified PDF due to encoding issues')
                
                # Save with a different name
                simple_filename = os.path.join(PDF_RESULTS_DIR, f"quiz_{quiz_id}_simple.pdf")
                simple_pdf.output(simple_filename)

...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.args and len(context.args) > 0:
        ...
        # Create a fake callback query to reuse the existing quiz start code
        # This is a clean way to handle direct links without duplicating code
        class FakeCallbackQuery:
            def __init__(self, quiz_id, update, context):
                self.data = f"start_quiz_{quiz_id}"
                self.update = update
        ...
        # Create a fake callback query object
        fake_query = FakeCallbackQuery(quiz_id, update, context)
        
        # Send message about starting quiz from link (matching reference bot format)
        await update.message.reply_text(
            f"📝 Starting quiz with ID: {quiz_id} from direct link..."
        )
        
        # Call the existing function that handles quiz starting
        await start_created_quiz_callback(
            {"callback_query": fake_query, "effective_user": update.effective_user, "effective_chat": update.effective_chat}, 
            context
        )
        return

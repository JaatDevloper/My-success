...
def generate_pdf_results(quiz_id, title=None):
    ...
    # Create a simplified and more reliable PDF
    try:
        # Create timestamp for unique filename
        timestamp = datetime.datetime.now().strftime('%Y%m%d%H%M%S')
        filename = os.path.join(PDF_RESULTS_DIR, f"quiz_{quiz_id}_results_{timestamp}.pdf")
        logger.info(f"Will save PDF to: {filename}")
        
        # Create a PDF with proper layout matching the screenshot
        pdf = FPDF()
        pdf.add_page()
        
        # Set metadata
        pdf.set_author("Telegram Quiz Bot")
        pdf.set_creator("Premium Quiz Results Generator")
        pdf.set_title(title or f"Quiz {quiz_id} Results")
        
        # Create header
        # Blue header background that spans full width
        pdf.set_fill_color(28, 56, 141)  # Deep blue
        pdf.rect(0, 0, 210, 20, style='F')
        
        # Add title in white text
        pdf.set_font('Arial', 'B', 14)
        pdf.set_text_color(255, 255, 255)  # White text
        pdf.set_xy(10, 6)  # Position for title
        pdf.cell(130, 10, title or f"Quiz {quiz_id} Results", 0, 0, 'L')
        
        # Add date in right corner
        pdf.set_xy(140, 6)  # Position for date
        pdf.set_font('Arial', 'I', 8)
        pdf.cell(60, 10, f'Generated: {datetime.datetime.now().strftime("%Y-%m-%d %H:%M")}', 0, 0, 'R')
        
        # Reset position and text color for content
        pdf.set_y(25)
        ...
        pdf.set_text_color(230, 230, 240)  # Very light blue/gray
        pdf.set_xy(35, 110)  # Center of page
        pdf.cell(140, 40, "PREMIUM", 0, 0, 'C')
        # Restore position and text color
        pdf.set_xy(x, y)
        pdf.set_text_color(0, 0, 0)
        
        # Add "QUIZ ANALYTICS" header
        pdf.set_font('Arial', 'B', 12)
        pdf.cell(0, 10, "QUIZ ANALYTICS", 0, 1, 'L')
        
        # Add underline under section title
        ...
        pdf.set_x(10)
        for i, text in enumerate(header_texts):
            pdf.cell(col_widths[i], 10, text, 1, 0, 'C', True)
        pdf.ln()
        ...
        if not leaderboard or not isinstance(leaderboard, list) or len(leaderboard) == 0:
            ...
            pdf.cell(sum(col_widths), 10, "No participants found", 1, 1, 'C')
        else:
            # Process and sanitize leaderboard data for display
            processed_leaderboard = []
            for entry in leaderboard:
                if isinstance(entry, dict):
                    # Create a clean copy of entry data with rank
                    clean_entry = {}
                    clean_entry['rank'] = entry.get('rank', 0)
                    clean_entry['user_id'] = entry.get('user_id', '')
                    clean_entry['adjusted_score'] = entry.get('adjusted_score', 0)
                    clean_entry['correct_answers'] = entry.get('correct_answers', 0)
                    clean_entry['wrong_answers'] = entry.get('wrong_answers', 0)
                    clean_entry['skipped'] = entry.get('skipped', 0)
                    clean_entry['penalty'] = entry.get('penalty', 0)
                    
                    # Process username safely to avoid encoding issues
                    raw_name = str(entry.get('user_name', 'Unknown'))
                    safe_chars = []
                    for c in raw_name:
                    ...
                    if not clean_name or clean_name.isspace():
                        uid = str(entry.get('user_id', ''))[-4:] if 'user_id' in entry else ''
                        rank = entry.get('rank', '')
                        clean_name = f"User_{rank}_{uid}"
                    
                    clean_entry['user_name'] = clean_name
                    processed_leaderboard.append(clean_entry)
            
            # Display leaderboard rows with alternating colors
            pdf.set_font('Arial', '', 10)
            for idx, entry in enumerate(processed_leaderboard):
                ...
                rank = idx + 1
                name = entry.get('user_name', f"User_{rank}")[:25]  # Truncate long names
                score = entry.get('adjusted_score', 0)
                ...
                # Draw row cells
                pdf.set_x(10)
                pdf.cell(col_widths[0], 10, str(rank), 1, 0, 'C', True)
                pdf.cell(col_widths[1], 10, name, 1, 0, 'L', True)
                pdf.cell(col_widths[2], 10, str(score), 1, 0, 'C', True)
                pdf.cell(col_widths[3], 10, str(correct), 1, 0, 'C', True)
                pdf.cell(col_widths[4], 10, str(wrong), 1, 0, 'C', True)
                pdf.cell(col_widths[5], 10, str(skipped), 1, 0, 'C', True)
                pdf.cell(col_widths[6], 10, str(penalty), 1, 0, 'C', True)
                pdf.ln()
        ...
        try:
            ...
            logger.info("PDF created successfully with standard method")
            return filename
        except Exception as e1:
            logger.error(f"Error saving PDF with standard method: {e1}")
            try:
                # Method 2: Binary mode
                pdf_content = pdf.output(dest='S').encode('latin-1')
                with open(filename, 'wb') as f:
                    f.write(pdf_content)
                logger.info("PDF created successfully with binary method")
                return filename
            except Exception as e2:
                logger.error(f"Error saving PDF with binary method: {e2}")
        ...
        return Nonedef process_quiz_end(quiz_id, user_id, user_name, total_questions, correct_answers, 
                   wrong_answers, skipped, penalty, score, adjusted_score, is_creator=False):
...
async def end_quiz(context, chat_id):
    ...
    # Format results
    if final_scores:
        # Add styled header exactly like in the screenshot

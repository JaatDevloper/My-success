...
class InsaneResultPDF(FPDF):
    ...
    def __init__(self, quiz_id, title=None):
        ...
        # Use valid values for the FPDF parameters according to typing requirements
    ...
    def footer(self):
        try:
            ...
            # Use the alias to properly show page numbers (Page X of Y)
            self.cell(90, 10, f'Page {self.page_no()}/{self.alias_nb_pages()}', 0, 0, 'R')
        except Exception as e:
    ...
    try:
        ...
        if file_valid:
            ...
            try:
                ...
                # Create a fresh FPDF instance with minimal content
                final_pdf = FPDF()
                final_pdf.add_page()
                ...
                # Add score and stats
                final_pdf.set_font('Arial', '', 12)
                final_pdf.cell(0, 8, f"Score: {adjusted_score}", 0, 1, 'L')
                final_pdf.cell(0, 8, f"Correct Answers: {correct_answers}", 0, 1, 'L')
                final_pdf.cell(0, 8, f"Wrong Answers: {wrong_answers}", 0, 1, 'L')
                final_pdf.cell(0, 8, f"Skipped Questions: {skipped}", 0, 1, 'L')
                final_pdf.cell(0, 8, f"Penalty: {penalty}", 0, 1, 'L')
                
                # Add timestamp
                final_pdf.ln(10)
                final_pdf.set_font('Arial', 'I', 10)
                final_pdf.cell(0, 8, f"Generated: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}", 0, 1, 'R')

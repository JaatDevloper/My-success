...
def generate_quiz_pdf(quiz_id, quiz_title, questions_data, file_path=None):
    ...
    try:
        ...
        # Ensure file_path is a string or path-like object, not a boolean
        if isinstance(file_path, bool):
            if file_path is True:
                # If True was passed, generate a default path
                file_path = get_quiz_pdf_path(quiz_id, quiz_title)
                logger.warning(f"Boolean True was passed as file_path, using default path: {file_path}")
            else:
                # If False was passed, treat it as None (return BytesIO)
                file_path = None
                logger.warning("Boolean False was passed as file_path, treating as None (will return BytesIO)")

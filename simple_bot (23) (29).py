...
async def handle_poll_custom_id(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if context.user_data.get("awaiting_poll_id"):
        # Accept any string as the ID - no validation needed
        custom_id = update.message.text.strip()
    ...
    # If we get here without the awaiting_poll_id flag, something went wrong
    print("Error: awaiting_poll_id flag not set in context")
    await update.message.reply_text(
        "I wasn't expecting an ID at this time. Please try the /poll2q command again."
    )
    return ConversationHandler.END
...
async def process_txt_file(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not file_path:
        ...
        if update.message:
            ...
            await update.callback_query.edit_message_text("❌ File path not found. Please try uploading again.")
        return ConversationHandler.END
    ...
    if not os.path.exists(file_path):
        ...
        if update.message:
            ...
            await update.callback_query.edit_message_text("❌ File not found on disk. Please try uploading again.")
        return ConversationHandler.END

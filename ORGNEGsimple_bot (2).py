...
def fetch_url_content_with_bs4(url: str) -> Optional[str]:
    ...
    try:
        ...
        if response.encoding == 'ISO-8859-1':
            # Try to guess the encoding from the content
            # Since we can't use requests.utils directly, use alternative approach
            
            # Common encodings to check in priority order 
            potential_encodings = ['utf-8', 'utf-16', 'utf-32', 'windows-1252', 'latin-1']
            
            # For non-English sites, we'll often want UTF-8
            if 'hindi' in url.lower() or 'hindi' in response.text.lower() or 'राजस्थान' in response.text:
...
def extract_questions_from_text(text: str) -> List[Dict]:
    ...
    option_patterns = [
        ...
        # Hindi option patterns
        r'^(?:[०-९][\.\)]|[अआइईउऊएऐओऔ][\.\)])\s*(.+)', 
        # Hindi letter options
...
async def process_url(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # For Google AMP URLs, extract and try with the real URL if possible
    original_url = url
    if 'google.com/amp' in url:

...
async def parse_questions_from_text(text: str) -> List[Dict[str, Any]]:
    ...
    patterns = [
        ...
        r'(?:Q|प्रश्न)[.\s]*(\d+)[.\s]*\s*([^?]+\??)\s*(?:[aअ])[.\s)*]\s*([^बb]+)\s*(?:[bब])[.\s)*]\s*([^cग]+)\s*(?:[cग])[.\s)*]\s*([^dड]+)\s*(?:[dड])[.\s)*]\s*([^\n]+)',
    ...
    for pattern in patterns:
        ...
        for match in matches:
            try:
                ...
                elif len(match.groups()) == 6:  # Pattern 2
                    question_id = match.group(1)
                    question_text = match.group(2).strip()
                    options = [
                        match.group(3).strip(),
    ...
    # If no questions found with the regex patterns, try a simpler approach
    if not questions:
        # Look for lines that seem like questions (ending with ?)
        question_lines = re.findall(r'([^\n]+\?)[^\n]*', text)
        
        # Look for lines that could be options (starting with a/b/c/d or 1/2/3/4)
        option_patterns = [
            r'(?:[aअ])[.\s)*]\s*([^\n]+)',
            r'(?:[bब])[.\s)*]\s*([^\n]+)',
            r'(?:[cग])[.\s)*]\s*([^\n]+)',
        ...
        for pattern in option_patterns:
            ...
            potential_options.extend(matches)
        
        # If we have at least one question and 4+ potential options, create a question
        if question_lines and len(potential_options) >= 4:
            for i, q_text in enumerate(question_lines):
                options = potential_options[i*4:i*4+4] if i*4+4 <= len(potential_options) else potential_options[:4]
                if len(options) == 4:
                    question = {

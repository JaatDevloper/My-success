...
def generate_quiz_pdf(quiz_id, quiz_title, questions_data, file_path=None):
    ...
    try:
        ...
        target = file_path if file_path else buffer
...
async def send_quiz_pdf(context, chat_id, quiz_id, quiz_title, questions):
    ...
    try:
        ...
        # Generate PDF
        pdf_path = get_quiz_pdf_path(quiz_id, quiz_title)
        result = generate_quiz_pdf(quiz_id, quiz_title, questions, pdf_path)
...
async def quizpdf_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if formatted_questions:
        try:
            ...
            safe_title = "".join(c for c in quiz_title if c.isalnum() or c in " _-").rstrip()
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            pdf_filename = f"quiz_{quiz_id or 'manual'}_{safe_title}_{timestamp}.pdf"
            pdf_path = os.path.join(pdf_dir, pdf_filename)
            
            # Generate PDF
            result = generate_quiz_pdf(quiz_id or "manual", quiz_title, formatted_questions, pdf_path)

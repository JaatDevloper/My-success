...
def get_user_quizzes(user_id):
    ...
    # Get all quizzes from the questions database
    for quiz_id, quiz_data in all_questions.items():
        # Skip non-dictionary entries or entries without quiz metadata
        if not isinstance(quiz_data, dict) and not isinstance(quiz_data, list):
            continue
            
        # For list-type entries, check the first item
        if isinstance(quiz_data, list) and quiz_data:
            # If it's a list of questions, get the first one to extract metadata
            if quiz_data and isinstance(quiz_data[0], dict):
                first_question = quiz_data[0]
                # Extract quiz metadata
                title = first_question.get("quiz_name", first_question.get("quiz_title", "Untitled Quiz"))
                creator_id = str(first_question.get("creator_id", ""))
                
                # If user is the creator, add to the list
                if creator_id == str(user_id):
                    # Get engagement info (participation count)
                    quiz_results = get_quiz_results(quiz_id)
                    engagement = len(quiz_results.get("participants", []))
                    
                    user_quizzes.append({
                        "id": quiz_id,
                        "title": title,
                        "type": first_question.get("quiz_type", "Free"),
                        "engagement": engagement,
                        "is_creator": True
                    })
        
        # For dictionary-type entries with quiz_id field
        elif isinstance(quiz_data, dict):
            # If this is a question with quiz_id field
            if "quiz_id" in quiz_data:
                # This is a single question with quiz metadata
                quiz_id_from_data = quiz_data.get("quiz_id", "")
                title = quiz_data.get("quiz_name", quiz_data.get("quiz_title", "Untitled Quiz"))
                creator_id = str(quiz_data.get("creator_id", ""))
                
                # If this is a different format where quiz_id is a field
                if quiz_id_from_data and creator_id == str(user_id):
                    # Get engagement info
                    quiz_results = get_quiz_results(quiz_id_from_data)
                    engagement = len(quiz_results.get("participants", []))
                    
                    # Add it only if it's not already in the list
                    if not any(q["id"] == quiz_id_from_data for q in user_quizzes):
                        user_quizzes.append({
                            "id": quiz_id_from_data,
                            "title": title,
                            "type": quiz_data.get("quiz_type", "Free"),
                            "engagement": engagement,
                            "is_creator": True
                        })
    
    # Get quizzes where user has participated (from quiz results)
    ...
    for quiz_id, result_data in quiz_results.items():
        ...
        for participant in participants:
            if str(participant.get("user_id", "")) == str(user_id):
                # Check if quiz is already in the list (added as creator)
                if not any(q["id"] == quiz_id for q in user_quizzes):
                    ...
                    if quiz_id in all_questions:
                        ...
                        if isinstance(quiz_data, list) and quiz_data and isinstance(quiz_data[0], dict):
                            first_question = quiz_data[0]
                            title = first_question.get("quiz_name", first_question.get("quiz_title", "Untitled Quiz"))
                            quiz_type = first_question.get("quiz_type", "Free")
...
async def myquizzes_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Show all quizzes created by or participated in by the user."""
    user_id = update.effective_user.id
    ...
    if not user_quizzes:
        await update.message.reply_text(
            "You haven't created or participated in any quizzes yet.\n\n"
            "Use /create to create a new quiz or /quiz to start a random quiz."
        )
        return
    ...
    message = f"📝 <b>Your Quizzes (Page {page}):</b>\n\n"
    ...
    for i, quiz in enumerate(current_page_quizzes, start=start_idx + 1):
        ...
        # Format each quiz entry
        message += f"<b>{i}. {html.escape(title)}</b>\n"
        message += f"   - 🆔 ID: {quiz_id}\n"
        message += f"   - 📄 Type: {quiz_type}\n"
        message += f"   - 👥 Engagement: {engagement}\n"
        message += f"   - ✏️ Edit: /edit {quiz_id}\n"
...
async def start_created_quiz_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # DON'T edit message (keep quiz details visible)
    # Instead, send a new message
    await context.bot.send_message(
        chat_id=update.effective_chat.id,
        text=f"⏳ Starting quiz with ID: {quiz_id}\n"
    ...
    try:
        ...
        results.append(
            InlineQueryResultArticle(
                ...
                # Removed thumb_url as it's not supported in newer PTB versions
            )
        )

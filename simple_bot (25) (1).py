...
# PDF viewing is now a simple command with arguments, no conversation states needed
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    # Create welcome keyboard with channel button matching screenshot
    keyboard = [
        [InlineKeyboardButton("🔔 Join Our Channel", url="https://t.me/quizbot_channel")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    # Create welcome message with emoji and formatting to exactly match the screenshot
    welcome_message = (
        "👋 Welcome to Advance Quiz Bot!\n\n"
        "Create quizzes with MCQs, sections, timers, and even convert TestBook tests "
        "to quizzes. Add media/text, share in groups, or invite users to answer.\n\n"
...
async def features_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Show available features when the command /features is issued."""
    features_text = (
        "✨ *Bot Features* ✨\n\n"
        "*Quiz Creation*\n"
        "• Create MCQs from questions you add\n"
        "• Import questions from PDF or TXT files\n"
        "• Convert Telegram polls to quiz questions\n"
        "• Organize questions with custom IDs\n\n"
        
        "*Quiz Settings*\n"
        "• Set negative marking for wrong answers\n"
        "• Create sectional quizzes with specific IDs\n"
        "• Set timers for automatic question progression\n\n"
        
        "*Administration*\n"
        "• Generate PDFs with quiz results\n"
        "• Get detailed user statistics\n"
        "• Reset penalties or quiz progress\n"
    )
...
async def add_question_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Check if custom ID was provided
    if context.args:
        custom_id = context.args[0]
        # Validate custom ID
        if validate_custom_id(custom_id):
            context.user_data["custom_id"] = custom_id
            await update.message.reply_text(f"Using custom ID: {custom_id}\nLet's add a new quiz question! First, send me the question text.")
            return QUESTION
        else:
            await update.message.reply_text("Invalid custom ID. It must be 3-10 alphanumeric characters and not already in use.")
    
    # No custom ID or invalid custom ID
    await update.message.reply_text(
        "Let's add a new quiz question! First, send me the question text."
    )
    return QUESTION
...
# ------- QUIZ PDF GENERATION COMMAND --------
async def quizpdf_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Generate a PDF of quiz questions for a specific ID."""
    user = update.effective_user
    logger.info(f"User {user.id} ({user.username}) started /quizpdf")
    
    # Check if ID is provided
    args = context.args
    if not args or len(args) == 0:
        # No quiz ID provided, show instructions
        await update.message.reply_text(
            "📄 *Quiz PDF Generator*\n\n"
            "This command generates a PDF containing all questions from a specific quiz ID.\n\n"
            "*Usage:*\n"
            "`/quizpdf [QUIZ_ID]`\n\n"
            "Example: `/quizpdf GK2023`\n\n"
            "You can find your quiz IDs by using the /quizid command without parameters.",
            parse_mode='Markdown'
        )
        return
    ...
    # Check if questions with this ID exist
    questions = load_questions()
    if quiz_id not in questions or not questions[quiz_id]:
        await update.message.reply_text(
            f"❌ No questions found for quiz ID: *{quiz_id}*\n\n"
            "Please check the ID and try again.",
            parse_mode='Markdown'
        )
        return
    
    # Let the user know we're generating the PDF
    status_message = await update.message.reply_text(
        f"🔄 Generating PDF for quiz ID: *{quiz_id}*\n"
        f"Found {len(questions[quiz_id])} questions.\n"
        "Please wait...",
        parse_mode='Markdown'
    )
    ...
    # Generate the PDF
    try:
        pdf_path = await generate_questions_pdf(quiz_id, questions[quiz_id])
        ...
        # Send the PDF file
        await status_message.edit_text("✅ PDF generated successfully! Uploading...")
        ...
        with open(pdf_path, 'rb') as pdf_file:
            await context.bot.send_document(
                ...
                caption=f"📚 Quiz Questions - {quiz_id}\n"
                        f"Total Questions: {len(questions[quiz_id])}"
            )
...
async def generate_questions_pdf(quiz_id, questions_list):
    """Generate a PDF document containing all questions for a quiz ID with Unicode support."""
    ...
    pdf_path = os.path.join(PDF_RESULTS_DIR, pdf_filename)
    
    if not FPDF_AVAILABLE:
        logger.error("FPDF library not available, cannot generate PDF")
        return None
    
    try:
        # Define safe text function to handle non-Latin characters 
        def safe_text(text):
        ...
        # We'll use the simpler approach that's more reliable
        # This approach works consistently without font issues
        pdf = FPDF()
        pdf.add_page()
        
        # For a safer approach, sanitize all text before adding to PDF 
        safe_quiz_id = safe_text(quiz_id)
        
        # Process all questions and options to ensure they're PDF-safe
        sanitized_questions = []
        for q in questions_list:
            sanitized_q = {
                "question": safe_text(q.get("question", "")),
                "options": [safe_text(opt) for opt in q.get("options", [])],
                "answer": q.get("answer", 0),
                "category": safe_text(q.get("category", ""))
            }
            sanitized_questions.append(sanitized_q)
        
        # Continue with the sanitized data
        questions_list = sanitized_questions
        
        # Set up title and header
        pdf.set_font("Helvetica", "B", 16)
        pdf.cell(0, 10, f"Quiz Questions - {safe_quiz_id}", 0, 1, "C")
        ...
        for i, question_data in enumerate(questions_list, 1):
            # Get question data - already sanitized
            question_text = question_data.get("question", "")
            options = question_data.get("options", [])
            correct_answer = question_data.get("answer", 0) 
            category = question_data.get("category", "")
            
            # Question number
            pdf.set_font("Helvetica", "B", 12)
            pdf.cell(0, 10, f"Question {i}:", 0, 1)
            ...
            for j, option in enumerate(options):
                ...
                pdf.cell(0, 7, option_text, 0, 1)
            
            # Category if available
            if category:
                pdf.ln(2)
                pdf.set_font("Helvetica", "I", 10)
                pdf.cell(0, 7, f"Category: {category}", 0, 1)
...
async def receive_txt_file(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        try:
            ...
            # Check if we have a custom ID from command or use the auto-generated one
            custom_id = context.user_data.get('txt_custom_id', auto_id)
            
            # Ask user to confirm or enter custom ID
            keyboard = [
                [InlineKeyboardButton("Use this ID", callback_data=f"use_txtid_{custom_id}")],
                [InlineKeyboardButton("Enter custom ID", callback_data="custom_txtid")]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            
            # Notify user about ID options
            await update.message.reply_text(
                f"✅ File processed successfully!\n\n"
                f"Auto-generated ID: <b>{custom_id}</b>\n\n"
                "You can use this ID or enter a custom one:",
                parse_mode='HTML',
                reply_markup=reply_markup
            )
...
async def handle_txtimport_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle callback queries for text import options"""
    query = update.callback_query
    await query.answer()
    
    data = query.data
    logger.info(f"Received text import callback: {data}")
    
    if data.startswith("use_txtid_"):
        # Extract the ID from the callback data
        custom_id = data.replace("use_txtid_", "")
        context.user_data['txt_custom_id'] = custom_id
        
        await query.edit_message_text(
            f"✅ Using ID: <b>{custom_id}</b>\n\nProcessing your file...",
            parse_mode='HTML'
        )
        
        # Process the file with this ID
        return await process_txt_file(update, context)
    
    elif data == "custom_txtid":
        await query.edit_message_text(
            "📝 Please enter a custom ID for this quiz.\n\n"
            "This ID will be used to reference these questions when starting a quiz.\n"
            "Enter a single word without spaces (e.g. 'history2025').\n\n"
            "Type /cancel to abort."
        )
        
        # Set state to await custom ID input
        context.user_data['awaiting_txt_custom_id'] = True
        # This doesn't change the conversation state directly since we're in a callback
...
def main() -> None:
    ...
    # TXT Import Callback handler
    application.add_handler(CallbackQueryHandler(
        callback=lambda u, c: handle_txtimport_callback(u, c),
        pattern=r"^(use_txtid_|custom_txtid)"
    ))
    
    # Handle custom ID input after the callback
    application.add_handler(MessageHandler(
        filters.TEXT & ~filters.COMMAND,
        lambda u, c: set_custom_id_txt(u, c),
        lambda u, c: c.user_data.get("awaiting_txt_custom_id", False)
    ))
    ...
    # Add the quizpdf command handler to generate PDFs with quiz questions

    ...
    try:
        ...
        if context.args and len(context.args) > 0:
            ...
            if recent_quizzes:
                ...
                # If no recent quizzes, use '1' as default quiz ID
                quiz_id = '1'
        ...
        # REPORTLAB METHOD - BEST UNICODE SUPPORT
        try:
            if REPORTLAB_AVAILABLE:
                from reportlab.lib.pagesizes import letter
                from reportlab.pdfgen import canvas
                ...
                pdf_dir = os.path.join(os.getcwd(), "pdf_results")
                ...
                # Add title
                story.append(Paragraph(f"Quiz #{quiz_id} - Questions and Answers", title_style))
                ...
                for i, question_data in enumerate(quiz_questions, 1):
                    # Process question data safely - with fallback for unicode issues
                    try:
                        ...
                        # Add question
                        story.append(Paragraph(f"<b>Q{i}. {question_text}</b>", normal_style))
                        ...
                        for j, option in enumerate(options):
                            ...
                            # Mark correct answer
                            if j == correct_index:
                                option_text = f"{chr(65+j)}. {option_text} (CORRECT)"
                            else:
                                option_text = f"{chr(65+j)}. {option_text}"
                            
                            story.append(Paragraph(f"&nbsp;&nbsp;{option_text}", normal_style))
                        
                        story.append(Spacer(1, 0.2 * inch))
                    except Exception as q_err:
                        # If there are any encoding issues with a question, skip it
                        story.append(Paragraph(f"<b>Q{i}. [Format error]</b>", normal_style))
                        story.append(Spacer(1, 0.2 * inch))
                ...
                if os.path.exists(pdf_path):
                    ...
                    # Return to avoid fallback method
                    return
                else:
                    # If file wasn't created, try fallback
                    raise FileNotFoundError("PDF file wasn't created")
        except Exception as reportlab_err:
            # Log the error but continue to fallback
            logger.error(f"ReportLab error: {reportlab_err}")
        
        # ALTERNATIVE TEXT-ONLY APPROACH
        try:
            # Create text file with quiz content
            text_dir = os.path.join(os.getcwd(), "pdf_results")
            ...
            with open(text_path, 'w', encoding='utf-8') as f:
                ...
                for i, question_data in enumerate(quiz_questions, 1):
                    ...
                    for j, option in enumerate(options):
                        ...
                        # Mark correct answer
                        if j == correct_index:
                            f.write(f"   {chr(65+j)}. {option_text} (CORRECT)\n")
                        else:
            ...
            with open(text_path, 'rb') as text_file:
                await context.bot.send_document(
                    chat_id=chat_id,
                    document=text_file,
                    filename=f"Quiz_{quiz_id}_Questions.txt",
            ...
            await context.bot.edit_message_text(
                chat_id=chat_id,
                message_id=processing_message.message_id,
                text=f"✅ Quiz data for Quiz #{quiz_id} has been sent as a text file! (PDF generation failed)"
            )
            
        except Exception as text_err:
            # If even text file fails, send error
            await context.bot.edit_message_text(
                chat_id=chat_id,
                message_id=processing_message.message_id,
                text=f"❌ Error generating quiz content: {str(text_err)[:50]}..."
            )
        ...
        await context.bot.edit_message_text(
            ...
            text=f"❌ An error occurred: {str(e)[:50]}..."
                            ...
                            # Keep the fallback if not ASCII compatible
                            pass
                    ...
                    # Add options (ASCII only)
                    pdf.set_font("Arial", "", 11)
                    for j, option in enumerate(options):
                        # Test if option can be encoded as ASCII
                        try:
                            option_text = str(option)
                            option_text.encode('ascii', 'strict')
                        except UnicodeEncodeError:
                            option_text = f"Option {j+1}"  # Fallback for non-ASCII
                ...
                await context.bot.edit_message_text(
                    chat_id=chat_id,
                    message_id=processing_message.message_id,
                    text=f"✅ PDF for Quiz #{quiz_id} has been generated and sent! (ASCII mode)"
                )
            except Exception as fpdf_err:
                # Very basic fallback with just numbers
                await context.bot.edit_message_text(
                    chat_id=chat_id,
                    message_id=processing_message.message_id,
                    text=f"❌ Error generating PDF. Unicode/encoding issue: {str(fpdf_err)[:50]}..."
                )

...
async def check_subscription(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    try:
        ...
        # Check if user is a member, creator, or administrator
        return chat_member.status in ['member', 'creator', 'administrator']
    except Exception as e:
        logger.error(f"Error checking subscription: {e}")
        # For testing purposes, we can return True to bypass subscription check
        # In production, this should be False to enforce subscription
        return False
...
async def force_subscription_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Just one button, exactly as shown in the screenshot
    keyboard = [
        [InlineKeyboardButton("Join Now...", url=CHANNEL_URL)]
    ]
    # Add a second button for checking subscription status
    ...
    try:
        ...
        image_data = base64.b64decode(ROBOT_IMAGE_BASE64)
        photo = io.BytesIO(image_data)
        photo.name = 'robot_image.png'
...
async def handle_subscription_check_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check if user is subscribed now
    ...
    if is_subscribed:
        ...
        # User is still not subscribed, send message with join button again
        # First just the "Join Now..." button, exactly like the screenshot
        keyboard = [
            [InlineKeyboardButton("Join Now...", url=CHANNEL_URL)]
        ]
        # Then add the Check Again button
        keyboard.append([InlineKeyboardButton("Check Again", callback_data="check_subscription")])

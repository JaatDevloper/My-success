...
ROBOT_IMAGE_URLS = [
    "https://i.ibb.co/HPFckph/robot-image.png",  # Primary image URL
    "https://i.imgur.com/HPFckph.png",  # Backup URL
...
async def handle_subscription_check_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # IMPORTANT: We have to use the raw username WITHOUT @ sign for the API call!
    # But we need to ensure it doesn't already have the @ sign first
    channel = CHANNEL_USERNAME
    if channel.startswith('@'):
        channel = channel[1:]  # Remove @ if it exists
    
    # Then add it for the API call
    channel_with_at = f"@{channel}"
    logger.info(f"Checking membership in channel: {channel_with_at}")
    
    try:
        # The key fix: Use different formats for the channel_id
        # Try multiple methods to check membership - this is crucial for reliability
        
        # Method 1: Try with username with @ sign (most common format)
        try:
            chat_member = await context.bot.get_chat_member(chat_id=channel_with_at, user_id=user_id)
            status = chat_member.status
            logger.info(f"Method 1 status for user {user_id}: {status}")
        except Exception as e1:
            logger.info(f"Method 1 failed, trying alternate methods: {e1}")
            
            # Method 2: Try with just the username without @ sign
            try:
                chat_member = await context.bot.get_chat_member(chat_id=channel, user_id=user_id)
                status = chat_member.status
                logger.info(f"Method 2 status for user {user_id}: {status}")
            except Exception as e2:
                logger.info(f"Method 2 failed, trying with channel ID: {e2}")
                
                # Method 3: Try with a numeric channel ID if it's available
                try:
                    # Use the channel ID as a last resort
                    if CHANNEL_ID != -1001234567890:  # Only if it's been properly set
                        chat_member = await context.bot.get_chat_member(chat_id=CHANNEL_ID, user_id=user_id)
                        status = chat_member.status
                        logger.info(f"Method 3 status (ID) for user {user_id}: {status}")
                    else:
                        raise ValueError("CHANNEL_ID is not properly configured")
                except Exception as e3:
                    logger.error(f"All verification methods failed: {e3}")
                    status = "failed"
        
        # Valid member statuses
        if status in ['member', 'creator', 'administrator']:
            logger.info(f"SUCCESS: User {user_id} is confirmed as channel subscriber with status: {status}")
            ...
            logger.info(f"User {user_id} is NOT a subscriber (status: {status})")

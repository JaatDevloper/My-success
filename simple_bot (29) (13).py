            ...
            if leaderboard and isinstance(leaderboard, list):
                ...
                for entry in leaderboard:
                    ...
                    if 'user_name' in clean_entry:
                        ...
                        # Replace any problematic character
                        safe_name = ''
                        for c in raw_name:
    ...
    try:
        ...
        if file_valid:
            try:
                ...
                try:
                    # Add notice that we're generating HTML report
                    await update.message.reply_text("📊 *Generating Interactive HTML Report...*", parse_mode="Markdown")
                    
                    # First try to copy the HTML generator from attached_assets if needed
                    try:
                        import os
                        import shutil
                        
                        # Check if html_results_generator.py already exists in the current directory
                        if not os.path.exists("html_results_generator.py"):
                            logger.info("HTML generator not found in current directory, looking elsewhere...")
                            
                            # Look in the attached_assets directory first
                            assets_file = os.path.join("attached_assets", "html_results_generator.py")
                            if os.path.exists(assets_file):
                                logger.info(f"Found HTML generator in attached_assets directory")
                                shutil.copy2(assets_file, "html_results_generator.py")
                                logger.info("Copied HTML generator from attached_assets to current directory")
                    except Exception as copy_error:
                        logger.error(f"Error checking or copying HTML generator file: {copy_error}")
                    
                    # Try to import the HTML generator with multiple fallbacks
                    html_generator = None
                    import_errors = []
                    
                    # Make sure we add current directory to path
                    import sys
                    ...
                    try:
                        ...
                        html_generator = html_results_generator
                        logger.info("Successfully imported HTML results generator module directly")
                    except ImportError as e:
                        import_errors.append(f"Direct import failed: {e}")
                        logger.error(f"Direct import of HTML generator failed: {e}")
                    
                    # If direct import failed, try from the same directory
                    if html_generator is None:
                        try:
                            ...
                            html_generator_path = os.path.join(current_dir, "html_results_generator.py")
                            
                            if os.path.exists(html_generator_path):
                                logger.info(f"Found HTML generator at: {html_generator_path}")
                                spec = importlib.util.spec_from_file_location("html_results_generator", html_generator_path)
                                if spec and spec.loader:
                                    html_generator = importlib.util.module_from_spec(spec)
                                    spec.loader.exec_module(html_generator)
                                    logger.info(f"Successfully imported HTML generator from current directory")
                            else:
                                import_errors.append(f"HTML generator not found in current directory")
                                logger.error(f"HTML generator not found at: {html_generator_path}")
                        except Exception as e:
                            import_errors.append(f"Import from current directory failed: {e}")
                            logger.error(f"Error importing HTML generator from current directory: {e}")
                    
                    # If both imports failed, try the parent directory
                    if html_generator is None:
                        try:
                            import importlib.util
                            import os
                            ...
                            html_generator_path = os.path.join(parent_dir, "html_results_generator.py")
                            
                            if os.path.exists(html_generator_path):
                                logger.info(f"Found HTML generator in parent directory: {html_generator_path}")
                                spec = importlib.util.spec_from_file_location("html_results_generator", html_generator_path)
                                if spec and spec.loader:
                                    html_generator = importlib.util.module_from_spec(spec)
                                    spec.loader.exec_module(html_generator)
                                    logger.info("Successfully imported HTML generator from parent directory")
                            else:
                                import_errors.append(f"HTML generator not found in parent directory")
                                logger.error(f"HTML generator not found in parent directory: {html_generator_path}")
                        except Exception as e:
                            import_errors.append(f"Import from parent directory failed: {e}")
                            logger.error(f"Error importing HTML generator from parent directory: {e}")
                    
                    # If all import attempts failed, notify the user and return
                    if html_generator is None:
                        error_msg = "; ".join(import_errors)
                        logger.error(f"All HTML generator import attempts failed: {error_msg}")
                        await update.message.reply_text(
                            f"❌ HTML report generation failed: Could not load the HTML generator module. PDF results are still available."
                        )
                        return True  # PDF was still sent successfully
                    ...
                    # Get quiz questions (convert from dict to list format if needed)
                    questions_data = load_questions()
                    logger.info(f"Loaded questions: {len(questions_data)} question sets")
                    ...
                    for qid, q_data in questions_data.items():
                        if str(qid).startswith(str(quiz_id)):
                            ...
                            if isinstance(q_data, list):
                                for q in q_data:
                                    q["id"] = qid  # Make sure ID is in the question data
                                    quiz_questions.append(q)
                            else:
                                q_data["id"] = qid  # Make sure ID is in the question data
                                quiz_questions.append(q_data)
                    
                    logger.info(f"Found {len(quiz_questions)} questions for quiz {quiz_id}")
                    
                    # Print first question for debugging if available
                    if quiz_questions:
                        logger.info(f"Sample question: {quiz_questions[0]}")
                    
                    # Get quiz results with better error handling
                    quiz_results = get_quiz_results(quiz_id)
                    if not quiz_results:
                        quiz_results = []  # Ensure we have at least an empty list
                    logger.info(f"Found {len(quiz_results)} participants for quiz {quiz_id}")
                    
                    # Print sample participant data if available
                    if quiz_results:
                        logger.info(f"Sample participant: {quiz_results[0]}")
                    
                    # Prepare quiz metadata with defaults for safety
                    quiz_metadata = {
                        "total_questions": total_questions or len(quiz_questions) or 0,
                    ...
                    for participant in quiz_results:
                        ...
                        if "time_taken" not in participant:
                            participant["time_taken"] = 0  # Default
                        if "user_name" not in participant:
                            participant["user_name"] = f"User_{participant.get('user_id', 'unknown')}"
                        if "answers" not in participant:
                            participant["answers"] = {}
                    
                    logger.info("Calling HTML generator with processed data")
                    
                    # Debug parameters being passed to the generator
                    logger.info(f"HTML generator params: quiz_id={quiz_id}, title='Quiz {quiz_id} Analysis', "
                               f"questions_count={len(quiz_questions)}, participants_count={len(quiz_results)}")
                    
                    # Generate HTML report with verbose error handling
                    try:
                        html_file = html_generator.generate_html_report(
                            ...
                            leaderboard=quiz_results,
                            quiz_metadata=quiz_metadata
                        )
                        logger.info(f"HTML generator returned: {html_file}")
                    except Exception as gen_error:
                        logger.error(f"HTML generator exception: {gen_error}")
                        # Provide more detailed error info
                        import traceback
                        logger.error(f"HTML generator traceback: {traceback.format_exc()}")
                        await update.message.reply_text(f"❌ HTML report generation failed: {str(gen_error)[:100]}... PDF results are still available.")
                        return True
                    
                    # Send HTML file if generated with extra verification
                    if html_file and os.path.exists(html_file):
                        file_size = os.path.getsize(html_file)
                        logger.info(f"HTML report generated: {html_file} (Size: {file_size} bytes)")
                        
                        if file_size > 100:  # Basic size validation
                            try:
                                # Send the HTML file
                                ...
                                await update.message.reply_text(html_success_message)
                                logger.info("HTML document sent successfully")
                            except Exception as send_error:
                                logger.error(f"Error sending HTML file: {send_error}")
                                await update.message.reply_text(f"❌ Error sending HTML report: {str(send_error)[:100]}... PDF results are still available.")
                        else:
                            logger.error(f"HTML file too small: {file_size} bytes")
                            await update.message.reply_text("❌ HTML report seems invalid (file too small). PDF results are still available.")
                    else:
                        logger.error(f"HTML file not found or empty: {html_file}")
                        await update.message.reply_text("❌ HTML report generation failed: File not created. PDF results are still available.")
                except Exception as html_error:
                    logger.error(f"Error in HTML report generation process: {html_error}")
                    # Provide more detailed error info
                    import traceback
                    logger.error(f"HTML error traceback: {traceback.format_exc()}")
                    # Notify user about failure
                    await update.message.reply_text("❌ HTML report generation encountered an error. PDF results are still available.")
...
async def html_report_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if html_generator is None:
            ...
            logger.error(f"All HTML generator import attempts failed: {error_msg}")
            await update.message.reply_text(
                f"❌ HTML report generation failed: Could not load the HTML generator module."
            )
            return

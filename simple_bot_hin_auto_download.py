...
# Hindi OCR model auto-download
import os
import requests

def download_hin_model():
    tessdata_dir = "./tessdata"
    os.makedirs(tessdata_dir, exist_ok=True)
    hin_path = os.path.join(tessdata_dir, "hin.traineddata")

    if not os.path.exists(hin_path):
        url = "https://github.com/tesseract-ocr/tessdata/raw/main/hin.traineddata"
        print("Downloading hin.traineddata...")
        r = requests.get(url)
        with open(hin_path, "wb") as f:
            f.write(r.content)
        print("Download complete.")

    os.environ['TESSDATA_PREFIX'] = tessdata_dir

# Call this before OCR functions
download_hin_model()

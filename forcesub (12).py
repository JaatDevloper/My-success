...
async def handle_download_profile_html_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle download HTML button to send raw HTML code of the profile"""
    query = update.callback_query
    ...
    try:
        ...
        # Show loading message
        await query.message.reply_text("⏳ Generating HTML code for your profile...")
        
        # Replicate the HTML building logic from user_profile_command
        # Extract basic stats
        total_quizzes = user_profile.get("total_quizzes", 0)
        correct_answers = user_profile.get("total_correct_answers", 0)
        total_questions = user_profile.get("total_questions_answered", 0)
        ...
        if total_questions == 0:
            # No quiz activity yet
            await query.message.reply_text("You haven't taken any quizzes yet! Use /quiz to start your first quiz.")
            return
        ...
        # Get categories
        categories = user_profile.get("categories", {})
        top_categories = sorted(categories.items(), key=lambda x: x[1].get("quiz_count", 0), reverse=True)[:3]
        ...
        # Calculate time-based statistics
        daily_avg = sum([q.get("score", 0) for q in daily_quizzes]) / len(daily_quizzes) if daily_quizzes else 0
        weekly_avg = sum([q.get("score", 0) for q in weekly_quizzes]) / len(weekly_quizzes) if weekly_quizzes else 0
        monthly_avg = sum([q.get("score", 0) for q in monthly_quizzes]) / len(monthly_quizzes) if monthly_quizzes else 0
        yearly_avg = sum([q.get("score", 0) for q in yearly_quizzes]) / len(yearly_quizzes) if yearly_quizzes else 0
        
        # Generate tips
        tips = []
        if total_quizzes < 5:
            tips.append("Take more quizzes to build your profile statistics.")
        if current_streak < 3 and total_quizzes > 0:
            tips.append("Take quizzes daily to build your streak.")
        if avg_score < 70 and total_quizzes > 5:
            tips.append("Review previous quizzes to improve your score.")
        if len(categories) < 3 and total_quizzes > 3:
            tips.append("Try quizzes from different categories to broaden your knowledge.")
        if top_categories and avg_score < 80:
            top_cat = top_categories[0][0]
            tips.append(f"Practice more in your top category ({top_cat}) to improve your score.")
            
        # Add a default tip if none generated
        if not tips:
            tips.append("Keep taking quizzes to improve your statistics and earn achievements!")
            
        # Create HTML for profile header (premium vs non-premium styling)
        ...
        # Create HTML for performance section
        performance_html = (
            f"<b>🎯 PERFORMANCE SUMMARY</b>\n"
            f"• <b>Total Quizzes:</b> {total_quizzes}\n"
            f"• <b>Questions Answered:</b> {total_questions}\n"
        ...
        # Create HTML for streak section
        streak_html = (
            f"<b>🔥 STREAK & ACTIVITY</b>\n"
            f"• <b>Current Streak:</b> {current_streak} days\n"
            f"• <b>Best Streak:</b> {best_streak} days\n"
            f"• <b>Last Quiz Date:</b> {last_quiz_date}\n\n"
        )
        
        # Create HTML for time periods
        time_period_html = (
            f"<b>📅 ACTIVITY TRENDS</b>\n"
            f"• <b>Today:</b> {len(daily_quizzes)} quizzes ({daily_avg:.1f}% avg)\n"
            f"• <b>This Week:</b> {len(weekly_quizzes)} quizzes ({weekly_avg:.1f}% avg)\n"
            f"• <b>This Month:</b> {len(monthly_quizzes)} quizzes ({monthly_avg:.1f}% avg)\n"
            f"• <b>This Year:</b> {len(yearly_quizzes)} quizzes ({yearly_avg:.1f}% avg)\n\n"
        )
        
        # Create HTML for categories section with medal emoji ranking
        ...
        if top_categories:
            ...
            for i, (category, data) in enumerate(top_categories):
                if i < len(medals):
                    ...
                    category_avg = data.get("avg_score", 0)
                    category_html += f"{medal} <b>{category}:</b> {quiz_count} quizzes (Avg: {category_avg:.1f}%)\n"
            category_html += "\n"
        else:
            category_html += "No category data available yet.\n\n"
            
        # Create HTML for achievements section
        ...
        if recent_quizzes:
            for quiz in recent_quizzes:
                ...
                recent_activity_html += f"{emoji} <b>{quiz_name}:</b> {score:.1f}% on {date}\n"
        else:
            recent_activity_html += "No recent quiz activity.\n"
        recent_activity_html += "\n"
        
        # Create HTML for tips section
        tips_html = (
            f"<b>💡 PERSONALIZED TIPS</b>\n"
        )
        ...
        # Create HTML for premium features (only for non-premium users)
        premium_html = ""
        if not is_premium:
            premium_html = (
                f"<b>💎 PREMIUM FEATURES</b>\n"
        ...
        # Send the HTML code as a message with code formatting
        html_message = f"<pre>{profile_message}</pre>"
        
        # Send raw HTML code to the user in a new message
        ...
        logger.info(f"HTML profile code sent successfully to user {user.id}")
...
async def user_profile_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Generate profile content with professional HTML formatting
        profile_text = f"""
<b>🌟 USER PROFILE: {user.full_name}</b>

<b>📊 Performance Overview</b>
• Quizzes Completed: <code>{user_profile.get("quizzes_taken", 0)}</code>
• Total Questions: <code>{user_profile.get("total_questions", 0)}</code>
• Correct Answers: <code>{user_profile.get("total_correct", 0)}</code>
• Incorrect Answers: <code>{user_profile.get("total_incorrect", 0)}</code>
• Average Score: <code>{user_profile.get("avg_score", 0):.1f}%</code>
• Accuracy: <code>{accuracy:.1f}%</code>

<b>🔥 Activity Streaks</b>
• Current Streak: <code>{user_profile.get("streaks", {}).get("current", 0)}</code> days
• Best Streak: <code>{user_profile.get("streaks", {}).get("best", 0)}</code> days
"""
        ...
        if user_profile.get("categories"):
            ...
            for category, stats in user_profile.get("categories", {}).items():
                ...
                profile_text += f"\n• {category}: <code>{cat_accuracy:.1f}%</code> accuracy ({stats.get('correct', 0)}/{stats.get('total', 0)})"
...
<b>⏱️ Recent Activity</b>
• Today: <code>{user_profile.get("stats", {}).get("daily", {}).get("quizzes", 0)}</code> quizzes completed
• This Week: <code>{user_profile.get("stats", {}).get("weekly", {}).get("quizzes", 0)}</code> quizzes completed
• This Month: <code>{user_profile.get("stats", {}).get("monthly", {}).get("quizzes", 0)}</code> quizzes completed
"""
        ...
        await update.message.reply_html(
            ...
            reply_markup=InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton("📥 Download PDF Report", callback_data="download_profile_pdf")],
                [InlineKeyboardButton("📱 Check Premium Status", callback_data="check_premium")]
            ])
        )

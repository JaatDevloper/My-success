...
async def send_next_question(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if current_index + 1 >= len(questions):
        ...
        context.job_queue.run_once(
            lambda _: end_quiz_job(context, chat_id),
            35
        )
    else:
        # Otherwise schedule the next question after 35 seconds
        context.job_queue.run_once(
            lambda _: send_next_question_job(context, chat_id),
            35
        )

async def send_next_question_job(context, chat_id):
    """Function to send the next question from a job."""
    # Find the quiz for this chat
    for user_id, user_data in context.dispatcher.user_data.items():
        quiz = user_data.get("quiz", {})
        if quiz.get("active", False) and quiz.get("chat_id") == chat_id:
...
async def end_quiz_job(context, chat_id):
    """Function to end the quiz from a job."""
    # Find the user who started the quiz
    for user_id, user_data in context.dispatcher.user_data.items():
        quiz = user_data.get("quiz", {})
        if quiz.get("active", False) and quiz.get("chat_id") == chat_id:
            # Create a dummy update to pass to end_quiz
            class DummyUpdate:
                def __init__(self, chat_id, user_id):
                    self.effective_chat = type('obj', (object,), {'id': chat_id})
                    self.effective_user = type('obj', (object,), {'id': user_id})
